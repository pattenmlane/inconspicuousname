"""
ASH_COATED_OSMIUM — same Emerald MM as ``osmium_mm_emeraldstyle_fixed_10k.py``,
but **fair value = wall mid** each tick (Frankfurt Hedgehogs ``get_walls`` style):

  bid_wall = min(bid prices in book), ask_wall = max(ask prices),
  wall_mid = (bid_wall + ask_wall) / 2.

If depth is incomplete, falls back to **micro mid** (best bid + best ask) / 2, then
to **10_000** only if no two-sided book exists.

**Logging:** ``OSMIUM_WM_JSON`` lines (same shape as fixed-10k ``OSMIUM_EM_JSON``).

Backtest:
  PYTHONPATH="$PWD/imc-prosperity-4-backtester:$PWD/imc-prosperity-4-backtester/prosperity4bt" \\
  python3 -m prosperity4bt "$PWD/Round1/osmium_mm_emeraldstyle_wallmid.py" 1--2 \\
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        fair_value = _fair_from_depth(depth)
