"""
ASH_COATED_OSMIUM — Emerald MM like ``osmium_mm_emeraldstyle_wallmid.py``, but on a
**wall-mid spike** (|Δ raw wall| ≥ SPIKE vs previous tick) set FV to **previous tick’s
FV** and **keep that FV for exactly N trader ticks** (including the spike tick), then
return to tracking **current wall mid**.

Defaults: ``OSMIUM_WM_SPIKE=3``, ``OSMIUM_WM_FREEZE_TICKS=5``.

State JSON: ``prev_wall``, ``last_fv``, ``freeze_left``, ``frozen_fv``.

**Logging:** ``OSMIUM_WMFN_JSON``.

Backtest:
  PYTHONPATH="$PWD/imc-prosperity-4-backtester:$PWD/imc-prosperity-4-backtester/prosperity4bt" \\
  python3 -m prosperity4bt "$PWD/Round1/osmium_mm_emeraldstyle_wallmid_spike_freeze_n.py" 1--2 \\
