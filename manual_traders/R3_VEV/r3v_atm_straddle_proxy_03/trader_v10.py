"""
Iteration 10: Parent v5 with small ATM leg-tilt using IV skew (iv5100-iv5000).

Same strategy thesis; TTE hist day d -> (8-d) days from round3description.
Base params as v5 plus quantity tilt: slightly larger size on cheaper-IV leg.
"""
...
def _leg_tilt(iv0: float, iv1: float) -> tuple[float, float]:
    # if 5100 IV > 5000 IV, prefer 5000 leg size and vice versa; capped gentle tilt
    d = max(-0.03, min(0.03, iv1 - iv0))
    t = d / 0.03  # in [-1,1]
    # q0 multiplier, q1 multiplier
    return 1.0 + 0.15 * t, 1.0 - 0.15 * t
...
class Trader:
    def run(self, state: TradingState):
        ...
        if spread_ok and trade_short_bundle:
            base_q = min(ORDER_CLIP, POS_LIMITS[VEV_5000] + p0, POS_LIMITS[VEV_5100] + p1)
            base_q = max(0, base_q)
            m0, m1 = _leg_tilt(iv0, iv1)
            q0 = min(POS_LIMITS[VEV_5000] + p0, int(round(base_q * m0)))
            q1 = min(POS_LIMITS[VEV_5100] + p1, int(round(base_q * m1)))
            q0 = max(0, q0)
            q1 = max(0, q1)
            if q0 > 0:
                out[VEV_5000].append(Order(VEV_5000, bb0, -q0))
            if q1 > 0:
                out[VEV_5100].append(Order(VEV_5100, bb1, -q1))
            qh = min(q0, q1)
            hedge = int(round(unit_straddle_delta * qh))
            ...
            q1 = max(0, q1)
            if q0 > 0:
                out[VEV_5000].append(Order(VEV_5000, ba0, q0))
            if q1 > 0:
                out[VEV_5100].append(Order(VEV_5100, ba1, q1))
            qh = min(q0, q1)
            hedge = int(round(unit_straddle_delta * qh))
            if hedge > 0 and px > -POS_LIMITS[EXTRACT]:
                hq = min(hedge, POS_LIMITS[EXTRACT] + px, ORDER_CLIP * 3)
