"""
Iteration 17: Family10 plain control + same extract-propagation filter as v16.

Execution remains near v5, but fair-value signal uses plain quadratic smile fit; threshold SHOCK 0.20 as strict control variant.
"""
...
class Trader:
    def run(self, state: TradingState):
        ...
        pred = _fit_smile_quadratic_logm(S, T, mids, spreads, robust=False)
        if pred is not None:
        ...
        prev_bundle = td.get("_prev_bundle")
        dS = 0.0 if prev_S is None else (S - float(prev_S))
        dB = 0.0 if prev_bundle is None else ((mid0 + mid1) - float(prev_bundle))
        td["_prev_S"] = S
        td["_prev_bundle"] = (mid0 + mid1)

        # From tape analysis: beta_dC_dS approx 0.6535 (5000) + 0.5773 (5100) for contemporaneous move response.
        expected_bundle_move = 1.2308 * dS
        explained_by_underlying = abs(dS) >= 2.0 and (dB * dS > 0) and (abs(dB) <= 1.4 * abs(expected_bundle_move) + 1e-9)
        ...
        if spread_ok and (not explained_by_underlying) and trade_short_bundle:
        ...
        elif spread_ok and (not explained_by_underlying) and trade_long_bundle:
            q = min(ORDER_CLIP, POS_LIMITS[VEV_5000] - p0, POS_LIMITS[VEV_5100] - p1)
            q = max(0, q)
            if q > 0:
