"""
Iteration 18: Family10 robust (from v14) + vega-weighted mispricing floors + extract-shock scaling.

Tape analysis: simultaneous extract/voucher move correlation dominates; lead/lag beyond k=0 is weak. Hence we do not add
lag-based trading, but use per-tick BS vega at fitted smile IV to require larger |mid-fair| when vega is high, and scale
IV-shock threshold when |dS| is large (common co-move regime). TTE: hist day d -> (8-d) days.
"""
...
MISPRICE_EDGE = 25.0  # unused directly; vega floor below approximates this at typical vega
RV_WINDOW = 50
...
class Trader:
    def run(self, state: TradingState):
        ...
        mis_floor = VEGA_MISPRICE_BASE + VEGA_MISPRICE_PER_VEGA * vega_ref

        prev_s = td.get("_prev_S")
        dS = 0.0 if prev_s is None else (S - float(prev_s))
        td["_prev_S"] = S
        shock_edge = SHOCK_ABS_EDGE * (1.0 + SHOCK_DS_SCALE * min(1.0, abs(dS) / SHOCK_DS_REF))
        ...
        want_short = (mis > mis_floor) or (shock > shock_edge)
        want_long = (mis < -mis_floor) or (shock < -shock_edge)
