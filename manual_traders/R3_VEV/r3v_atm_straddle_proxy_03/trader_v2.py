"""
Iteration 2: ATM 5000/5100 cluster with shared-vol anchor + IV-shock z-score.

Keeps same thesis (coordinated ATM bundle legs on extract/IV shocks), but avoids
the identity issue of per-leg IV inversion by pricing both legs with a blended sigma.
...
class Trader:
    def run(self, state: TradingState):
        ...
        iv_avg = 0.5 * (iv0 + iv1)
        sigma_anchor = 0.65 * iv_avg + 0.35 * sigma_rv
        sigma_anchor = max(0.08, min(0.95, sigma_anchor))

        theo0 = _bs_call(S, K0, T, sigma_anchor)
        theo1 = _bs_call(S, K1, T, sigma_anchor)
        theo_bundle = theo0 + theo1
        bundle_mid = mid0 + mid1
        mis = bundle_mid - theo_bundle

        del0 = _bs_delta_call(S, K0, T, sigma_anchor)
        del1 = _bs_delta_call(S, K1, T, sigma_anchor)
        unit_straddle_delta = del0 + del1

        shock = iv_avg - sigma_rv
        shock_hist = td.get("_shock_hist", [])
        shock_hist.append(float(shock))
        if len(shock_hist) > 120:
            shock_hist = shock_hist[-120:]
        td["_shock_hist"] = shock_hist
        if len(shock_hist) >= 12:
            m = sum(shock_hist) / len(shock_hist)
            var = sum((x - m) ** 2 for x in shock_hist) / max(1, len(shock_hist) - 1)
        ...
        want_short = (mis > MISPRICE_EDGE) or (shock_z > IV_SHOCK_Z)
        want_long = (mis < -MISPRICE_EDGE) or (shock_z < -IV_SHOCK_Z)
