"""
Iteration 25 — vouchers_final_strategy: joint 5200/5300 tight gate + extract EMA reversion,
with **equal 5200+5300** delta bundle (Sonic: hedge into a tight surface; STRATEGY optional layer:
long/short both strikes with extract, short holds). Not legacy smile/ATM work.

- Gate: s(VEV_5200) <= TH, s(VEV_5300) <= TH, s(extract) <= MAX_EX_SPREAD.
- Edge (inclineGod): we still observe spreads; optional extra: only boost voucher size when
  combined wing spread s52+s53 is very tight.
- v24 falsified *momentum*; here we reuse v23 *mean reversion* direction for all three products.

No HYDROGEL_PACK. TTE context: historical day d -> 8-d days in round3description; engine injects
...
class Trader:
    def run(self, state: TradingState):
        ...
        if dev < -EXTRACT_FADE_EDGE:
            ...
            qv = min(vclip, POS_LIMITS[VEV_5200] - p52, POS_LIMITS[VEV_5300] - p53)
            qe = max(0, qe)
            qv = max(0, qv)
            if qe > 0:
