...
Params (iteration 0): CLOSEST_K=6, MM_EDGE=2.
"""
...
# Opening underlying mids at timestamp 0 (Prosperity4Data/ROUND_3/prices_round_3_day_*.csv)
...
SKEW_PER_UNIT = 0.08
EMA_ALPHA = 0.12
# Refit IV smile only every N ticks (bisection IV per strike is expensive at 10k steps/day).
REFIT_EVERY = 80
...
def polyfit2(xs: list[float], ys: list[float]) -> tuple[float, float, float] | None:
    """Return (a,b,c) for y = a*x^2 + b*x + c; least squares."""
    n = len(xs)
    if n < 2:
        return None
    s0 = float(n)
