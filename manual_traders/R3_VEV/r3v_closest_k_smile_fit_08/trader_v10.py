"""
Family 13 (distinct variant): RV-IV regime state machine with hysteresis + persistence.

Delta vs v9:
- Regime switching uses separate enter/exit thresholds (interpretable hysteresis).
- Require 2 consecutive qualifying ticks to switch regime (de-noising).
- Regime affects both voucher edge AND voucher size multipliers.

Core local-surface thesis unchanged: closest-k smile fit and MM off local surface.
"""
...
# RV-IV regime with hysteresis + persistence
RV_ALPHA = 0.06
Z_ALPHA = 0.01
ENTER_STRESSED = 1.10
EXIT_STRESSED = 0.55
