"""
Family 13 variant (response-aware regime widths): keep RV-IV hysteresis regime machine from v10,
and add strike-response/spread-aware edge shaping from tape analysis.

Key addition:
- Per-strike beta/downside asymmetry from analyze_response_spread_asymmetry.json is baked in as
  simple constants for interpretability.
- On stressed regime and downside z, widen more for high-beta and downside-sensitive strikes.
- Keep local-smile valuation unchanged and regime logic interpretable.
"""
...
# RV-IV regime with hysteresis + persistence (same as v10)
RV_ALPHA = 0.06
Z_ALPHA = 0.01
ENTER_STRESSED = 1.10
EXIT_STRESSED = 0.55
...
REGIME_EDGE_MULT = {"calm": 0.90, "neutral": 1.00, "stressed": 1.25}
REGIME_SIZE_MULT = {"calm": 1.10, "neutral": 1.00, "stressed": 0.85}

# Tape-derived response map (coarse averages from response_spread_asymmetry).
# Higher beta => stronger passthrough to underlying shocks; positive asym => reacts more on up moves.
BETA_BY_K = {
    4000: 0.745,
    4500: 0.662,
    5000: 0.653,
...
def _next_regime(prev: str, z: float, pending: str, pending_count: int) -> tuple[str, str, int]:
    target = prev
    if prev == "neutral":
        if z >= ENTER_STRESSED:
            target = "stressed"
    ...
    pending_count += 1
    if pending_count >= SWITCH_CONFIRM_TICKS:
        return target, "", 0
    return prev, pending, pending_count


def response_edge_bonus(k: int, z: float, regime: str) -> int:
    """Extra width for contracts that pass spot shocks strongly, especially in stressed downside."""
    beta = BETA_BY_K.get(k, 0.0)
    asym = ASYM_UP_MINUS_DOWN.get(k, 0.0)
    b = 0
    if regime == "stressed":
        if beta >= 0.62:
            b += 1
...
class Trader:
    def run(self, state: TradingState):
        ...
        if prev_s > 0 and s > 0:
            lr = math.log(s / prev_s)
            rv_var = float(td.get("rv_var", lr * lr))
            rv_var = (1.0 - RV_ALPHA) * rv_var + RV_ALPHA * (lr * lr)
            td["rv_var"] = rv_var
        else:
        ...
        if coeffs is not None:
            for k in sorted(STRIKES, key=lambda x: abs(float(x) - s))[:3]:
        ...
        prev_reg = str(td.get("regime", "neutral"))
        pending = str(td.get("pending_regime", ""))
        pending_count = int(td.get("pending_count", 0))
        regime, pending, pending_count = _next_regime(prev_reg, z, pending, pending_count)
        td["regime"] = regime
        td["pending_regime"] = pending
        td["pending_count"] = pending_count

        edge_mult = REGIME_EDGE_MULT[regime]
        ...
        for k in STRIKES:
            ...
            if vega > 220:
                ...
                edge_adj = 1
            edge_adj += response_edge_bonus(k, z, regime)
            edge = max(1, int(round(max(1.0, float(BASE_EDGE_VEV + edge_adj) * edge_mult))))
            ...
            if vega > 220:
                ...
                size = 8
            size = max(4, int(round(size * size_mult)))
