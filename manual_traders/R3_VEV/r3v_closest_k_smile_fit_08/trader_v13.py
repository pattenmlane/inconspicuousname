"""
Family 13: parent v12 + strike-tiered extract shock from pooled tape (voucher_lead_lag_shock.json).

Pooled |dU| p90=2.0. Mean |dV| on large-|dU| rows is much larger for low strikes than far OTM
(4000: ~1.75 vs 5500: ~0.12) while the shock/calm *ratio* is ~2.2x across the chain. v12 added a
flat +1 tick and size x0.88 on all strikes when |dU| is shock-sized. Here we add more edge and
a deeper size cut on strikes where the tape shows the largest absolute jump in voucher mid on
shocks, and a lighter (or no) extra edge on 5400+ where re-pricing is tiny.
"""
...
# Match tape: p90 |dU| on Round3 ~2 ticks.
SHOCK_ABS_DU = 2.0


def shock_extra_and_size_mult(strike: int) -> tuple[int, float]:
    """Tiers from mean_abs_dV_shock in voucher_lead_lag_shock.json (pooled, |dU|>=p90)."""
    if strike <= 5000:
        return 2, 0.82
    if strike <= 5300:
        return 1, 0.88
...
class Trader:
    def run(self, state: TradingState):
        ...
        for k in STRIKES:
            ...
            shock_edge_extra, shock_sm = shock_extra_and_size_mult(k)
            if du_shock:
                raw_edge += float(shock_edge_extra)
            ...
            if vega > 220:
                ...
                size = 8
            if du_shock:
                size = max(4, int(round(size * shock_sm)))
