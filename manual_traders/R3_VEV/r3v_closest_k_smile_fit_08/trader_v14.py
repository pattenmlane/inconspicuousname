"""
Family 13: v12 extract shock, restricted to 5000–5300 (from voucher_lead_lag_shock.json).

Pooled Pearson corr(dU,dV) at lag 0 is very high in the 5000–5200 band (~0.72–0.75) and still
nontrivial at 5300 (~0.62), while 4000/4500/5400+ show the same 2.2x shock/calm |dV| *ratio* but
either sit in a different microstructure (ultra-wide deep wings) or have tiny |dV| in levels (5500+).
Widening the whole chain on a generic extract shock can over-tighten away from the densest co-movement
band; this variant keeps the v12 +1 / x0.88 only where same-tick co-movement is strongest.
"""
...
SHOCK_ABS_DU = 2.0
SHOCK_EXTRA_EDGE_TICKS = 1
SHOCK_SIZE_MULT = 0.88

# Same-tick co-movement band (see voucher_lead_lag_shock.json xcorr "0" row).
SHOCK_STRIKES = frozenset({5000, 5100, 5200, 5300})
...
class Trader:
    def run(self, state: TradingState):
        ...
        for k in STRIKES:
            ...
            if du_shock and k in SHOCK_STRIKES:
                raw_edge += float(SHOCK_EXTRA_EDGE_TICKS)
            ...
            if vega > 220:
                ...
                size = 8
            if du_shock and k in SHOCK_STRIKES:
                size = max(4, int(round(size * SHOCK_SIZE_MULT)))
