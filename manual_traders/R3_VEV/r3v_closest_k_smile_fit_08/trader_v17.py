"""
Family 13: parent v15 + directional shock nudge on 5100 from tape (shock_spread_asymmetry.json).

Pooled, on |dU|>=p90 shock rows only, OLS dV~dU: at 5100, beta on down-moves slightly exceeds
beta on up (median ~0.535 vs 0.520) — the clearest up/down separation among 4500-5300. v15
already adds +1 to edge on shock for core+near strikes. On extract shock, add one more tick
on 5100 when dU<0 to lean away from higher repricing per tick in the down-shock bucket.
Parent v9 regime + v15 selective shock sizing unchanged.
"""
...
# From pooled median beta_shock_up vs _down in shock_spread_asymmetry.json (5100 strike).
SHOCK_DU_DOWN_EXTRA_5100 = 1.0
...
class Trader:
    def run(self, state: TradingState):
        ...
        d_u_move = m_u - prev_u
        du_shock = abs(d_u_move) >= SHOCK_ABS_DU and ts > 0
        du_down = d_u_move < 0.0
        td["prev_u"] = m_u
        ...
        for k in STRIKES:
            ...
            if (
                ...
                raw_edge += float(SHOCK_DU_DOWN_EXTRA_5100)
