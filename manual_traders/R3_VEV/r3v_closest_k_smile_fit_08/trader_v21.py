"""
Joint 5200/5300 tight BBO gate + family-13 (v15) local smile / RV-IV regime.

Shared thesis (round3work/vouchers_final_strategy/STRATEGY.txt): when VEV_5200 and VEV_5300
both have top-of-book spread <= 2 at the same timestamp, short-horizon extract mid dynamics
are more favorable; use as risk-on filter and to size extract + VEV. We do not trade
HYDROGEL_PACK (PnL objective = extract + vouchers only).

Implementation: spread = ask1 - bid1 on 5200/5300; tight = both <= TIGHT_SPREAD_TH.
Tight: slightly tighter extract quotes + larger U/VEV sizes. Wide: wider extract edge + smaller
sizes (defensive). Keeps v15 shock overlay on selected strikes.
"""
...
# Tight gate (STRATEGY.txt default TH=2)
TIGHT_SPREAD_TH = 2

# Extract: risk-on when gate tight (smaller edge, larger size per thesis)
MM_EDGE_U_TIGHT = 1
ORDER_SIZE_U_TIGHT = 18
MM_EDGE_U_WIDE = 3
ORDER_SIZE_U_WIDE = 8

# Vouchers: scale base size; vega tiers still apply below
BASE_ORDER_SIZE_VEV_TIGHT = 14
BASE_ORDER_SIZE_VEV_WIDE = 10
...
def _spread(d: OrderDepth) -> int | None:
    ba = _best_bid_ask(d)
    if ba is None:
        return None
    return ba[1] - ba[0]
...
class Trader:
    def run(self, state: TradingState):
        ...
        d52 = depths.get(SY_5200)
        d53 = depths.get(SY_5300)
        sp52 = _spread(d52) if d52 else None
        sp53 = _spread(d53) if d53 else None
        joint_tight = (
            sp52 is not None
            and sp53 is not None
            and sp52 <= TIGHT_SPREAD_TH
            and sp53 <= TIGHT_SPREAD_TH
        )
        td["joint_tight"] = joint_tight
        td["sp_5200"] = sp52
        td["sp_5300"] = sp53
        ...
        if joint_tight:
            mm_edge_u = MM_EDGE_U_TIGHT
            order_sz_u = ORDER_SIZE_U_TIGHT
            base_vev = BASE_ORDER_SIZE_VEV_TIGHT
        else:
            mm_edge_u = MM_EDGE_U_WIDE
            order_sz_u = ORDER_SIZE_U_WIDE
            base_vev = BASE_ORDER_SIZE_VEV_WIDE
        ...
        orders[U] = mm_sym(U, d_u, s, mm_edge_u, order_sz_u)
        ...
        for k in STRIKES:
            ...
            size = base_vev
            if vega > 220:
                size = min(20, base_vev + 6)
            elif vega > 120:
                size = min(18, base_vev + 2)
            elif vega < 20:
                size = max(6, base_vev - 4)
            if du_shock and k in SHOCK_CORE_STRIKES:
