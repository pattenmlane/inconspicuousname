"""
Parent v15: no hydrogel; joint VEV_5200/VEV_5300 tight BBO gate sizes extract only (STRATEGY.txt).

When both spreads (ask1-bid1) <= 2, use tighter extract edge + larger U size (short-horizon mid edge).
Otherwise keep v15 extract defaults. Voucher quoting identical to v15 (local smile, RV-IV regime,
vega, shock overlay).
"""
...
MM_EDGE_U = 2
ORDER_SIZE_U = 12
# Joint tight gate: risk-on extract only
TIGHT_SPREAD_TH = 2
MM_EDGE_U_TIGHT = 1
...
class Trader:
    def run(self, state: TradingState):
        ...
        if joint_tight:
            u_edge, u_sz = MM_EDGE_U_TIGHT, ORDER_SIZE_U_TIGHT
        else:
            u_edge, u_sz = MM_EDGE_U, ORDER_SIZE_U
        ...
        orders[U] = mm_sym(U, d_u, s, u_edge, u_sz)
