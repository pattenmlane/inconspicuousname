"""
Round 3 VEV: local smile (closest-6) + vega-scaled MM — iteration 4 on v3.

Wing-aware edges: in addition to vega-based edge_adj, add ticks when |log(K/S)| is large,
matching tape where median top-of-book spreads are much wider for deep OTM/ITM than
near-ATM (see spread_median_by_product.json). Keeps the same local surface fit; does not
change extract/hydrogel quoting. DTE: round3 time map (d_open=8-csv_day, intraday wind-down).
"""
...
MM_EDGE_H = 2
BASE_EDGE_VEV = 2
# Extra half-spread in ticks for wings by log-moneyness (tape: wide books on 4000/4500/6000/6500).
WING_LM1 = 0.10
WING_BONUS1 = 2
WING_LM2 = 0.15
WING_BONUS2 = 1  # additional on top of BONUS1 when |log m| > 0.15
ORDER_SIZE_U = 12
ORDER_SIZE_H = 15
...
def wing_edge_bonus(s: float, k: float) -> int:
    lm = abs(math.log(k / s))
    b = 0
    if lm > WING_LM1:
        b += WING_BONUS1
    ...
    def run(self, state: TradingState):
        ...
        for k in STRIKES:
            ...
            if vega > 220:
                ...
                edge_adj = 1
            wing_b = wing_edge_bonus(s, float(k))
            edge = max(1, BASE_EDGE_VEV + edge_adj + wing_b)
