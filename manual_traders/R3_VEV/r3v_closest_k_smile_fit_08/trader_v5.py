"""
Round 3 VEV: local smile (closest-6) + vega-scaled MM — iteration 5 (parent: v3, variant of v4).

Lighter wing edge than v4: a single +2 tick bonus only when |log(K/S)| > 0.12 (avoids
penalizing the 0.10–0.15 moneyness band that still trades often). Otherwise identical to v3.
"""
    ...
    def run(self, state: TradingState):
        ...
        for k in STRIKES:
            ...
            if vega > 220:
                ...
                edge_adj = 1
            edge = max(1, BASE_EDGE_VEV + edge_adj + wing_edge_bonus(s, float(k)))
