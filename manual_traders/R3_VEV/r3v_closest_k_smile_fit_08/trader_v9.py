"""
Family 13 variation: realized-vol regime state machine on top of local-smile MM.

Regime signal (interpretable):
- Compute annualized realized vol from extract log-return EWMA.
- Compute near-ATM IV from local smile fit (median of 3 closest strikes IVs).
- z = (rv_ann - iv_atm - mu) / sigma  (online EWMA mu/var)
- Regime:
    calm: z <= -0.8
    stressed: z >= 0.8
    neutral: otherwise
...
Policy:
- Keep same closest-k local smile fairing.
- Keep same vega-based edge/size.
- Multiply voucher edge by regime multiplier:
    calm 0.85, neutral 1.00, stressed 1.30
"""
...
# RV-IV regime settings
REGIME_LOW_Z = -0.8
REGIME_HIGH_Z = 0.8
REGIME_EDGE_MULT = {"calm": 0.85, "neutral": 1.00, "stressed": 1.30}
RV_ALPHA = 0.06  # EWMA of squared log-return
Z_ALPHA = 0.01   # EWMA of spread mean/var
...
class Trader:
    def run(self, state: TradingState):
        ...
        # Realized vol tracker from extract
        ...
        # ATM-ish IV from fitted surface (use 3 closest strikes)
