"""
Round 3 — cross-strike IV rank fade (r3v_cross_strike_iv_rank_07), iteration v1.

Same core as v0 (IV rank → fade bottom/top decile with neighbor IV cap) but:
- Parameter sweep: looser NEIGHBOR_IV_CAP (0.12 vs 0.09) so more decile trades pass the
  neighbor filter (analysis p90 neighbor gap ~0.20 on subsampled tapes).
- No VELVETFRUIT_EXTRACT delta hedge: v0’s passive hedge dominated PnL negatively under
  --match-trades worse.

Timing / TTE: same as v0 (round3description + plot_iv_smile_round3 intraday winding).

Products: VEV_* only in practice (allowed set includes extract/hydrogel but not quoted here).
"""
