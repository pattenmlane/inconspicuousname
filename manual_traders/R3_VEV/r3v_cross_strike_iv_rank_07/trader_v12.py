"""
Round 3 — cross-strike IV rank fade (r3v_cross_strike_iv_rank_07), iteration v12.
...
Same as v10, but rank key uses vega-weighted smile residuals (residual * (1 + vega/100)).
"""
...
class Trader:
    def run(self, state: TradingState):
        ...
        if res is None or len(res) != n:
            ...
            if RANK_MODE == "vega_weighted":
                rank_key = []
                for i in range(n):
                    sig = ivs[i]
                    K = k_list[i]
                    if sig <= 1e-9 or K <= 0 or T <= 0:
                        rank_key.append(res[i])
                        continue
                    d1 = (math.log(S / K) + 0.5 * sig * sig * T) / (sig * math.sqrt(T))
                    vg = float(S * norm.pdf(d1) * math.sqrt(T))
                    rank_key.append(res[i] * (1.0 + vg / 100.0))
            else:
                rank_key = res
