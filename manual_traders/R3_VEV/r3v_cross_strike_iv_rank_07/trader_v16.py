"""
Round 3 — cross-strike IV rank fade (r3v_cross_strike_iv_rank_07), iteration v16.
...
Realized-vol regime variant: regime = EWMA realized vol minus ATM IV, with cooldown=15 ticks and regime-based width/inventory caps. threshold=0.02.
"""
...
# Family 13 regime: EWMA RV vs ATM IV
RV_ALPHA = 0.06
...
def _regime_params(regime: str) -> tuple[int, int, int]:
    # (quote width offset, inv cap, size multiplier%)
    if regime == "high":
        return 2, 220, 85
    if regime == "low":
        return 0, 280, 120
...
def _nearest_atm_iv(S: float, k_list: list[float], ivs: list[float]) -> float | None:
    if not k_list or not ivs:
        return None
    best_i = min(range(len(k_list)), key=lambda i: abs(k_list[i] - S))
    v = ivs[best_i]
...
class Trader:
    def run(self, state: TradingState):
        ...
        # EWMA RV from underlying returns vs ATM IV regime signal
        ...
        if isinstance(prev_s, (int, float)) and float(prev_s) > 0:
            ...
            inst_var = ret * ret * (365.0 * 10_000.0)
            rv_ewma = (1.0 - RV_ALPHA) * rv_ewma + RV_ALPHA * inst_var
        td["prev_S"] = S
        td["rv_ewma"] = rv_ewma

        atm_iv = _nearest_atm_iv(S, k_list, ivs)
        rv_sigma = math.sqrt(max(rv_ewma, 0.0)) if rv_ewma > 0 else 0.0
        raw_sig = rv_sigma - (atm_iv if atm_iv is not None else rv_sigma)
        target_regime = "mid"
