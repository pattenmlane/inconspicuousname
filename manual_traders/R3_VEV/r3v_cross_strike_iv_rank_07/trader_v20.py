"""
Round 3 — cross-strike IV rank fade (r3v_cross_strike_iv_rank_07), iteration v20.
...
Propagation-aware directional gate: keep smile-residual decile ranking, but only buy low-residual strikes on up-move or sell high-residual strikes on down-move when strike beta>= 0.25.
"""
...
BETA_MIN = 0.25
BETA_BY_STRIKE = {4000:0.8147904803620967,4500:0.8863297384121156,5000:0.869793125175021,5100:0.7623754588358679,5200:0.5794965915049817,5300:0.36400104876769795,5400:0.17097535395909805,5500:0.0750480685194896,6000:0.0,6500:0.0}
...
class Trader:
    def run(self, state: TradingState):
        ...
        dS = S - S_raw if isinstance(S_raw, (int, float)) else 0.0
        for i in range(n):
            sym = syms[i]
            ...
            if neighbor_rank_gap(i) > NEIGHBOR_RESIDUAL_CAP:
                continue
            k_i = int(round(k_list[i]))
            beta_i = BETA_BY_STRIKE.get(k_i, 0.0)
            q_use = BASE_Q
            ...
            buy_ok = (dS >= 0.0 and beta_i >= BETA_MIN)
            sell_ok = (dS <= 0.0 and beta_i >= BETA_MIN)

            if i in low_pair and buy_ok:
                qb = min(q_use, lim - pos_i)
                if qb > 0 and bb2 + 1 < ba2:
                    add_o(sym, bb2 + 1, qb)
            if i in high_pair and sell_ok:
