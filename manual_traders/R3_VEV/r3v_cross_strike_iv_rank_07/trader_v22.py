"""
Round 3 — r3v_cross_strike_iv_rank_07, iteration v22.

Same as v14 (smile-residual decile fade, NEIGHBOR_RESIDUAL_CAP=0.12, stride=1) but applies
**soft** propagation-aware risk from historical dP/dS betas (underlying_propagation_lag_elasticity.json):
- per-strike size multiplier from beta_t (stronger flow ~ larger size within caps);
- slightly widened effective neighbor residual cap for low-beta wings (more trades where
  propagation is weak / smile neighbor gaps are often borderline), without hard side filters.
...
# Contemporaneous dP/dS betas from analysis_outputs/underlying_propagation_lag_elasticity.json (step-5 sample, R3 days 0–2).
...
def _effective_neighbor_cap(beta: float) -> float:
    # ITM / high-beta: keep base cap; low-beta wings: allow slightly wider neighbor gap.
    ...
    def run(self, state: TradingState):
        ...
        for i in range(n):
            ...
            if neighbor_rank_gap(i) > cap_i:
                continue
            q_use = int(round(BASE_Q * sm))
            q_use = max(1, min(lim, q_use))
            if i > 0 and ((i - 1) in low_pair or (i - 1) in high_pair):
                q_use = min(MAX_CLUSTER_Q, int(round(BASE_Q * 2 * sm)))
                q_use = max(1, min(lim, q_use))
            if i < n - 1 and ((i + 1) in low_pair or (i + 1) in high_pair):
                q_use = min(MAX_CLUSTER_Q, max(q_use, int(round(BASE_Q * 2 * sm))))
                q_use = max(1, min(lim, q_use))
