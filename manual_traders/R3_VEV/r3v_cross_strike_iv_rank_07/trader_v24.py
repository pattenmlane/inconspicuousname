"""
Round 3 — r3v_cross_strike_iv_rank_07, iteration v24.

Parent v23: smile-residual decile fade + soft beta_t size multipliers (no neighbor-cap slack).

Adds **soft microstructure / propagation stress** sizing (no hard gates):
- Track extract mid S_raw at each action tick; scale size by 1/(1 + MOVE_SHRINK * |ΔS|/S) clipped,
  so a fast underlying step reduces clip size before re-stabilizing.
- Per voucher, scale by L1 spread vs SPREAD_EXTRA_REF (from tape: median spread ~2 ticks, p90 ~17):
  wider books get a mild size haircut (adverse-selection proxy), still passive join-bid/offer.
