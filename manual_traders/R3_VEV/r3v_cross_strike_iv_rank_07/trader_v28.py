"""
Round 3 — r3v_cross_strike_iv_rank_07, iteration v28.

v27 (smile-residual decile, beta/theta/move/spread soft sizing) +
**joint VEV_5200 & VEV_5300 tight book gate** from round3work/vouchers_final_strategy/STRATEGY.txt:
if both L1 spreads <= TH=2, treat as risk-on (size * TIGHT_SIZE_MULT + small join-bid long on extract);
else size * WIDE_SIZE_MULT. No hard VEV off-switch.
...
# Joint 5200/5300 tight gate (STRATEGY.txt, TH=2 ticks)
TIGHT_TH = 2
TIGHT_SIZE_MULT = 1.25
WIDE_SIZE_MULT = 0.45
# Optional short-hold long extract when gate is on (position limit 200)
EXTRACT_TIGHT_Q = 3
EXTRACT_LIM = 200
...
class Trader:
    def run(self, state: TradingState):
        ...
        # Joint tight gate: 5200 & 5300 both have spread <= TIGHT_TH (same as tape analysis)
        sym_5200 = _sym_for_product(state, "VEV_5200")
        sym_5300 = _sym_for_product(state, "VEV_5300")
        gate: float
        if sym_5200 and sym_5300:
            d520 = depths.get(sym_5200)
            d530 = depths.get(sym_5300)
            bb5, ba5 = _best_ba(d520)
            bb3, ba3 = _best_ba(d530)
            if (
                bb5 is not None
                and ba5 is not None
                and bb3 is not None
                and ba3 is not None
                and (ba5 - bb5) <= TIGHT_TH
                and (ba3 - bb3) <= TIGHT_TH
            ):
                gate = TIGHT_SIZE_MULT
            else:
                gate = WIDE_SIZE_MULT
        else:
            gate = WIDE_SIZE_MULT
        ...
        if gate == TIGHT_SIZE_MULT and sym_u is not None:
            pos_e = int(pos.get(sym_u, 0))
            qe = min(EXTRACT_TIGHT_Q, EXTRACT_LIM - pos_e)
            if qe > 0 and uba is not None and ubb is not None and ubb + 1 < uba:
                add_o(sym_u, ubb + 1, qe)
