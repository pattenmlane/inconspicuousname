"""
r3v_cross_strike_iv_rank_07 — iteration v32 (vouchers_final_strategy).

Same joint gate (5200+5300 L1 spread<=2) as v30, but **only** trade
VELVETFRUIT_EXTRACT + VEV_5200 + VEV_5300 when tight — no other strikes (Sonic: hedge
into tight surface; avoid off-anchor flow risk per inclineGod per-contract spread noise).
"""
...
WARMUP = 5
ANCHOR_PRODS = ("VEV_5200", "VEV_5300")
EXTRACT_LIM = 200
VEV_LIM = 300
...
class Trader:
    def run(self, state: TradingState):
        ...
        td[_EMA_KEY] = ema_s
        td["csv_day"] = _update_csv_day(td, ts, s_raw)
        ...
        _add_buy(out, sym_u, ubb, uba, EXTRACT_BUY_Q, int(pos.get(sym_u, 0)), EXTRACT_LIM)

        for prod in ANCHOR_PRODS:
