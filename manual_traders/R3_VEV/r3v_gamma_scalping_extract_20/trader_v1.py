"""
Gamma-scalping extract v1: v0 without hydrogel noise; softer extract hedge (partial delta).

Parameter grid vs v0 (documented for iteration sweep):
  - GAMMA_THRESH 9e-5 (was 1.2e-4) — slightly more quotes when gamma is meaningful
  - DELTA_HEDGE_FRAC 0.62 — under-hedge to reduce churn / whip on day-2 tape
  - HEDGE_BAND 6, OPTION_SIZE 16
"""
...
class Trader:
    def run(self, state: TradingState) -> tuple[dict[str, list[Order]], int, str]:
        ...
        if iv_ema is None:
            ...
            iv_ema = 0.1 * iv + 0.9 * float(iv_ema)
        store["iv_ema"] = iv_ema
        ...
        target_ex = int(round(-DELTA_HEDGE_FRAC * opt_delta_exposure))
        target_ex = max(-LIMITS[EXTRACT], min(LIMITS[EXTRACT], target_ex))

        if pos_ex > target_ex + HEDGE_BAND:
            bb, ba = _best(d_ex)
            if bb is not None:
                q = min(pos_ex - target_ex, pos_ex + LIMITS[EXTRACT], 20)
                if q > 0:
            ...
            if ba is not None:
                ...
                q = min(target_ex - pos_ex, room_buy, 20)
                if q > 0:
