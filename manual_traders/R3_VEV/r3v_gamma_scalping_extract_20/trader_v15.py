"""
v15: day-1 asymmetric extract hedge around target (distinct from prior scalar sweeps).

Parent: trader_v13. Keep same option logic and day-2 no-hedge rule.
On day 1 only, hedge sells faster than buys around target to reduce the
extract leak seen in prior runs while avoiding full reversion to v4 settings.
"""
...
class Trader:
    def run(self, state: TradingState) -> tuple[dict[str, list[Order]], int, str]:
        ...
        if tape_day < 2:
            ...
            if tape_day == 1:
                band_sell = DAY1_BAND_SELL
                band_buy = DAY1_BAND_BUY
            else:
                band_sell = HEDGE_BAND
                band_buy = HEDGE_BAND
            if pos_ex > target_ex + band_sell:
                bb, ba = _best(d_ex)
                if bb is not None:
                    q = min(pos_ex - target_ex, pos_ex + LIMITS[EXTRACT], 18)
                    if q > 0:
                        result[EXTRACT].append(Order(EXTRACT, bb, -q))
            elif pos_ex < target_ex - band_buy:
                bb, ba = _best(d_ex)
                if ba is not None:
