"""
v16: v13 + day-1 delta-weighted option clip split.

Data motivation:
- Voucher response beta to extract is monotone by strike distance (VEV_5200 >
  VEV_5300 > VEV_5400 around the trading region), while spread shock ratio stays
  near 1.0 across days.
- On day 1 we therefore allocate larger clip to the lower-delta of the two
  nearest strikes (farther strike) to reduce extract bleed while keeping long
  near-ATM gamma.
"""
    ...
    def run(self, state: TradingState) -> tuple[dict[str, list[Order]], int, str]:
        ...
        if tape_day == 1 and len(syms) == 2:
            ...
            if d0 is not None and d1 is not None:
                # Larger clip on lower-delta strike to soften extract exposure.
                if d0 > d1:
