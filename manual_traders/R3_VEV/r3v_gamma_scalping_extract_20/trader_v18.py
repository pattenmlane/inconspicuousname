"""
v18: v17 plus regime-adaptive extract hedge fraction from 20-step trend sign.

Analysis shows day-specific extract behavior at 20-step horizon:
- day 1: momentum (past20 and future20 positively aligned)
- day 2: reversal (past20 and future20 negatively aligned)
So we soften hedge in day-1 momentum direction and tighten in day-2 reversal.
"""
...
TREND_WIN = 20
DAY1_HEDGE_MOMENTUM = 0.38
DAY2_HEDGE_REVERSION = 0.85
...
class Trader:
    def run(self, state: TradingState) -> tuple[dict[str, list[Order]], int, str]:
        ...
        ex_hist = list(store.get("ex_mid_hist", []))
        ex_hist.append(float(S))
        if len(ex_hist) > TREND_WIN + 5:
            ex_hist = ex_hist[-(TREND_WIN + 5):]
        store["ex_mid_hist"] = ex_hist
        trend20 = 0.0
        if len(ex_hist) >= TREND_WIN + 1:
            trend20 = ex_hist[-1] - ex_hist[-1 - TREND_WIN]
        ...
        if tape_day < 2:
            ...
            if tape_day == 1 and trend20 > 0:
                hedge_frac = DAY1_HEDGE_MOMENTUM
        else:
            # day2 reversal regime: hold tighter extract delta in direction of recent move.
            hedge_frac = DAY2_HEDGE_REVERSION if trend20 != 0 else DELTA_HEDGE_FRAC
        if tape_day < 3:
        ...
        if tape_day == 1 and len(syms) == 2:
            ...
            if d0 is not None and d1 is not None:
                # v17: larger clip on higher-delta (higher extract-beta) strike.
                if d0 > d1:
