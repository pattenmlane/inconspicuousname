"""
v19: v17 + day-1-only trend-adaptive extract hedge fraction.

Data motivation:
- At 20-step horizon, day-1 extract shows mild continuation while day-2 shows
  mean reversion (see analysis iteration 12). To avoid v18's day-2 blow-up, we
  only adapt hedge fraction on day 1 and keep day-2 policy unchanged.
"""
...
DAY1_HEDGE_BAND = 12
DAY1_TREND_WINDOW = 20
DAY1_HEDGE_FRAC_UP = 0.40
DAY1_HEDGE_FRAC_DOWN = 0.58
...
class Trader:
    def run(self, state: TradingState) -> tuple[dict[str, list[Order]], int, str]:
        ...
        # Keep short history of extract mid for day-1 trend gating.
        hist = store.get("extract_hist")
        if not isinstance(hist, list):
            hist = []
        hist.append(float(S))
        if len(hist) > DAY1_TREND_WINDOW + 5:
            hist = hist[-(DAY1_TREND_WINDOW + 5):]
        store["extract_hist"] = hist
        ...
        if tape_day < 2:
            ...
            if tape_day == 1 and len(hist) > DAY1_TREND_WINDOW:
                trend = hist[-1] - hist[-1 - DAY1_TREND_WINDOW]
                hedge_frac = DAY1_HEDGE_FRAC_UP if trend > 0 else DAY1_HEDGE_FRAC_DOWN
