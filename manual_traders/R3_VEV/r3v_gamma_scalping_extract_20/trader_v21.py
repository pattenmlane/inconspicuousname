"""
v21: v17 with response-per-spread strike selection.

From response_per_spread_rank.json, VEV_5100 consistently outranks VEV_5300 as
the second contract behind VEV_5200 (higher |dOpt/dS| relative to spread).
Instead of always quoting the two nearest strikes, quote:
  - primary = nearest strike to S
  - secondary = nearest of {5100, 5300} not equal to primary
This remains a near-ATM gamma concept but changes strike mix by tape evidence.
"""
...
def select_strikes(S: float) -> tuple[int, int]:
    """Primary nearest strike plus data-ranked companion from {5100, 5300}."""
    primary = min(STRIKES, key=lambda x: abs(float(x) - S))
    candidates = [k for k in (5100, 5300) if k != primary]
    if not candidates:
        ranked = sorted(STRIKES, key=lambda x: abs(float(x) - S))
        second = ranked[1]
    else:
        second = min(candidates, key=lambda x: abs(float(x) - S))
    return (primary, second) if primary < second else (second, primary)
