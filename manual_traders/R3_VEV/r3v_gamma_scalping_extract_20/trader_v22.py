"""
v22: v20 + **joint tight two-leg book gate** (round3work/vouchers_final_strategy/STRATEGY.txt).

When **VEV_5200** and **VEV_5300** both have BBO spread (ask1−bid1) **≤ 2** at the
same time, the historical analysis shows more favorable short-horizon extract-mid
dynamics. Here we use that as a **risk filter only**: if either leg is wide, we
**skip** posting new VEV market-making quotes (still run extract hedging and keep
TTE/IV gating logic). TTE extended to tape day 3 (5d) after `day_3` data merge;
mapping follows round3work/round3description.txt pattern: 8/7/6/5 for days 0..3.
"""
...
# Joint gate: both ATM-adjacent legs tight (Sonic / inclineGod write-up, TH=2).
GATE_S5200 = "VEV_5200"
GATE_S5300 = "VEV_5300"
SPREAD_TH = 2
...
def _bbo_spread(depth: OrderDepth | None) -> int | None:
    """Level-1 ask − bid, same units as the tape; None if one side missing."""
    bb, ba = _best(depth)
