"""
v24: v23 follow-up — **meaningful wide-regime clip reduction** (not a no-op).

v23 used **0.5×** with rounding; for our 5/6 day-1 base clips that rounded back to
5 and 3 (same as unscaled). Here **wide** uses `floor(base * 0.39)` (min 1) so
5→1 and 6→2, while **tight** keeps full base clips. Same v20 hedge + day1
GAMMA_THRESH relaxation; TTE 8/7/6/5 for tape days 0..3.
"""
...
def _scale_clip_for_gate(base: int, *, gate_tight: bool) -> int:
    if gate_tight:
        return max(1, int(base))
    return max(1, int(float(base) * WIDE_CLIP_FLOOR_MULT))
...
class Trader:
    def run(self, state: TradingState) -> tuple[dict[str, list[Order]], int, str]:
        ...
        for sym in syms:
            ...
            clip = _scale_clip_for_gate(base_clip, gate_tight=gate_tight)
            if room_buy > 0:
