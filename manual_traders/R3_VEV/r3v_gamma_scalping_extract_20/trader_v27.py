"""
v27: Same design center as v26 (vouchers_final_strategy only: Sonic joint gate
VEV_5200+VEV_5300 both ≤2; off = flat; on = long extract toward target on early
tape days + MM both legs; no hydrogel). **Sonic “per-event edge”**: once the
surface is **tight**, we **size up** VEV activity (STRATEGY layer 1 “size up”
when the gate is on) and requote the surface every tick (no MM bucket skip).

- **OPTION_CLIP** 10 → 24 (clamped by position limits; spread-as-signal already
  passed via joint gate)
- **REQUOTE_EVERY** 2 → 1
"""
