"""
v28: v26 + **extract BBO spread filter** (inclineGod: track spreads per
contract, not just mids; STRATEGY uses inner-join on extract + 5200 + 5300).
When joint **VEV** gate is on, we only run the **optional** long-extract
directional layer if **VELVETFRUIT_EXTRACT** L1 spread is also “reasonable”
(default **≤ 4** ticks), so we do not pay a huge half-spread to express the
mid-forward toy. Surface MM (5200+5300) runs whenever the joint VEV gate is on
unchanged. Last tape day still skips extract targeting (v26 ablation). No
legacy BS/gamma.
"""
...
SPREAD_TH = 2
# Tighter extract = cheaper to build/rebalance long; aligned with "trade when
# execution is not noise" (STRATEGY caveat on bid/ask vs mid)
EXTRACT_SPREAD_MAX_FOR_TARGET = 4
...
class Trader:
    def run(self, state: TradingState) -> tuple[dict[str, list[Order]], int, str]:
        ...
        s_ex = _bbo_spread(d_ex)
        store["s_extract"] = s_ex
        can_build_extract = s_ex is not None and s_ex <= EXTRACT_SPREAD_MAX_FOR_TARGET

        if (
            can_build_extract
            and tape_day < SKIP_EXTRACT_TARGET_FROM_TAPE_DAY
            and bb_e is not None
            and ba_e is not None
        ):
