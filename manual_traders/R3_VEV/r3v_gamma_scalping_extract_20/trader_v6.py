"""
v6: keep v4 day-2 no-hedge logic, but let profitable long-gamma inventory run longer.

Parameter sweep vs v4:
- same quotes / hedge settings
- only change: do not auto-offer inventory each cycle; sell back only when net option
  position gets large. This tests whether inventory unwinding was capping gamma gains.
"""
...
class Trader:
    def run(self, state: TradingState) -> tuple[dict[str, list[Order]], int, str]:
        ...
        net_opt_pos = sum(int(state.position.get(s, 0)) for s in syms)
        for sym in syms:
            ...
            # Keep long-gamma inventory unless aggregate option position is too large.
            if room_sell > 0 and pos > 0 and net_opt_pos > MAX_NET_LONG_OPT:
