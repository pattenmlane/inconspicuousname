"""
v8: v4 with tape-day-2 tighter option quotes (MM_EDGE=0 on day 2 only).

Rationale: vega on nearest strikes falls as TTE shortens; we tried one tick
tighter on day 2. Under match-trades worse, MM_EDGE=0 can yield zero taker
matches; backtest regressed to 710 total (do not use as production variant).
"""
...
class Trader:
    def run(self, state: TradingState) -> tuple[dict[str, list[Order]], int, str]:
        ...
        edge = 0 if tape_day == 2 else MM_EDGE
        ...
        for sym in syms:
            ...
            bid_px = min(bb + edge, ba - 1)
            sell_px = max(ba - edge, bb + 1)
            if room_buy > 0:
