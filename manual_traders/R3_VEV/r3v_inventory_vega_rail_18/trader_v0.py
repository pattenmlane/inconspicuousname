"""
Round 3 — inventory vega rail (r3v_inventory_vega_rail_18).

Thesis: cap proxy vega / |position| per strike (vega from BS at smile IV) so inventory
does not accumulate where mark-to-model liquidation risk is large when FV mean-reverts.

TTE: round3work/round3description.txt (historical day 0,1,2 -> DTE at open 8,7,6) plus
intraday winding from round3work/plotting/original_method/combined_analysis/plot_iv_smile_round3.py:
  DTE_eff = max(DTE_open - (timestamp//100)/10000, 1e-6), T = DTE_eff/365 years.

IV model: global quadratic smile in m = log(K/S)/sqrt(T) (coeffs from voucher_work fit).

Execution: Frankfurt-style IV scalping on each VEV (theo diff vs wall_mid EMAs), with
per-strike order size capped by min(300, floor(VEGA_RAIL/vega) - |pos|). Partial delta
hedge on VELVETFRUIT_EXTRACT; light MM on HYDROGEL_PACK.
"""
...
class Trader:
    def run(self, state: TradingState):
        ...
        if ts // 100 < WARMUP_STEPS:
            bag["ema"] = ema_store
            return orders_out, 0, json.dumps(bag, separators=(",", ":"))
