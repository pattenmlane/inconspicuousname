"""
Round 3 — inventory vega rail (iteration 1).

Changes vs v0 (parameter sweep / fixes):
- Looser IV-scalp gate (IV_SCALPING_THR 0.7 -> 0.35) so directional scalps fire when
  theo-diff volatility is moderate, not only in extreme regimes.
- Higher VEGA_RAIL (700 -> 900) to allow slightly larger clips where vega is low.
- Softer delta hedge (0.22 -> 0.12) and only act when |hedge| >= 3 to cut churn on extract.
- No HYDROGEL_PACK quoting (v0 showed hydrogel PnL was masking poor option/extract mix).
- Merge multiple orders at the same price per symbol so hedge + MM do not violate limits.

TTE / smile: same as trader_v0 (round3description + plot_iv_smile_round3 intraday winding).
"""
...
IV_SCALPING_THR = 0.35
LOW_VEGA_CUTOFF = 1.0
THEO_NORM_WINDOW = 20
IV_SCALPING_WINDOW = 100
WARMUP_STEPS = 10
DELTA_HEDGE_FRAC = 0.12
HEDGE_MIN_ABS = 3
EXTRACT_EDGE = 2
EXTRACT_MM_Q = 5
