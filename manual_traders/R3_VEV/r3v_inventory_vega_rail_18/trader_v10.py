"""
Round 3 — inventory vega rail (iteration 10).

Parent: v8. **IV activity gate** IV_SCALPING_THR: tape `switch_mean` median is ~0.23
(see analysis_outputs/switch_mean_quantiles.json subsample); 0.7 is almost never met.
Set gate to **0.25** (just above median) so the main Frankfurt scalping branch can run
when volatility-of-theo_diff is normal. No extract hedge; same vega_rail.
"""
...
# --- tunables (iteration 10) ---
VEGA_RAIL = 650.0
THR_OPEN = 0.35
THR_CLOSE = 0.0
LOW_VEGA_THR_ADJ = 0.5
IV_SCALPING_THR = 0.25
LOW_VEGA_CUTOFF = 1.0
THEO_NORM_WINDOW = 20
