"""
Round 3 — inventory vega rail (iteration 17).

Parent: v12 (+130). Pivot to family 12: smile_cubic_spline_logk.\nExecution coupling: when vega concentration is high, skew execution stronger toward de-risking current sign; softer otherwise.
"""
...
def iv_scalp_orders(
    ...
    skew = SKEW_STRONG if conc_hhi >= SKEW_CONC_HHI else SKEW_SOFT
    # Stronger de-risk skew when vega is concentrated: ease exits, harden adds.
    open_sell_thr = THR_OPEN - (skew if pos > 0 else -0.0)
    open_buy_thr = THR_OPEN - (skew if pos < 0 else -0.0)
    open_sell_thr = max(0.05, open_sell_thr)
    open_buy_thr = max(0.05, open_buy_thr)
...
class Trader:
    def run(self, state: TradingState):
        ...
        lim_v = 300
        conc_hhi = vega_concentration_hhi(S, T)
