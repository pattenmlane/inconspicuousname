"""
Round 3 — inventory vega rail (iteration 19).

Parent: v12 (+130), adjusted distinct family-12 variant.\nExecution coupling uses OWN inventory-vega concentration per strike (continuous),\nnot market-wide HHI threshold gating.
"""
...
SPLINE_LOGK = np.log(np.array(STRIKES, dtype=float))
# Family 12 proxy: spline anchors in IV-space by strike (logK).
SPLINE_IV = np.array([0.37, 0.34, 0.30, 0.29, 0.28, 0.275, 0.27, 0.265, 0.255, 0.245], dtype=float)
_IV_SPLINE = CubicSpline(SPLINE_LOGK, SPLINE_IV, bc_type="natural")

# --- tunables (iteration 19) ---
VEGA_RAIL = 650.0
K_OVER_S_MIN = 0.97
K_OVER_S_MAX = 1.03
THR_OPEN = 0.35
...
def iv_smile(S: float, K: float, T: float) -> float:
    if S <= 0 or K <= 0 or T <= 0:
        return float("nan")
    # family 12: cubic spline in log-strike for IV surface
    lk = math.log(float(K))
    iv = float(_IV_SPLINE(lk))
    return max(0.05, iv)
...
def vega_concentration_hhi(S: float, T: float) -> float:
    vegas = []
    for K in STRIKES:
        iv = iv_smile(S, K, T)
    ...
    tot = sum(max(v, 0.0) for v in vegas)
    if tot <= 0:
        return 0.0
    w = [max(v, 0.0) / tot for v in vegas]
    return float(sum(x * x for x in w))



def own_vega_shares(
    state_pos: dict[str, int],
    S: float,
    T: float,
    state: TradingState,
) -> dict[str, float]:
    ...
    for v in VOUCHERS:
        ...
        weighted[sym_v] = abs(p) * max(vega, 0.0)
    tot = sum(weighted.values())
    if tot <= 1e-12:
        return {}
    return {k: val / tot for k, val in weighted.items()}
...
def iv_scalp_orders(
    ...
    # Continuous own-inventory vega concentration coupling.
    skew = OWN_SKEW_BASE + OWN_SKEW_SCALE * max(0.0, own_share)
    if pos > 0:
        # long: reduce threshold to sell (de-risk), increase to buy more
        open_sell_thr = max(0.05, THR_OPEN - skew)
        open_buy_thr = THR_OPEN + skew
    elif pos < 0:
        # short: reduce threshold to buy (cover), increase to sell more
        open_buy_thr = max(0.05, THR_OPEN - skew)
        open_sell_thr = THR_OPEN + skew
    else:
        open_sell_thr = THR_OPEN
        open_buy_thr = THR_OPEN
    if switch_mean >= IV_SCALPING_THR:
        if current_theo_diff - wall_mid + best_bid - mean_theo_diff >= (open_sell_thr + low_vega_adj) and max_sell > 0:
        ...
        elif current_theo_diff - wall_mid + best_ask - mean_theo_diff <= -(open_buy_thr + low_vega_adj) and max_buy > 0:
...
class Trader:
    def run(self, state: TradingState):
        ...
        lim_v = 300
        own_share_map = own_vega_shares(pos, S, T, state)
        ...
        for v in VOUCHERS:
            ...
            bids, asks = iv_scalp_orders(
                ...
                max_buy,
                max_sell,
                float(own_share_map.get(sym_v, 0.0)),
            )
            ol: list[Order] = []
            for px, q in bids:
                q = min(q, lim_v - pos_v)
