"""
Round 3 — inventory vega rail (iteration 21).

Parent: v12. Analysis-driven update from extract->voucher propagation: use per-strike beta to scale allowed inventory (lower for weak-response strikes) and add spread-shock guard after large extract moves.
"""
...
# --- tunables (iteration 21) ---
VEGA_RAIL = 650.0
EXTRACT_SHOCK = 12
SPREAD_GUARD_MULT = 1.45
K_OVER_S_MIN = 0.97
K_OVER_S_MAX = 1.03
THR_OPEN = 0.35
THR_CLOSE = 0.0
...
# From analysis_outputs/extract_move_propagation_by_contract.json
BETA_BY_STRIKE = {
    4000: 0.7451, 4500: 0.6618, 5000: 0.6535, 5100: 0.5773, 5200: 0.4367,
    5300: 0.2727, 5400: 0.1289, 5500: 0.0549, 6000: 0.0, 6500: 0.0,
}
BASE_SPREAD = {
    4000: 4.0, 4500: 5.0, 5000: 4.0, 5100: 3.0, 5200: 3.0,
    5300: 3.0, 5400: 2.0, 5500: 1.0, 6000: 1.0, 6500: 1.0,
}
...
class Trader:
    def run(self, state: TradingState):
        ...
        du = 0.0 if prev_u is None else (S - float(prev_u))
        bag["_u_mid"] = float(S)
        ...
        for v in VOUCHERS:
            ...
            if not math.isfinite(sig) or sig <= 0:
                continue
            # shock spread guard: skip opening after large underlying jump if spread is widened
            curr_spread = best_ask - best_bid
            if abs(du) >= EXTRACT_SHOCK and curr_spread > SPREAD_GUARD_MULT * BASE_SPREAD.get(K, 3.0):
                continue
            ...
            beta_w = max(0.25, BETA_BY_STRIKE.get(K, 0.2) / 0.75)
            room = max(0, int(beta_w * min(lim_v, vega_cap)) - abs_p)
