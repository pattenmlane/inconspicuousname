"""
Round 3 — inventory vega rail (iteration 29).

Parent: v25 (+146). Add continuous du_ema regime gate scaling (mild): higher |du_extract| EMA lowers effective IV switch gate.
"""
...
def iv_scalp_orders(
    ...
    gate = max(0.05, IV_SCALPING_THR * gate_scale)
    if switch_mean >= gate:
...
class Trader:
    def run(self, state: TradingState):
        ...
        prev_u_mid = bag.get("_u_mid")
        du_ema = float(bag.get("_du_ema", DU_EMA_BASE))
        ...
        if isinstance(prev_u_mid, (int, float)):
            du_ema = ema(ema_store, "_du_ema", DU_EMA_WINDOW, abs(S - float(prev_u_mid)))
        bag["_u_mid"] = float(S)
        bag["_du_ema"] = float(du_ema)
        ...
        for v in VOUCHERS:
            ...
            gate_scale = max(DU_GATE_MIN, min(DU_GATE_MAX, 1.0 - DU_GATE_K * (du_ema - DU_EMA_BASE)))
