"""
Round 3 — joint tight gate, VEV-only mean reversion (v33).

Same thesis as trader_v32 (vouchers_final_strategy only) but:
- **No VELVETFRUIT_EXTRACT** quotes (STRATEGY caveat: mid-forward edge != bid/ask PnL;
  v32 lost heavily on extract+VEV volume in worse matching).
- Trade **only** VEV_5100 / VEV_5200 / VEV_5300 when 5200+5300 spreads both <= 2.
- Smaller clips, higher EDGE vs v32 to reduce over-trading in trends.

Position limit 300 per VEV.
"""
...
TRADE: tuple[str, ...] = ("VEV_5100", "VEV_5200", "VEV_5300")
TH = 2
EMA_WINDOW = 80
EDGE = 2.5
WARMUP = 8
