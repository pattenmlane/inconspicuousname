"""
r3v_jump_gap_filter_17 — iteration 0

Concept: after a large extract move (vs slow EMA), widen OTM / pause directional IV scalps;
resume when neighbor-strike pricing residuals (wall_mid − BS theo) compress near ATM.

TTE: plot_iv_smile_round3.t_years_effective(csv_day, ts) with csv_day inferred once per
session at ts==0 from first VELVETFRUIT_EXTRACT mid vs tape probes (days 0–2).
...
class Trader:
    def run(self, state: TradingState):
        ...
        jump = abs(u_mid - s_ema) >= _JUMP_SHELL or dS >= 8.0
        ...
        # --- Neighbor residual spread (ATM cluster) ---
        resid_spread = 0.0
        ...
        allow_v = (not jump) and compressed and ts // 100 >= warmup

        # --- VEV: Frankfurt IV scalp when not jumping and neighbors aligned ---
        if allow_v:
            for k in STRIKES:
                ...
                ind = compute_option_indicators(
                    cal, ema_store, u_mid, k, T_base, float(wm2), float(bb2), float(ba2), sym_o
                )
                ...
                for px, q in asks:
                    ...
                    if q > 0:
                        olist.append(Order(sym_o, int(px), -q))
                for px, q in bids:
                    q = min(int(q), lim_v - pos_sym)
                    if q > 0:
                        olist.append(Order(sym_o, int(px), q))
        ...
        # --- Extract: light mean-reversion when jump (reduce delta shock) ---
        if sym_u and jump:
            pos_u = int(pos.get(sym_u, 0))
            u_olist: list[Order] = []
            if u_mid > s_ema + 5 and pos_u > -lim_u + 20:
                u_olist.append(Order(sym_u, int(uba), -min(12, pos_u + lim_u)))
            elif u_mid < s_ema - 5 and pos_u < lim_u - 20:
                u_olist.append(Order(sym_u, int(ubb), min(12, lim_u - pos_u)))
            if u_olist:
                orders_out[sym_u] = u_olist

        # --- Hydrogel: passive MM only while paused (inventory softener) ---
        if sym_h and jump and sym_h not in orders_out:
            dh = depths.get(sym_h)
            if dh is not None:
                _, _, _, _, hb, ha, _ = book_from_order_depth(dh)
                if hb is not None and ha is not None:
                    pos_h = int(pos.get(sym_h, 0))
                    hm = 0.5 * (float(hb) + float(ha))
