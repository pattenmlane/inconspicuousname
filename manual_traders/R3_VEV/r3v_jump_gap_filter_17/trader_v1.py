"""
r3v_jump_gap_filter_17 — iteration 1

Fix vs v0: jump filter was effectively always on (|mid − slow EMA| stays wide during trends).
Now: pause VEV only for a short cooldown after a *discrete* extract step jump (|ΔS| large).

Neighbor residuals: max(wall_mid − BS_theo) − min(·) over strikes near ATM; resume scalping
when this spread is below NEIGH_RESID_MAX (grid: 8.0 here vs 6.0 in v0).
