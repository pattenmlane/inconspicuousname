"""
r3v_jump_gap_filter_17 — iteration 12 (family12 smile_cubic_spline_logk)

Variation target: jump-aware refit policy for smile_cubic_spline_logk.

Model: cubic spline in log-moneyness x = log(K/S), fit to per-timestamp implied vols
computed from tape mids and T=t_years_effective.

Refit policy:
- immediate refit after large extract move (|ΔS| >= _JUMP_DS),
- otherwise slower cadence (every _REFIT_CADENCE ticks).

Use this smile for theo on nearest-ATM voucher; keep pause/resume concept and hydrogel
behavior from prior best hybrids.
...
def _fit_spline_logk(u_mid: float, iv_by_strike: dict[int, float]) -> tuple[list[float], list[float]] | None:
    xs: list[float] = []
    ys: list[float] = []
    for k in STRIKES:
        iv = iv_by_strike.get(int(k))
        if iv is None or not math.isfinite(iv) or iv <= 0:
            continue
        xs.append(math.log(float(k) / float(u_mid)))
        ys.append(float(iv))
    if len(xs) < 4:
        return None
    pair = sorted(zip(xs, ys), key=lambda t: t[0])
    x = [t[0] for t in pair]
    y = [t[1] for t in pair]
    return x, y


def _safe_eval_spline(x_nodes: list[float], y_nodes: list[float], xq: float) -> float | None:
    try:
        cs = CubicSpline(np.asarray(x_nodes), np.asarray(y_nodes), extrapolate=True)
        v = float(cs(float(xq)))
...
class Trader:
    def run(self, state: TradingState):
        ...
        # family12: smile_cubic_spline_logk refit policy
        model = bucket.get("smile_model") if isinstance(bucket.get("smile_model"), dict) else {}
        x_nodes = model.get("x") if isinstance(model.get("x"), list) else []
        y_nodes = model.get("y") if isinstance(model.get("y"), list) else []
        last_refit = int(bucket.get("last_refit_tick", -10**9))
        need_refit = (not x_nodes) or (tick - last_refit >= _REFIT_CADENCE) or (dS >= _JUMP_DS)
        if need_refit:
            iv_map: dict[int, float] = {}
            for k in STRIKES:
                sym_k = _symbol_for_product(state, f"VEV_{k}")
                if sym_k is None:
                    continue
                depth_k = depths.get(sym_k)
                if depth_k is None:
                    continue
                _, _, bw_k, aw_k, bb_k, ba_k, wm_k = book_from_order_depth(depth_k)
                _, _, wm_k2, bb_k2, ba_k2 = synthetic_walls_if_missing(bw_k, aw_k, bb_k, ba_k)
                if wm_k2 is None:
                    continue
                T_k = float(t_years_effective(int(bucket.get("csv_day", 0)), ts))
                try:
                    from plot_iv_smile_round3 import implied_vol_call as _iv_call
                    iv = _iv_call(float(wm_k2), float(u_mid), float(k), float(T_k), 0.0)
                except Exception:
                    iv = float("nan")
                if math.isfinite(iv) and iv > 0:
                    iv_map[int(k)] = float(iv)
            fit = _fit_spline_logk(float(u_mid), iv_map)
            if fit is not None:
                x_nodes, y_nodes = fit
                bucket["smile_model"] = {"x": x_nodes, "y": y_nodes}
                bucket["last_refit_tick"] = tick
        ...
        for k in STRIKES:
            ...
            if wm2 is None:
                continue
            # Use cubic-spline smile when available; fall back to coeff-based theo.
            xq = math.log(float(k) / float(u_mid))
            iv_s = _safe_eval_spline(x_nodes, y_nodes, xq) if x_nodes and y_nodes else None
            if iv_s is not None:
                try:
                    from plot_iv_smile_round3 import bs_call_price as _bs_call
                    theo = float(_bs_call(float(u_mid), float(k), float(T_base), float(iv_s), 0.0))
                except Exception:
                    theo = float("nan")
            else:
