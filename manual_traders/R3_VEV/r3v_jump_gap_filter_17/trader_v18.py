"""
Family-12: LOO spline fair, intraday T, jump/cadence refit; v17 jump gate; v18 edges.

v18 (iter 18): **Post-shock quoting edges + OTM widening** on top of v17.

Tape `analysis_vev_spread_after_shocks.json`: top-of-book spread is mostly 2 ticks
on shock vs calm (median ratio 1), so we do not key off raw spread levels.
Next-tick spread change on |dS_ex|>=3 is mildly positive at **5100** and **4000**
(~0.03–0.05 ticks mean), weak elsewhere — microstructure noise after shocks.

We still implement "widen after jump" via **fair edges**: on |dS|>=3 reset a
shock-age counter; for the next ~20–40 ticks add extra take/make edge decaying as
exp(-age/tau). **OTM** strikes (K/S > 1.02) scale that bump up to 1.35×
(Greek: higher-strike calls are less delta-one, more vega/noise vs extract).

Signed tilt (v15), neighbor ptp pause/resume (v17) unchanged. TTE: t_years_effective.
"""
...
# Post-shock edge bump (decays with ticks since last |dS_ex|>=_JUMP_DS)
_SHOCK_EDGE_TAU_TICKS = 22.0
_SHOCK_TAKE_ADD = 2.0
_SHOCK_MAKE_ADD = 1.15
_OTM_KS = 1.02
_OTM_WIDEN_MAX = 1.35
...
def shock_edge_decay_weight(bu: dict[str, Any], tick: int, dS: float) -> float:
    """1.0 right after extract jump tick, decays exponentially with tick age."""
    if dS >= _JUMP_DS:
        bu["shock_tick"] = int(tick)
        return 1.0
    st = bu.get("shock_tick")
    if not isinstance(st, int):
        return 0.0
    age = max(0, int(tick) - int(st))
    return float(math.exp(-age / _SHOCK_EDGE_TAU_TICKS))


def otm_widen_mult(K: float, s_opt: float) -> float:
    if s_opt <= 0:
        return 1.0
    ks = float(K) / float(s_opt)
    if ks <= _OTM_KS:
        return 1.0
    span = 0.10
    t = min(1.0, max(0.0, (ks - _OTM_KS) / span))
    return 1.0 + t * (_OTM_WIDEN_MAX - 1.0)
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        shock_w = shock_edge_decay_weight(bu, tick, dS)
        ...
        if not vev_pause:
            for sym, K in zip(VEV_SYMS, STRIKES):
                ...
                lim = LIMITS[sym]
                om = otm_widen_mult(float(K), s_opt)
                te = self.TAKE_EDGE + _SHOCK_TAKE_ADD * shock_w * om
                me = self.MAKE_EDGE + _SHOCK_MAKE_ADD * shock_w * om
                out[sym].extend(
                    self._vev_orders(sym, d, int(pos), lim, fair_a, te, me, self.MM_SIZE, self.TAKE_SIZE)
                )
