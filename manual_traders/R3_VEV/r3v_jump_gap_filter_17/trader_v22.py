"""
v22: **vouchers_final_strategy** only — see round3work/vouchers_final_strategy/STRATEGY.txt and
ORIGINAL_DISCORD_QUOTES.txt (Sonic: both 5200+5300 spread at or below 2; inclineGod: spreads per leg).

- **Tight** = both VEV_5200 and VEV_5300 have ask_1 - bid_1 <= 2 (TOB, same as tape / analysis).
- **Wide regime** (not tight): VEV orders **off**, extract only with smaller clip (STRATEGY: do not
  trust small edge when you cannot hedge into a tight surface).
- **Fair**: LOO cubic spline IV in log(K) (family-12) + BS call; `need_refit` on **cadence** only
  (no extract-jump refit, no post-jump pause — those were the deprioritized launch line).
- **Short tape (day 3)**: downscale VEV size when gate is on and reduce extract clip vs full days
  to limit wing tail (learned from v21 soft-gate day-3 loss).

**No HYDROGEL_PACK**. T = t_years_effective (round3work/round3description.txt DTE map).
"""
...
_REFIT_CADENCE = 40
_TD_KEY = "r3v_spline22"
# R3 `prices_round_3_day_3` is 1k rows vs 10k: scale down clips / VEV size when the gate is on.
_SHORT_DAY = 3
_VEV_SIZE_SCALE_SHORT = 0.6
_EX_CLIP_TIGHT_SHORT = 15
_EX_CLIP_WIDE_SHORT = 5
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        is_short = int(day_idx) == _SHORT_DAY
        v_scale = _VEV_SIZE_SCALE_SHORT if is_short and joint_t else 1.0
        mms = max(1, int(round(self.MM_SIZE * v_scale)))
        tks = max(1, int(round(self.TAKE_SIZE * v_scale)))
        if is_short and joint_t:
            ex_tight, ex_wide = _EX_CLIP_TIGHT_SHORT, _EX_CLIP_WIDE_SHORT
        else:
            ex_tight, ex_wide = 25, 10
        ...
        ex_clip = ex_tight if joint_t else ex_wide
        out[EXTRACT].extend(
            self._quote_extract(
        ...
        if joint_t:
            for sym, K in zip(VEV_SYMS, STRIKES):
                ...
                out[sym].extend(
                    self._vev_orders(
                        sym, d, int(pos), lim, fair, self.TAKE_EDGE, self.MAKE_EDGE, mms, tks
                    )
                )
    ...
    def _append_extract_only(
        ...
        *,
        ...
        d = int(_tape_day())
        if d == _SHORT_DAY and joint_tight:
            ex_clip = _EX_CLIP_TIGHT_SHORT
        elif d == _SHORT_DAY and not joint_tight:
            ex_clip = _EX_CLIP_WIDE_SHORT
        else:
