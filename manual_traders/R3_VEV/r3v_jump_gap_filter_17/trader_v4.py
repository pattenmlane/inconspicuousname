"""
r3v_jump_gap_filter_17 — iteration 4

Grid vs v3: revert negative naive diff-EMA; keep jump pause + neighbor residual filter.

Replace Frankfurt *switch_mean* gate (inactive on tape) with scalping on
current_theo_diff vs its EMA (mean_theo_diff) from compute_option_indicators —
same BS smile IV, delta, vega in the indicator dict.

One-tick aggressive limits (sell bb-1, buy ba+1). Single nearest-ATM strike only
(simpler, fewer conflicting signals than v0–v2 multi-strike).
"""
