"""
r3v_jump_gap_filter_17 — iteration 6

Parent: v1 (best total PnL tie). Keep thesis: after large extract jump, pause/widen OTM
and only resume when neighbor residuals compress.

Grid update: shorten pause to 80 ticks and make the neighbor compression gate dynamic:
if ATM vega is low, require tighter compression before re-enabling voucher risk.

TTE: t_years_effective(csv_day, ts); csv_day from first-tick extract mid vs day-0/1/2 probes.
"""
...
_EXTRACT_EMA_W = 400
_JUMP_DS = 3.0
_PAUSE_UNTIL_TICK = 80
_NEIGH_RESID_BASE = 8.0
...
class Trader:
    def run(self, state: TradingState):
        ...
        # Regime-aware compression gate: as ATM vega gets lower, residual alignment
        # must be tighter before we resume voucher exposure after jumps.
        k_atm = min(STRIKES, key=lambda kk: abs(float(kk) - u_mid))
        theo_atm, _delta_atm, vega_atm = get_option_values(float(u_mid), float(k_atm), float(T_base), coeffs)
        vega_adj = 1.0 if (not math.isfinite(vega_atm) or vega_atm < 0.9) else 0.0
        neigh_cap = _NEIGH_RESID_BASE - vega_adj
        compressed = resid_spread <= neigh_cap and len(wall_mids) >= 3
