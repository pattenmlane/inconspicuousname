"""
r3v_jump_gap_filter_17 — iteration 7

Parents: v1 (best PnL — jump pause + neighbor gate + hydrogel on pause) and v4 (VEV fills
via current_theo_diff − mean_theo_diff; Frankfurt touch + switch_mean produced 0 VEV trades).

Thesis unchanged: pause/widen after extract step jump; resume only when neighbor
residuals compress; then scalp mispriced wall vs smile theo on nearest-ATM strike.

Grid vs v4: slightly lower scalp threshold 0.40 and clip 20 (v4 used 0.42 / 22).
