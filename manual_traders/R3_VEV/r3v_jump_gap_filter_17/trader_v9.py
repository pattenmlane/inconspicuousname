"""
r3v_jump_gap_filter_17 — iteration 9

Parent: v8 (jump pause, no extract on pause, hydrogel MM on pause, v4-style theo_diff scalp).

Grid vs v8: tighter neighbor residual cap 6.5 (closer smile alignment before resuming VEV),
higher scalp threshold 0.45, smaller clip 18 — direction from v5 grid on the theo signal.

Tape note: nearest-ATM VEV bid-ask width is almost always 2–3 ticks (see
analysis_atm_vev_spread.json); spread is not the binding filter here.

TTE: t_years_effective(csv_day, ts); csv_day from first-tick extract vs day-0/1/2 probes.
"""
                    ...
                    if depth_o is not None:
                        ...
                        if wm2 is not None and bb2 is not None and ba2 is not None:
                            ...
                            if ctd is not None and mtd is not None and vega >= _LOW_VEGA:
                                ...
                                if dev > _SCALP_THR and pos_sym > -lim_v + 5:
                                    q = min(_MAX_VEV_QTY, lim_v + pos_sym)
                                    if q > 0:
                                        orders_out[sym_o] = [Order(sym_o, sell_px, -q)]
                                elif dev < -_SCALP_THR and pos_sym < lim_v - 5:
                                    q = min(_MAX_VEV_QTY, lim_v - pos_sym)
                                    if q > 0:
