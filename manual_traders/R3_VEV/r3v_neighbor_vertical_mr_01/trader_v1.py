"""Neighbor vertical MR v1: day-aware TTE, per-day EWMA reset, delta hedge on extract.

Day / TTE: CSV timestamps restart each historical day; on timestamp rollback we increment
day_idx. TTE_days = 8 - day_idx for day_idx in {0,1,2}, matching round3description example
(historical days 1/2/3 -> 8/7/6d) aligned to our CSV days 0/1/2.

Greek: Black–Scholes delta (calls) at IV from mid; hedge VELVETFRUIT_EXTRACT to ~neutralize
bundle delta (integer contracts, capped by limits).
"""
...
# --- iteration 1 (sweep vs v0): tighter entry, slower updates, smaller clips, delta hedge ---
ALPHA = 0.03
Z_THRESHOLD = 2.35
Z_SCALE = 2.8
BASE_SIZE = 8
...
class Trader:
    def run(self, state: TradingState):
        ...
        if best_z > 0:
            ...
            # short high call, long low call -> delta ~ q*(dl - dh)
            net_d = q * (dl - dh)
        else:
            ...
            # long high call, short low call -> delta ~ q*(dh - dl)
            net_d = q * (dh - dl)
