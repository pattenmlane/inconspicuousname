"""Family10 v12: v11 smile + elasticity-per-spread edge selection.

Tape analysis (analysis_spread_elasticity_r3.json): mean L1 half-spread falls from ~10.4 (VEV_4000)
to ~1.4 (VEV_5200) while OLS dOption/dU beta also falls; opportunity proxy beta/mean_h peaks near
VEV_5200 and VEV_5100. v12 ranks candidate strikes by score = |z| * BETA_EMP / max(half_spread, eps)
among agreement-filtered symbols, with fallback to max |z| if all betas are zero.
Same dual WLS in m=log(K/S)/sqrt(T), EWMA, OPEN_Z/CLOSE_Z, beta-capped hedge as v11.
"""
