"""Family10 v14: v12 + joint VEV_5200/VEV_5300 tight book gate (shared STRATEGY.txt thesis).

round3work/vouchers_final_strategy/STRATEGY.txt: when BOTH VEV_5200 and VEV_5300 have
top-of-book full spread (ask1-bid1) <= TIGHT_MAX_SP, short-horizon extract regime is
favorable; use as risk-on filter. inclineGod: book state, not price alone.
Implementation: do not *open* new VEV+hedge legs unless joint gate is on; always allow
mean-reversion *close* when |z| < CLOSE_Z. When gate on, size up by TIGHT_SIZE_MULT.
Spread uses same definition as analysis: min(ask) - max(bid) on L1 = 2*half in tick units.
"""
...
GATE_5200 = "VEV_5200"
GATE_5300 = "VEV_5300"
# STRATEGY.txt default: both legs <= 2 in tape price units (full width)
TIGHT_MAX_SP = 2.0
TIGHT_SIZE_MULT = 1.25
...
def _l1_spread_width(d) -> float | None:
    """Full top-of-book width = min(ask) - max(bid) (same as ask1-bid1 on L1)."""
    if not d.buy_orders or not d.sell_orders:
        return None
    return float(min(d.sell_orders) - max(d.buy_orders))


def _joint_tight_books(d) -> bool:
    if GATE_5200 not in d or GATE_5300 not in d:
        return False
    w0 = _l1_spread_width(d[GATE_5200])
    w1 = _l1_spread_width(d[GATE_5300])
    if w0 is None or w1 is None or w0 < 0 or w1 < 0:
        return False
    return w0 <= TIGHT_MAX_SP and w1 <= TIGHT_MAX_SP
...
class Trader:
    def run(self, state: TradingState):
        ...
        for s in SYMS:
            ...
            if be <= 0.0:
                continue
            sc = az * be / max(hs[s], SPREAD_EPS)
            if sc > best_score or (sc == best_score and az > abs(best_z)):
                best_score = sc
                best = s
                best_z = z
        if best is None:
