"""vouchers_final_strategy only: v15 core + VELVETFRUIT_EXTRACT overlay (low churn).

- Sonic gate: both VEV_5200 and VEV_5300 L1 spread (ask1-bid1 style) <= 2.
- Pair: D=mid(5200)-mid(5300) EWMA+z, mean-rev when gate and |z|>=ZO (same as v15).
- Extract (STRATEGY layer-2, execution-aware): when gate is **on**, **slowly** build toward
  E_TARGET with at most one **small** buy every E_COOLDOWN ticks (not every bar — avoids
  the -6k+ churn failure). When gate is **off**, one-shot **flatten long** at bid.
- Still no other VEV strikes; no hydrogel.
"""
...
QMAX = 28
# extract overlay: target long only under gate; wide spacing to limit turnover
E_TARGET = 55
E_LOT = 3
E_COOLDOWN = 25
...
class Trader:
    def run(self, state: TradingState):
        ...
        if pre is not None and ts < pre:
            ...
            td["ticks"] = 0
            td["last_ex_tick"] = -E_COOLDOWN
        td["prev_ts"] = ts
        tick = int(td.get("ticks", 0)) + 1
        td["ticks"] = tick
        # allow first add once cooldown satisfied after session start
        lxt = int(td.get("last_ex_tick", -E_COOLDOWN))

        d0, d1, du = dep[S5200], dep[S5300], dep[SEX]
        a0, a1 = _mid(d0), _mid(d1)
        if a0 is None or a1 is None:
            return o, 0, json.dumps(td)
        diff = a0 - a1
        pe = state.position.get(SEX, 0)
        ...
        if tight:
            ...
            td["ew_mu"] = mu0
            td["ew_var"] = v0
            if pe > 0 and du.buy_orders:
                qe = min(pe, 120, pe)
                o[SEX].append(Order(SEX, max(du.buy_orders), -qe))
            td["last_ex_tick"] = tick
            p0, p1 = state.position.get(S5200, 0), state.position.get(S5300, 0)
        ...
        if (
            pe < E_TARGET
            and (tick - lxt) >= E_COOLDOWN
            and du.sell_orders
        ):
            add = min(E_LOT, E_TARGET - pe, LIMITS[SEX] - pe, 20)
            if add > 0:
                o[SEX].append(Order(SEX, min(du.sell_orders), add))
                td["last_ex_tick"] = tick
        else:
            td["last_ex_tick"] = lxt
