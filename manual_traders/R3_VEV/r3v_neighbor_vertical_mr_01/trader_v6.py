"""Neighbor vertical MR v6: v5 + net-gamma-aware clip scaling for selected vertical.

Same neighbor-gap thesis, EWMA, Z_THRESHOLD, vega sizing, TTE, delta hedge as v2.
Change: among edges with |z| >= Z_THRESHOLD, maximize
  score_i = |z_i| / max(half_spread_i + half_spread_{i+1}, SPREAD_EPS)
instead of maximizing |z| alone. Trades the dislocation that is largest **per unit of
two-leg book friction** (sizes to two-leg costs at the microstructure level).

SPREAD_EPS avoids division blowups on zero-width books.

New greek control: compute BS gamma for both option legs and shrink size by
  gamma_scale = GAMMA_REF / (GAMMA_REF + |gamma_hi - gamma_lo|)
so very high local convexity verticals get smaller clips.
"""
...
def _bs_gamma_call(S: float, K: float, T: float, sigma: float) -> float:
    if T <= 0 or sigma <= 0 or S <= 0:
        return 0.0
    st = math.sqrt(T)
    d1 = (math.log(S / K) + 0.5 * sigma * sigma * T) / (sigma * st)
    n1 = math.exp(-0.5 * d1 * d1) / math.sqrt(2 * math.pi)
    return n1 / (S * sigma * st)
...
class Trader:
    def run(self, state: TradingState):
        ...
        for i in range(N_EDGE):
            ...
            if az < Z_THRESHOLD:
                continue
            two_leg = spreads[i] + spreads[i + 1]
            denom = max(two_leg, SPREAD_EPS)
            s = az / denom
            if s > best_score:
                best_score = s
                best_z = z
        ...
        gh = _bs_gamma_call(Su, Kh, T, sh)
        gl = _bs_gamma_call(Su, Kl, T, sl)

        two_leg = spreads[best_i] + spreads[best_i + 1]
        edge_scale = max(two_leg, 2.0)
        zfac = min(best_absz / Z_SCALE, 2.0)
        gamma_scale = GAMMA_REF / (GAMMA_REF + abs(gh - gl))
        gamma_scale = max(0.35, min(1.0, gamma_scale))
        raw = int(BASE_SIZE * zfac * (200.0 / vega_norm) * (10.0 / edge_scale) * gamma_scale)
