"""Family10 distinct variant: dual-cutoff quadratic smile agreement.

Distinct from parent branch exploration:
- Build TWO WLS quadratic smiles in log-moneyness each tick:
  cutoff_lo = 0.5 extrinsic, cutoff_hi = 1.5 extrinsic
- Trade only when both models agree on residual sign for the candidate strike,
  and use conservative execution gates.

This is a nearby but different assumption from single-cutoff smile variants.
"""
...
def _fit_wls(xs,ys,ws):
    ...
    for x,y,w in zip(xs,ys,ws):
        x2=x*x
        s00+=w; s01+=w*x; s02+=w*x2
        s11+=w*x*x; s12+=w*x2*x; s22+=w*x2*x2
...
def _fit_for_cutoff(cutoff,S,T,mids,hs,ivs,xs):
    fx=[];fy=[];fw=[]
...
class Trader:
    def run(self,state:TradingState):
        ...
        day_idx=int(td.get('day_idx',0)); T=_tte_years(day_idx)
        ...
        c_lo=_fit_for_cutoff(CUTOFF_LO,S,T,mids,hs,ivs,xs)
        c_hi=_fit_for_cutoff(CUTOFF_HI,S,T,mids,hs,ivs,xs)
        if c_lo is None or c_hi is None: return orders,0,json.dumps(td)
        ...
        for sym,K in zip(SYMS,STRIKES):
            x=xs[sym]
            ivl=max(0.01,c_lo[0]+c_lo[1]*x+c_lo[2]*x*x)
            ivh=max(0.01,c_hi[0]+c_hi[1]*x+c_hi[2]*x*x)
            fl=_bs_call(S,float(K),T,ivl)
            fh=_bs_call(S,float(K),T,ivh)
            r_lo=mids[sym]-fl
            r_hi=mids[sym]-fh
            agree[sym]=(r_lo*r_hi)>0
            r=0.5*(r_lo+r_hi)
            res[sym]=r
        ...
        ivx=max(0.01,0.5*(c_lo[0]+c_hi[0]) +0.5*(c_lo[1]+c_hi[1])*x +0.5*(c_lo[2]+c_hi[2])*x*x)
        delta=_bs_delta(S,K,T,ivx)
