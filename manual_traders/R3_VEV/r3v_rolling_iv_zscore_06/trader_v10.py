"""
v10: log-IV rolling z on VEV_5000 — neighborhood grid on v9.

Parent v9 showed large PnL from strict spread (MAX_SPREAD=3). A focused grid
around v9 on Round 3 tapes found a better point: slightly shorter ROLL_WIN,
softer Z_ENTRY, slightly wider exit band, **tighter** spread cap (<=2 ticks),
smaller clip.

Parameters: ROLL_WIN=400, Z_ENTRY=2.6, Z_EXIT=0.07, MAX_SPREAD=2, ORDER_Q=6.

IV: Black–Scholes call implied vol from mids (r=0, T=7/365 reference). Greek
context: same as v8/v9 — z on log(IV); orders are still VEV_5000 touch-only.
