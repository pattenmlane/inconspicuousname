"""
v15: v14 (log-IV z + dynamic entry spread) plus **vega-graded clip size** on VEV_5000.

Tape follow-up (iter 15): re-checked smile filter IV(5000) vs median IV(4500/5100/5200/5300)
on cheap-vol ticks (z < -Z_ENTRY) — **100%** of such ticks already satisfy
IV(5000) <= neighbor median on days 0–2, so a cross-strike gate would be a no-op.
Greek thread instead: scale touch orders by BS call vega (dC/dsigma) vs a vega
budget, clamped to Q_MIN..Q_MAX, so larger clips when vega per shell is lower
and the same risk budget is approximated.

Same microstructure as v14: relaxed spread cap on **entry** only after extract drop;
**exit** still MAX_SPREAD_BASE.

Parameters: v14 spread block; VEGA_TARGET / Q_MIN / Q_MAX from small grid (see results).
T=7/365, r=0. TTE: round3description — ROUND_3 tape day 0/1/2 ~ 8d/7d/6d at start.
"""
