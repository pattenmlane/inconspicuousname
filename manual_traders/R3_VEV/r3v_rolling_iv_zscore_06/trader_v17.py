"""
v17: v15 + tuned extract-drop trigger for relaxed **entry** spread.

v15 used DROP_THRESH=2.0 and S_LOOKBACK=12 for the “recent extract drawdown” gate
that allows MAX_SPREAD_RELAXED on new longs. A focused grid on Round 3 days 0–2
(`--match-trades worse`) found a better point: **DROP_THRESH=1.0**, **S_LOOKBACK=10**
(total ~19.8k vs v15 ~18.9k). Interpretation: smaller cumulative drops still align
with wider VEV_5000 books in cheap-vol windows often enough that the looser gate
adds good entries without blowing up worse-fill exits (exit cap unchanged).

Otherwise identical to v15: log-IV rolling z, vega-graded clip, BS IV/vega (r=0,
T=7/365). TTE: round3description — ROUND_3 tape day 0/1/2 ~ 8d/7d/6d at start.
"""
