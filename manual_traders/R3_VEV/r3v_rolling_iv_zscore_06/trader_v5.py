"""
v5: single-strike (VEV_5000) rolling-IV z-score, long-cheap-vol only.

Iterative update from v3/v4:
- Keep the same thesis (rolling IV z extremes -> mean reversion) and product
  scope (extract + vouchers; this strategy only sends VEV_5000 orders).
- Use past-only IV stats (no look-ahead in z denominator).
- Restrict to extreme negative z entries (z < -6.0) and tight spread <= 4.
- Exit longs when z re-normalizes above -0.2.

TTE note from round3description:
historical tape day index 0/1/2 in ROUND_3 aligns with TTE 8d/7d/6d at day
start (example mapping in spec); this implementation uses fixed T=7/365 as a
reference maturity proxy for IV inversion.
"""
