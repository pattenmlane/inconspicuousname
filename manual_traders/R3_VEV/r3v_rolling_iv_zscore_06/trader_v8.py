"""
v8: v5 with **log-IV rolling z** (variance-stabilizing for strictly positive IV).

Thesis unchanged: VEV_5000 only, long when rolling z of implied vol is very
negative, exit toward neutral, past-only window, touch-only.

Instead of z = (IV - mean_IV) / std_IV, we use log_iv = log(max(IV, IV_FLOOR)) and
z = (log_iv - mean_log) / std_log on the past deque. Thresholds re-tuned (see analysis.json 3x3 grid): Z_ENTRY=2.75, Z_EXIT=0.1
(log-z scale differs from level-IV v5).

Greek/IV: same Black–Scholes implied vol inversion as v5 (r=0, T=7/365).
TTE: round3description — ROUND_3 day 0/1/2 ≈ 8d/7d/6d at start.
"""
...
ROLL_WIN = 480
# Grid on log-IV z (see analysis.json): Z_ENTRY 2.75, Z_EXIT 0.1 maximized PnL vs
# 3.0/0.1 (780) and 2.75/0.12 (963) on Round 3 days 0–2, worse fills.
Z_ENTRY = 2.75
Z_EXIT = 0.1
MAX_SPREAD = 4
ORDER_Q = 10
