"""
Round 3 VEV: cubic spline of implied vol vs log(strike); fair option price from spline IV.
Robustness: optional drop of one strike whose IV deviates most from the median when the
cross-sectional IV range is wide (one bad tick).
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        for sym, K in zip(VEV_SYMS, STRIKES):
            ...
            iv = implied_vol(float(f_ex), float(K), T, float(wm), initial=init)
            if iv is None:
                continue
            iv_prev[sym] = iv
        ...
        for sym, K in zip(VEV_SYMS, STRIKES):
            ...
            net_delta += pos * bs_delta_call(float(f_ex), float(K), T, sig)
        ...
        for sym, K in zip(VEV_SYMS, STRIKES):
            ...
            fair = bs_call(float(f_ex), float(K), T, fair_iv)
            pos = state.position.get(sym, 0)
            lim = LIMITS[sym]
            result[sym].extend(
                self._vev_orders(sym, d, pos, lim, fair, self.TAKE_EDGE, self.MAKE_EDGE, self.MM_SIZE, self.TAKE_SIZE)
            )
