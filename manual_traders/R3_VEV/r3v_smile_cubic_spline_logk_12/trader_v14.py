"""
Round 3 VEV (trader_v14): v11 + extract-shock-aware quote widening using recent |dS| (contiguous
propagation analysis showed strong same-step beta and mild spread widening after shocks).
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        if isinstance(prev_s, (int, float)):
            ...
            if len(ds_hist) > self.SHOCK_WIN:
                ds_hist = ds_hist[-self.SHOCK_WIN:]
        td["_prev_s"] = float(S)
        td["_ds_hist"] = ds_hist

        shock = 0.0
        if ds_hist:
            shock = max(abs(v) for v in ds_hist)
    ...
    def _vev_orders(
        ...
        # When extract has recent large shocks, widen required edge slightly (observed spread widening).
        shock_edge = min(1.5, self.SHOCK_SCALE * max(0.0, shock - 1.0))
        take_e = max(0.5, take_edge - adj + shock_edge)
        make_e = max(0.0, make_edge - 0.4 * adj + 0.5 * shock_edge)
