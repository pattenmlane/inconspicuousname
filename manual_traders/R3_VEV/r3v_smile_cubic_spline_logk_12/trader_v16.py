"""
Round 3 VEV (v16): **trader_v11** + **signed extract momentum** adjustment to BS fair.
From tape analysis (extract_updown_asymmetry.csv), large-step betas differ by direction; a
short rolling sum of d(S) shifts fair for high-delta calls in the direction of the move
(∂C/∂S), small for OTM.
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        ds_hist = td.get("_ds_hist")
        if not isinstance(ds_hist, list):
            ds_hist = []
        if isinstance(prev_s, (int, float)):
            ds_hist.append(float(S) - float(prev_s))
            if len(ds_hist) > self.MOMENTUM_WIN:
                ds_hist = ds_hist[-self.MOMENTUM_WIN :]
        td["_prev_s"] = float(S)
        td["_ds_hist"] = ds_hist
        momentum = float(sum(ds_hist)) if ds_hist else 0.0
        ...
        for sym, K in zip(VEV_SYMS, STRIKES):
            ...
            sig_f = max(fair_iv, 1e-6)
            fair = bs_call(s_opt, float(K), T, sig_f)
            delta = bs_delta_call(s_opt, float(K), T, sig_f)
            # Shift fair along delta direction when extract has been trending (propagation + BS delta).
            fair = fair + self.MOMENTUM_K * momentum * delta
            vega = bs_vega(s_opt, float(K), T, sig_f)
            pos = state.position.get(sym, 0)
            lim = LIMITS[sym]
            result[sym].extend(
                self._vev_orders(
