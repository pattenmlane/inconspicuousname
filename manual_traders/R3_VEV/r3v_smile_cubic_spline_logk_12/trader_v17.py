"""
Round 3 VEV (trader_v17): v11 + propagation residual mean-reversion.
Uses prev residual r=dV-beta*dS per strike from extract propagation fit and shifts fair by -k*r.
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        for sym, K in zip(VEV_SYMS, STRIKES):
            ...
            fair = bs_call(s_opt, float(K), T, sig)
            vega = bs_vega(s_opt, float(K), T, sig)
            pos = state.position.get(sym, 0)
            lim = LIMITS[sym]
            delta = bs_delta_call(s_opt, float(K), T, sig)

            # Propagation residual mean-reversion: r_{t-1}=dV_{t-1}-beta*dS_{t-1}
            beta_ref = min(1.0, max(0.0, abs(delta) * 1.05))
            rv_prev = td.get("_rv_prev", {})
            if not isinstance(rv_prev, dict):
                rv_prev = {}
            resid_prev = float(rv_prev.get(sym, 0.0)) if isinstance(rv_prev.get(sym, 0.0), (int, float)) else 0.0
            fair = fair - self.RESIDUAL_K * resid_prev
            ...
            # update residual for next tick when we can measure dV and dS
            if prev_ds is not None and sym in mids:
                prev_mid_map = td.get("_prev_mid")
                if not isinstance(prev_mid_map, dict):
                    prev_mid_map = {}
                pm = prev_mid_map.get(sym)
                if isinstance(pm, (int, float)):
                    rv_prev[sym] = float(mids[sym]) - float(pm) - beta_ref * float(prev_ds)
                td["_rv_prev"] = rv_prev

        td["_prev_mid"] = {k: float(v) for k, v in mids.items()}
        td["_prev_ds"] = float(S - prev_s) if isinstance(prev_s, (int, float)) else None
