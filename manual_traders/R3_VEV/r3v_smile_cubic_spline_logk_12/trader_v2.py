"""
Round 3 VEV (v2): v1 + **edge / size ladder** vs fair (parameter sweep target): tighter take and
wider MM on penny strikes so spline mispricing can trade through the spread.
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        for sym, K in zip(VEV_SYMS, STRIKES):
            ...
            pos = state.position.get(sym, 0)
            lim = LIMITS[sym]
            te, me, mm_sz, tk_sz = self._edges_for_fair(fair)
            result[sym].extend(self._vev_orders(sym, d, pos, lim, fair, te, me, mm_sz, tk_sz))
    ...
    @staticmethod
    def _edges_for_fair(fair: float) -> tuple[float, float, int, int]:
        """Return (take_edge, make_edge, mm_size, take_size) by option fair in seashells."""
        if fair < 8.0:
            return 1.0, 0.0, 28, 40
        if fair < 40.0:
            return 1.0, 0.5, 22, 32
        if fair < 120.0:
            return 1.5, 1.0, 20, 26
        return 2.0, 1.0, 18, 22
