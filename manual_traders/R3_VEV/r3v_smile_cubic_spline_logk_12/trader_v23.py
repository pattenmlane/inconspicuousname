"""
Round 3 VEV (v23): v11 + joint VEV_5200/VEV_5300 tight BBO spread regime (shared thesis).

From `round3work/vouchers_final_strategy/STRATEGY.txt`: when both legs have
top-of-book spread (ask1 - bid1) <= TH (default 2), short-horizon extract mid
dynamics are more favorable; use as risk-on sizing / extract skew. We apply a
**mild** multiplier to VEV sizes and TAKE_EDGE, and slightly stronger extract
delta-skew when the gate is on. Wide regime uses a small defensive nudge (not
as harsh as an early v23 draft that dropped hydrogel).

Core model unchanged: LOO cubic spline IV(log K), vega-spread edge, delta headroom.
...
# Joint gate (same units as tape / STRATEGY.txt)
GATE_MAX_SPREAD = 2
# Mild: avoid large PnL swings vs v11
TIGHT_MM_MULT = 1.05
TIGHT_TAKE_MULT = 1.05
TIGHT_TAKE_EDGE_MULT = 0.98
WIDE_MM_MULT = 0.95
WIDE_TAKE_MULT = 0.95
WIDE_TAKE_EDGE_MULT = 1.04
TIGHT_EX_SKEW_K = 0.03
WIDE_EX_SKEW_K = 0.02
...
def top_bbo_spread(depth: OrderDepth | None) -> int | None:
...
def joint_tight_5200_5300(state: TradingState) -> bool:
    s52 = top_bbo_spread(state.order_depths.get("VEV_5200"))
    s53 = top_bbo_spread(state.order_depths.get("VEV_5300"))
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        tight = joint_tight_5200_5300(state)
        if tight:
            mm_mult, take_mult, take_edge_mult = TIGHT_MM_MULT, TIGHT_TAKE_MULT, TIGHT_TAKE_EDGE_MULT
            ex_skew_k = TIGHT_EX_SKEW_K
        else:
            mm_mult, take_mult, take_edge_mult = WIDE_MM_MULT, WIDE_TAKE_MULT, WIDE_TAKE_EDGE_MULT
            ex_skew_k = WIDE_EX_SKEW_K

        mm_sz = max(6, int(round(self.MM_SIZE * mm_mult)))
        take_sz = max(8, int(round(self.TAKE_SIZE * take_mult)))
        take_edge_eff = self.TAKE_EDGE * take_edge_mult
        ...
        if len(logks) < 4:
            self._append_extract_hydro(result, ex, state, float(f_ex), ex_skew_k=ex_skew_k)
            td["_fex"] = f_ex
            return result, 0, json.dumps(td)
        ...
        result[EXTRACT].extend(
            self._quote_extract(
                ex, state.position.get(EXTRACT, 0), float(f_ex), delta_skew=net_delta, skew_k=ex_skew_k
            )
        )
    ...
    def _append_extract_hydro(
        self, result: dict, ex: OrderDepth, state: TradingState, f_ex: float, ex_skew_k: float = WIDE_EX_SKEW_K
    ) -> None:
        result[EXTRACT].extend(
            self._quote_extract(
                ex, state.position.get(EXTRACT, 0), f_ex, delta_skew=0.0, skew_k=ex_skew_k
            )
        )
        hd = state.order_depths.get(HYDRO)
        if hd is not None and hd.buy_orders and hd.sell_orders:
    ...
    def _quote_extract(
        self, depth: OrderDepth, pos: int, fair: float, delta_skew: float = 0.0, skew_k: float = 0.02
    ) -> list[Order]:
        orders: list[Order] = []
        if not depth.buy_orders or not depth.sell_orders:
            return orders
        bb, ba = max(depth.buy_orders), min(depth.sell_orders)
        skew = int(round(max(-3.0, min(3.0, skew_k * delta_skew))))
