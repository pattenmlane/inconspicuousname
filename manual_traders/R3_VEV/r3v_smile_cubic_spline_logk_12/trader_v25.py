"""
Round 3 — vouchers_final_strategy (v25): joint 5200/5300 gate + per-contract spread edges.

Same thesis as v24 but fix under-trading: take/make edges scale with **this** voucher's
top-of-book spread (inclineGod: spreads are the object). Base edge is set by **joint**
tight vs wide regime; add a small penalty when local spread is wide even if the global
5100/5200 pair is tight (execution cost).

No LOO/spline/hydrogel. VELVETFRUIT_EXTRACT + VEVs only.
"""
...
GATE_MAX_SPREAD = 2
EX_SMOOTH = 0.12
# Per-tick spread penalty above 2 ticks (same TH as gate)
LOCAL_SPR_COEF = 0.12
...
class Trader:
    # Base take edge (before local spread penalty)
    TIGHT_BASE_TAKE = 0.85
    TIGHT_BASE_MAKE = 0.55
    TIGHT_MM = 22
    TIGHT_TAKE = 26
    TIGHT_EX_EDGE = 2
    TIGHT_EX_BIAS = 1
    ...
    def run(self, state: TradingState):
        ...
        result[EXTRACT].extend(self._quote_extract(ex, ex_pos, fi, ex_e, max_lot=25))
