"""
Round 3 VEV (v3): spline IV(logK) fair with robust outlier drop + edge-gated multi-level
option taking/clearing. Keeps same thesis, but allocates risk where tape edge is persistent.
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        for sym, K in zip(VEV_SYMS, STRIKES):
            ...
            pos = state.position.get(sym, 0)
            lim = LIMITS[sym]
            result[sym].extend(self._vev_orders(sym, d, pos, lim, fair, self.TAKE_EDGE, self.MAKE_EDGE, self.MM_SIZE, self.TAKE_SIZE))
            result[sym].extend(self._vev_take_levels(sym, d, pos, lim, fair))
    ...
    def _vev_take_levels(self, sym: str, depth: OrderDepth, pos: int, lim: int, fair: float) -> list[Order]:
        """Additional aggressive taking and unwind around fair (same spline thesis)."""
        orders: list[Order] = []
        ...
        # Strongest edge is concentrated in low strike in our analysis; scale aggressiveness there.
        max_take = self.TAKE_SIZE + (25 if sym == "VEV_4000" else 0)
        max_take = min(max_take, 70)

        bought = 0
        for px, vol in asks:
            avail = -vol
            edge = fair - px
            if edge < self.TAKE_EDGE + 0.25:
                break
            q = min(avail, buy_cap - bought, max_take - bought)
            if q <= 0:
                break
            orders.append(Order(sym, int(px), int(q)))
            bought += q
        ...
        for px, vol in bids:
            ...
            if edge < self.TAKE_EDGE + 0.25:
                break
            q = min(vol, sell_cap - sold, max_take - sold)
            if q <= 0:
                break
            orders.append(Order(sym, int(px), -int(q)))
            sold += q

        # Inventory clear near fair to realize pnl and recycle limits.
        bb = max(depth.buy_orders)
        ba = min(depth.sell_orders)
        if pos > 0 and (bb - fair) > -self.CLEAR_EDGE:
            q = min(pos, 30)
            if q > 0:
                orders.append(Order(sym, int(bb), -int(q)))
        elif pos < 0 and (fair - ba) > -self.CLEAR_EDGE:
            q = min(-pos, 30)
            if q > 0:
