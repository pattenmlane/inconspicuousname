"""
Round 3 VEV (v9): **trader_v5** with **leave-one-out (LOO) cubic spline IV** for each strike
when valuing/quoting that strike. If one voucher mid is a bad tick, the full smile still fits
it; LOO at K omits that point so fair IV is not pulled by a single outlier. When fewer than
five IV nodes remain, falls back to in-sample spline (same as v5).
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        for sym, K in zip(VEV_SYMS, STRIKES):
            ...
            result[sym].extend(
                self._vev_orders(
                    sym, d, pos, lim, fair, self.TAKE_EDGE, self.MAKE_EDGE, self.MM_SIZE, self.TAKE_SIZE, vega
                )
            )
