...
Variation: fit quadratic IV smile on core strikes only (VEV_5000..VEV_5400) and clamp km evaluation outside core domain for wing quoting.
Parent: trader_v9 (variation: core-strike-only smile fit).
"""
...
ATM_TIGHT_KM_D2 = 0.032
TOUCH_NUDGE_KM_D2 = 0.028
SIZE_VEV = 26
SIZE_EXTRACT = 14
SIZE_HYDRO = 10
SKEW_PER_UNIT = 0.045
...
class Trader:
    def run(self, state: TradingState):
        ...
        for p in VEV_PRODUCTS:
            ...
            if iv is None:
                continue
            if p not in CORE_STRIKES:
                # variation: exclude deep wings from smile calibration
                continue
            km = math.log(K / mid_u)
            spr = b[1] - b[0]
        ...
        def fair_iv_at_km(km: float) -> float:
            if coeffs is None:
                return 0.22
            c, b, a = coeffs
            # distinct variant: clamp evaluation km to core domain to avoid unstable wing extrapolation
            km_eval = min(max(km, CORE_KM_MIN), CORE_KM_MAX)
            return max(c + b * km_eval + a * km_eval * km_eval, 0.03)
