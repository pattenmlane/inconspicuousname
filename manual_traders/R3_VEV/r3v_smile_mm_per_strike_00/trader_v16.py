"""
Round 3 — **trader_v6** + **day-2-only** quote grid (shortest TTE: historical csv_day 2).

Tape Greek scan (see `analysis_outputs/iter5_vega_theta_by_day_atm6.json`): on day 2,
near-ATM strikes have higher |BS call theta| than days 0–1, so we slightly **tighten**
touch and ATM band on that day only to try to improve the small VEV PnL without
touching days 0–1.

If this tie-breaks with v6 on aggregate, prefer **v6** (simpler).
Parent: trader_v9; adds shock-aware per-strike spread widening on days 0-1.
"""
...
# analysis-driven spread shock controls (iter8_underlying_propagation_spread_shocks.json)
UNDERLYING_SHOCK_ABS_MOVE = 7
SHOCK_WIDEN_BY_STRIKE = {"VEV_5000": 0.35, "VEV_5100": 0.22, "VEV_5200": 0.18, "VEV_5300": 0.12, "VEV_5400": 0.20}
...
class Trader:
    def run(self, state: TradingState):
        ...
        if bu is None:
            return {}, 0, json.dumps(store, separators=(",", ":"))
        _, _, mid_u = bu
        prev_mid_u = store.get("prev_mid_u")
        ...
        shock_dir = 0
        if prev_mid_u is not None and csv_day < 2:
            try:
                du = float(mid_u) - float(prev_mid_u)
                if abs(du) >= UNDERLYING_SHOCK_ABS_MOVE:
                    shock_dir = 1 if du > 0 else -1
            except (TypeError, ValueError):
                shock_dir = 0
        for p in vev_products_ok:
            ...
            if shock_dir != 0:
                # asymmetry from analysis: up-moves stress high-delta calls more
                if shock_dir > 0 and p in ("VEV_5000", "VEV_5100", "VEV_5200"):
                    half += SHOCK_WIDEN_BY_STRIKE.get(p, 0.0)
                elif shock_dir < 0 and p in ("VEV_5300", "VEV_5400"):
                    half += 0.6 * SHOCK_WIDEN_BY_STRIKE.get(p, 0.0)
