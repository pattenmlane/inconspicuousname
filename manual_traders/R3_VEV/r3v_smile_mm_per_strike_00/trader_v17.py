"""
Round 3 — **trader_v9** + **R2-based VEV clip** from underlying–voucher propagation (iter8/iter9).

`iter8` contemporaneous regression R^2 (dV vs dS) is highest on 5000–5200 and lower on
wings (5500+). We scale per-strike quote size linearly in mean R^2 across days 0–2
so well-tracked strikes
get up to ~10% more size and low-R2 wings ~14% less — risk control without changing
spreads (v15/v16 shock widening tied v9).

Micro-grid: (SIZE_MULT_R2_LO, HI) = (0.86, 1.10) vs (0.90, 1.06); both tie v9 at
11664; keep gentler (0.90, 1.06) as default.
...
# Day 2 only (grid: see results.json iteration 9 note; local sweep beat v6)
ATM_TIGHT_KM_D2 = 0.034
TOUCH_NUDGE_KM_D2 = 0.030
SIZE_VEV = 26
# Mean r2_dV_dS (iter8) across csv_day 0,1,2; used for size multiplier bounds.
R2_SIZE_REF_LO = 0.12
R2_SIZE_REF_HI = 0.59
# Default after 2-point grid (also tested 0.86/1.10); see results iteration 17.
SIZE_MULT_R2_LO = 0.90
SIZE_MULT_R2_HI = 1.06
# Populated from iter8 means (iter9_r2_tiering_from_propagation.json).
R2_MEAN_BY_STRIKE: dict[str, float] = {
    "VEV_4000": 0.3538896436373708,
    "VEV_4500": 0.3575060114832928,
    "VEV_5000": 0.5679952756348189,
    "VEV_5100": 0.5858118373031798,
    "VEV_5200": 0.5145922135569578,
    "VEV_5300": 0.3864866972906098,
    "VEV_5400": 0.2918724944873567,
    "VEV_5500": 0.12145731767477018,
    "VEV_6000": 0.0,
    "VEV_6500": 0.0,
}
SIZE_EXTRACT = 14
...
def _vev_size_mult_from_r2(p: str) -> float:
    """Map mean iter8 r2 to a clip multiplier in [SIZE_MULT_R2_LO, SIZE_MULT_R2_HI]."""
    r2 = R2_MEAN_BY_STRIKE.get(p, 0.0)
    lo, hi = R2_SIZE_REF_LO, R2_SIZE_REF_HI
    if hi <= lo:
        return 1.0
    t = (r2 - lo) / (hi - lo)
    t = min(max(t, 0.0), 1.0)
    return SIZE_MULT_R2_LO + t * (SIZE_MULT_R2_HI - SIZE_MULT_R2_LO)
...
class Trader:
    def run(self, state: TradingState):
        ...
        for p in vev_products_ok:
            ...
            sz = max(1, int(round(SIZE_VEV * _vev_size_mult_from_r2(p))))
            qb = min(sz, lim - pos)
            qs = min(sz, lim + pos)
            ol: list[Order] = []
            if qb > 0 and bid_p > 0:
        ...
        # v6: on historical csv day 2 (TTE 6d at open), do not trade extract or hydrogel — focus on voucher smile
        if csv_day < 2:
