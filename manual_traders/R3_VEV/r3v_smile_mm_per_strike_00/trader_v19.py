"""
Round 3 — **trader_v9** + **vega-scaled inventory skew**.

Tape IV-inversion (subsampled): BS call vega at smile IV is ~10–15 on deep wings,
~110–290 near 5000–5400 depending on day. Flat `SKEW_PER_UNIT` therefore over-
penalizes low-vega wings and under-weights high-vega ATM gamma/vega risk. We scale
skew by clip(vega / VEGA_SKEW_REF, MIN, MAX).

Grid: VEGA_SKEW_REF = 200 vs 260 (see results iteration 19).
...
SIZE_HYDRO = 10
SKEW_PER_UNIT = 0.045
# Normalize vega to ~1 at near-ATM (tape median ~200–270); scale skew by vega/REF.
VEGA_SKEW_REF = 200.0
VEGA_SKEW_MULT_MIN = 0.35
VEGA_SKEW_MULT_MAX = 2.2
...
def _vega_skew_mult(vega: float) -> float:
    if VEGA_SKEW_REF <= 0:
        return 1.0
    t = vega / VEGA_SKEW_REF
    return min(max(t, VEGA_SKEW_MULT_MIN), VEGA_SKEW_MULT_MAX)
...
class Trader:
    def run(self, state: TradingState):
        ...
        for p in vev_products_ok:
            ...
            veg = bs_call_vega(mid_u, K, T, sig, RISK_FREE)
            pos = int(positions.get(p, 0))
            lim = LIMITS[p]
            vm = _vega_skew_mult(veg)
            skew = SKEW_PER_UNIT * vm * (pos / max(lim, 1))
