"""
Round 3 — **trader_v9** + **joint 5200/5300 tight gate** (shared strategy thesis).

From `round3work/vouchers_final_strategy/STRATEGY.txt`: when **VEV_5200** and **VEV_5300**
both have top-of-book spread ≤ **TIGHT_SPREAD_TH** (same tick units as tape), short-horizon
extract mid dynamics are more favorable on average — use as **risk-on** sizing for VEV +
extract (Sonic / inclineGod book-state view).

Implementation: if `spread_5200 <= TH` and `spread_5300 <= TH`, multiply VEV clip and extract
clip by `MULT_TIGHT`; else `MULT_WIDE`. Slightly tighten extract **half** when gate is on.

**No HYDROGEL_PACK** (per current PnL objective for this line of work).

Mild size/half lean: VEV 1.05/0.95, extract 1.05/0.95, half 0.99/1.01 (aggressive 1.12/0.88
equal total to mild in 4-day worse run; see results iteration 20).
...
GATE_B = "VEV_5300"
TIGHT_SPREAD_TH = 2
# Joint-gate sizing (v9 baseline = 1.0 for both)
SIZE_MULT_TIGHT = 1.05
SIZE_MULT_WIDE = 0.95
EXTRACT_SIZE_MULT_TIGHT = 1.05
EXTRACT_SIZE_MULT_WIDE = 0.95
EXTRACT_HALF_TIGHT_MULT = 0.99
EXTRACT_HALF_WIDE_MULT = 1.01
VEV_PRODUCTS = [
    "VEV_4000",
    "VEV_4500",
    "VEV_5000",
...
def top_of_book_spread_ticks(depth: OrderDepth | None) -> int | None:
    b = book_mid(depth)
    if b is None:
        return None
    bb, ba, _ = b
...
def joint_tight_gate(depths: dict[str, Any], th: int = TIGHT_SPREAD_TH) -> bool:
    da = depths.get(GATE_A)
    db = depths.get(GATE_B)
    if da is None or db is None:
        return False
    sa = top_of_book_spread_ticks(da)
    sb = top_of_book_spread_ticks(db)
    if sa is None or sb is None:
        return False
    return sa <= th and sb <= th
...
class Trader:
    def run(self, state: TradingState):
        ...
        tight = joint_tight_gate(depths, TIGHT_SPREAD_TH)
        store["tight_5200_5300"] = tight
        m_vev = SIZE_MULT_TIGHT if tight else SIZE_MULT_WIDE
        m_ex_sz = EXTRACT_SIZE_MULT_TIGHT if tight else EXTRACT_SIZE_MULT_WIDE
        m_ex_half = EXTRACT_HALF_TIGHT_MULT if tight else EXTRACT_HALF_WIDE_MULT
        ...
        for p in vev_products_ok:
            ...
            half = (BASE_HALF_SPREAD + wing_w) * atm_tight
            ...
            sz = max(1, int(round(SIZE_VEV * m_vev)))
            qb = min(sz, lim - pos)
            qs = min(sz, lim + pos)
            ol: list[Order] = []
            if qb > 0 and bid_p > 0:
        ...
        if csv_day < 2:
            ...
            if bu2 is not None:
                ...
                half_x = EXTRACT_HALF * m_ex_half
                bid_x = int(round(fair_x - half_x))
                ask_x = int(round(fair_x + half_x))
                bid_x = min(bid_x, int(bu2[1]) - 1)
                ask_x = max(ask_x, int(bu2[0]) + 1)
                if bid_x < ask_x:
                    ex_sz = max(1, int(round(SIZE_EXTRACT * m_ex_sz)))
                    qb = min(ex_sz, lim_u - pos_u)
                    qs = min(ex_sz, lim_u + pos_u)
                    olu: list[Order] = []
                    if qb > 0:
