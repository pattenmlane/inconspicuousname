"""
Round 3 — **vouchers_final_strategy** (Sonic / inclineGod / STRATEGY.txt).

Same as v21: joint BBO **s5200 ≤ 2 and s5300 ≤ 2** defines **tight** vs **wide** regime.
**Wide:** still quote (unlike v21 full flat) but **~30%** clip and **wider** half-spreads to
match “execution dominates in wide book” without turning off the entire book.

Tight: mid-touch on VEVs + extract (no IV), sizes SIZE_VEV_T / SIZE_EX_T.
Wide: scale sizes down, add extra half to both legs.

**No HYDROGEL_PACK.**

Parent: `trader_v21` concept; v22 is a sweep vs hard-off wide regime.
"""
...
WIDE_SIZE_FRAC = 0.3
WIDE_VEV_EXTRA_HALF = 0.9
WIDE_EXTRACT_EXTRA_HALF = 0.4
...
class Trader:
    def run(self, state: TradingState):
        ...
        szf = 1.0 if tight else WIDE_SIZE_FRAC
        vev_half_ex = 0.0 if tight else WIDE_VEV_EXTRA_HALF
        ex_half_ex = 0.0 if tight else WIDE_EXTRACT_EXTRA_HALF
        size_vev = max(1, int(round(SIZE_VEV_T * szf)))
        size_ex = max(1, int(round(SIZE_EXTRACT_T * szf)))
        ...
        for p in VEV_PRODUCTS:
            ...
            spr = b[1] - b[0]
            fair = mid - skew * spr
            half = BASE_VEV_HALF + WING_KM_SQ * (km**2) + vev_half_ex
