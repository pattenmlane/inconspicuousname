"""
Round 3 — **vouchers_final_strategy**: joint **VEV_5200 + VEV_5300** tight gate; **tight
surface** = quote **those two legs** + **VELVETFRUIT_EXTRACT** (Sonic: *hedge into* tight
voucher books, not the full 10-strike book).

- **Tight** (s5200≤2 and s5300≤2, BBO ask−bid): mid-touch on **VEV_5200**, **VEV_5300**,
  and **VELVETFRUIT_EXTRACT**. Extract uses the same `EXTRACT_HALF=2.4` and long-lean that
  worked in **v23**; VEVs use the same **BASE_VEV_HALF + WING_KM_SQ*km^2** wing shape as
  v21/v22 but **only** on the two gate strikes (no 4000–6500 book risk).
- **Wide:** no orders (v23-style off-gate).

inclineGod: still track per-contract state via spreads (gate) + per-strike mids here.

**No HYDROGEL_PACK**; no other VEV strikes.
"""
...
# v23-locked extract (grid 2.2/2.4/2.6; 2.4 best on `worse`)
EXTRACT_HALF = 2.4
SIZE_EXTRACT = 16
SKEW_PER_UNIT = 0.04
LONG_LEAN_TICKS = 0.15

# Gate-leg VEV MM (sweep: SIZE_VEV_ATM 16/20/24 in results)
BASE_VEV_HALF = 1.35
WING_KM_SQ = 280.0
SIZE_VEV_ATM = 20
...
class Trader:
    def run(self, state: TradingState):
        ...
        if bid_x < ask_x:
            ou: list[Order] = []
            qb = min(SIZE_EXTRACT, LIMITS[UNDERLYING] - pos_u)
            qs = min(SIZE_EXTRACT, LIMITS[UNDERLYING] + pos_u)
            if qb > 0:
