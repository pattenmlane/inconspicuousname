"""
Round 3 — same as trader_v0 with one tuned parameter from a small grid sweep:
**EXTRACT_HALF = 2.45** (vs 2.0 in v0) to slightly widen underlying quotes and
reduce adverse selection on VELVETFRUIT_EXTRACT in the worse-fill backtester.

Timing (authoritative mapping for historical CSV days 0–2):
  round3work/round3description.txt: 7-day option, each round = 1 day; example maps
  historical day 1→TTE 8d, day 2→7d, day 3→6d (simulation vs CSV index).
  round3work/plotting/original_method/combined_analysis/plot_iv_smile_round3.py:
  CSV day 0→8d at open, 1→7d, 2→6d; intraday DTE winds ~1 day over the session;
  T_years = dte_effective/365, r = 0.

Backtester does not expose CSV day on TradingState; we recover `data.day_num`
from the TestRunner call stack when present (local backtests only).
"""
...
# --- strategy params (v0 baseline) ---
RISK_FREE = 0.0
BASE_HALF_SPREAD = 1.2
WING_KM_SQ_SCALE = 360.0  # slightly tighter than v3, still wider in wings
SIZE_VEV = 26
SIZE_EXTRACT = 14
SIZE_HYDRO = 10
SKEW_PER_UNIT = 0.045  # fair shift per fraction of pos limit (voucher)
EXTRACT_HEDGE_BETA = 0.35  # shift extract fair against net call delta exposure
EXTRACT_HALF = 2.45
HYDRO_HALF = 2.0
...
def polyfit_quadratic(
    ...
    if s_w <= 0:
        return None
    # normal equations for weighted quadratic regression
...
class Trader:
    def run(self, state: TradingState):
        ...
        for p in vev_products_ok:
            ...
            # tighten around high-vega strikes near ATM (analysis: 5100-5400)
            atm_tight = 0.75 if abs(km) < 0.04 else 1.0
            half = (BASE_HALF_SPREAD + wing_w) * atm_tight
            bid_p = int(round(fair_adj - half))
            ask_p = int(round(fair_adj + half))
            # near ATM, quote at touch to improve fill rate for the smile/MM thesis
            if abs(km) < 0.035:
        ...
        # Extract: MM around mid with hedge bias
