"""
Round 3 — **trader_v4** smile MM with a day-conditional cash product cut.

Decomposition on v4 showed **Round 3 csv_day 2** PnL is dominated by **VELVETFRUIT_EXTRACT**
and **HYDROGEL_PACK** (−6.5k each in the backtester); voucher PnL on that day is near zero.
Historical tape day 2 corresponds to **TTE 6d at open** (see round3work/round3description.txt
example + our `dte_from_csv_day` mapping). On that slice we **skip extract and hydrogel
quotes** and run **voucher smile MM only** — still per-strike BS fair from the fitted smile,
inventory skew, ATM tightness (same as v4).

Parent: trader_v4 (best aggregate PnL before this change).
"""
