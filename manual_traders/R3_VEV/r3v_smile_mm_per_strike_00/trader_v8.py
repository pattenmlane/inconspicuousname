"""
Round 3 — **trader_v7** (EMA-smoothed smile) with a **day-2 hybrid**: on `csv_day == 2`
(historical tape day with TTE 6d at open, voucher-only cash) use the **unsmoothed**
quadratic fit each step so voucher fairs stay responsive; on days 0–1 keep the EMA on
`smile_abc_smooth` as in v7.

Rationale: EMA on day 2 alone erased a small +10 voucher PnL vs v6; raw fit restores it
while still smoothing the busy days 0–1.
...
class Trader:
    def run(self, state: TradingState):
        ...
        if isinstance(prev_smooth, list) and len(prev_smooth) == 3:
            prev_t: tuple[float, float, float] | None = (
                float(prev_smooth[0]), float(prev_smooth[1]), float(prev_smooth[2])
            )
        else:
            prev_t = None
        ...
        if coeffs_raw is not None:
            if csv_day < 2:
                ...
                # Shortest-TTE / voucher-only day: use live cross-section (no EMA), avoid stale state
                coeffs_t = coeffs_raw
        elif prev_t is not None and csv_day < 2:
            coeffs_t = prev_t
        else:
            coeffs_t = None
