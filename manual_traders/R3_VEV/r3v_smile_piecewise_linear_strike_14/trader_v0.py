"""
Round 3: piecewise-linear IV in strike (knots 5000/5200/5400), linear between, flat outside.
Fair = BS call with sigma_pwl(K). IV from mid via bisection (r=0). Delta/vega for sizing context.

Timing: round3work/round3description.txt — historical CSV day d has DTE 8-d at open; intraday
effective DTE = dte_open - (timestamp//100)/10000 (see plot_iv_smile_round3 / combined_analysis).
Backtester does not pass day_num in TradingState; csv_day is inferred by minimizing cross-strike
IV dispersion vs T among {0,1,2} (same helper as offline calibration).
"""
...
# Frankfurt-style scalping thresholds (tuned down slightly vs single-strike reference)
THR_OPEN = 0.45
THR_CLOSE = 0.05
...
def _median(xs: list[float]) -> float | None:
    ys = [x for x in xs if x is not None and __import__("math").isfinite(x)]
    if not ys:
        return None
    ys.sort()
    return ys[len(ys) // 2]
...
def _knot_ivs_from_surface(strike_iv: dict[int, float | None]) -> tuple[float, float, float] | None:
    """Few knots: local medians around 5000 / 5200 / 5400 then clip."""
    import math
...
class Trader:
    def run(self, state: TradingState):
        ...
        if "csv_day_est" not in ema and len(strike_mids) >= 6:
            ema["csv_day_est"] = float(infer_csv_day_from_smile(S, strike_mids, ts, STRIKES))
