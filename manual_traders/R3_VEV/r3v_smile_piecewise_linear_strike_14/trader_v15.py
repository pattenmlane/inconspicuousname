"""
v15: v12 + delta hedge with VELVETFRUIT_EXTRACT at hedge_factor=0.35.
"""
...
class Trader:
    def run(self, state: TradingState):
        ...
        for k, sym in zip(STRIKES, VOUCHERS):
            ...
            delta = bs_call_delta(S_fair, float(k), T, sig)
            pos = int((getattr(state, "position", None) or {}).get(sym, 0))
            target_delta += float(pos) * float(delta)
        ...
        # Hedge net option delta exposure using underlying extract.
        u_pos = int((getattr(state, "position", None) or {}).get(U, 0))
        desired_u = int(round(-HEDGE_FACTOR * target_delta))
        desired_u = max(-LIMIT_U, min(LIMIT_U, desired_u))
        delta_u = desired_u - u_pos
        if delta_u != 0 and U in depths:
            ubw0, uaw0, ubb2, uba2, _uwm = book_walls(depths[U])
            _ubw, _uaw, _uwm2, ubb3, uba3 = synthetic_walls(ubw0, uaw0, ubb2, uba2)
            if ubb3 is not None and uba3 is not None:
                q = min(abs(delta_u), 18)
                if q > 0:
                    if delta_u > 0:
                        orders[U] = [Order(U, int(uba3), int(q))]
                    else:
                        orders[U] = [Order(U, int(ubb3), -int(q))]
