"""
v17: v12 + re-infer csv_day every 5_000 time units (~50 * 100 tick steps) so T matches moving
DTE/IV surface (still PWL smile + same gates/caps).
"""
...
WING_CAP = 40
STRIKE_5000_CAP = 1
# Backtester timestamps step by 100; 50 bar steps => 50 * 100
CSV_DAY_REFRESH = 50 * 100
...
class Trader:
    def run(self, state: TradingState):
        ...
        if len(strike_mids) >= 6:
            last_ts = int(ema.get("csv_day_refresh_ts", -10**9))
            if "csv_day_est" not in ema or (ts - last_ts) >= CSV_DAY_REFRESH:
                ema["csv_day_est"] = float(infer_csv_day_from_smile(S_fair, strike_mids, ts, STRIKES))
                ema["csv_day_refresh_ts"] = float(ts)
