"""
v20: v18 + trade only inner band 5000-5300 (same PWL smile from full book; focus where smile fit is tight).
"""
...
TRADE_STRIKES = frozenset({5000, 5100, 5200, 5300})
...
class Trader:
    def run(self, state: TradingState):
        ...
        for k, sym in zip(STRIKES, VOUCHERS):
            if k in SKIP_TRADE or k not in TRADE_STRIKES:
                continue
            d = depths.get(sym)
            if d is None:
                continue
