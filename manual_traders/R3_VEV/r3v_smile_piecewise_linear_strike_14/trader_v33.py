"""
v33 (family13): rolling z-score realized-vol regime classifier for execution params.
"""
...
Z_LOW = -0.4
Z_HIGH = 0.9
REGIME_EXEC = {"low":{"thr_open":0.46,"iv_thr":0.56},"mid":{"thr_open":0.50,"iv_thr":0.60},"high":{"thr_open":0.54,"iv_thr":0.66}}
...
def _z_regime(z: float | None) -> str:
    if z is None:
        return "mid"
    if z < Z_LOW:
        return "low"
...
class Trader:
    def run(self, state: TradingState):
        ...
        # realized-vol regime via rolling z-score on extract rv
        ...
        for k, sym in zip(STRIKES, VOUCHERS):
            ...
            old_open = globals().get("THR_OPEN", 0.5)
            old_ivt = globals().get("IV_SCALPING_THR", 0.6)
            globals()["THR_OPEN"] = float(reg_cfg["thr_open"])
            globals()["IV_SCALPING_THR"] = float(reg_cfg["iv_thr"])
