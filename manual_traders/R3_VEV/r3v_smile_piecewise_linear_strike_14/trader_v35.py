"""
v35: fixed-threshold regime with stronger execution coupling (regime-specific spread floor and 5000 cap).
"""
...
RV_Q50 = 6.5e-4
RV_Q80 = 9.5e-4
REGIME_EXEC = {"low":{"thr_open":0.44,"iv_thr":0.54,"spread":4,"cap5000":1},"mid":{"thr_open":0.50,"iv_thr":0.60,"spread":5,"cap5000":1},"high":{"thr_open":0.56,"iv_thr":0.68,"spread":7,"cap5000":0}}
...
def _fixed_regime(rv: float | None) -> str:
    if rv is None:
        return "mid"
    if rv < RV_Q50:
        return "low"
...
class Trader:
    def run(self, state: TradingState):
        ...
        # realized-vol regime on extract log returns
        ...
        for k, sym in zip(STRIKES, VOUCHERS):
            ...
            if k == 5000:
                reg_cap5000 = int(reg_cfg.get("cap5000", STRIKE_5000_CAP))
                cap = min(cap, reg_cap5000)
            ol: list[Order] = []
