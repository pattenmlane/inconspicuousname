"""
v36 (family13-hys): fixed-threshold realized-vol classifier with hysteresis + min-hold regime persistence.
"""
...
def _fixed_regime_hys(ema: dict[str, float], rv: float | None) -> str:
    ...
    if reg == "mid":
        if rv <= FIX_IN_LOW:
            ...
            nxt = "high"
    elif reg == "low" and rv >= FIX_OUT_LOW:
        nxt = "mid"
    elif reg == "high" and rv <= FIX_OUT_HIGH:
        nxt = "mid"
    if nxt != reg:
...
class Trader:
    def run(self, state: TradingState):
        ...
        # realized vol for fixed hysteresis regime
        ...
        regime = _fixed_regime_hys(ema, rv)
