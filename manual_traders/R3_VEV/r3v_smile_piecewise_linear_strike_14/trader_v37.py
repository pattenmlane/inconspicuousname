"""
v37 (family13-hys): rolling z-score realized-vol classifier with hysteresis + min-hold persistence.
"""
...
Z_OUT_LOW = -0.2
Z_OUT_HIGH = 0.55
REGIME_EXEC = {"low":{"thr_open":0.46,"iv_thr":0.56,"spread":4},"mid":{"thr_open":0.50,"iv_thr":0.60,"spread":5},"high":{"thr_open":0.55,"iv_thr":0.67,"spread":6}}
...
def _z_regime_hys(ema: dict[str, float], z: float | None) -> str:
    reg = str(ema.get("regime", "mid"))
    hold = int(ema.get("reg_hold", 0))
    if z is None:
        ema["regime"] = reg
        return reg
    if hold > 0:
        ema["reg_hold"] = float(hold - 1)
        return reg
    nxt = reg
    if reg == "mid":
        if z <= Z_IN_LOW:
            ...
            nxt = "high"
    elif reg == "low" and z >= Z_OUT_LOW:
        nxt = "mid"
    elif reg == "high" and z <= Z_OUT_HIGH:
        nxt = "mid"
    if nxt != reg:
        ema["reg_hold"] = float(REGIME_HOLD)
    ema["regime"] = nxt
    return nxt
...
class Trader:
    def run(self, state: TradingState):
        ...
        # realized vol and z-score for hysteresis regime
        prev_s = float(ema.get("prev_s", S_fair))
        rv = None
        if prev_s > 0 and S_fair > 0:
            lr = math.log(S_fair / prev_s)
            rv_hist = ema.get("rv_hist", [])
            if not isinstance(rv_hist, list):
                rv_hist = []
            rv_hist.append(float(lr))
            if len(rv_hist) > RV_WINDOW:
                rv_hist = rv_hist[-RV_WINDOW:]
            ema["rv_hist"] = rv_hist
            if len(rv_hist) >= RV_WINDOW:
                mu = sum(rv_hist) / len(rv_hist)
                rv = (sum((x - mu) ** 2 for x in rv_hist) / len(rv_hist)) ** 0.5
        ema["prev_s"] = float(S_fair)
        rv_series = ema.get("rv_series", [])
        if not isinstance(rv_series, list):
            rv_series = []
        if rv is not None:
            rv_series.append(float(rv))
        if len(rv_series) > Z_WINDOW:
            rv_series = rv_series[-Z_WINDOW:]
        ema["rv_series"] = rv_series
        z = None
        if len(rv_series) >= max(50, Z_WINDOW // 2) and rv is not None:
            m = sum(rv_series) / len(rv_series)
            v = sum((x - m) ** 2 for x in rv_series) / len(rv_series)
            sd = v ** 0.5
            if sd > 1e-12:
                z = (rv - m) / sd
        regime = _z_regime_hys(ema, z)
        reg_cfg = REGIME_EXEC.get(regime, REGIME_EXEC["mid"])
        ...
        for k, sym in zip(STRIKES, VOUCHERS):
            ...
            if wm2 is None or bb2 is None or ba2 is None:
                continue
            reg_spread = int(reg_cfg.get("spread", MIN_SPREAD))
            if int(ba2) - int(bb2) < reg_spread:
                continue
            ...
            old_open = globals().get("THR_OPEN", THR_OPEN)
            old_ivt = globals().get("IV_SCALPING_THR", IV_SCALPING_THR)
            globals()["THR_OPEN"] = float(reg_cfg.get("thr_open", THR_OPEN))
            globals()["IV_SCALPING_THR"] = float(reg_cfg.get("iv_thr", IV_SCALPING_THR))
            bids, asks = _scalp_orders(
                float(wm2), int(bb2), int(ba2), pos, LIMIT_VEV, cur_diff, mean_diff, switch_mean, vega
            )
            globals()["THR_OPEN"] = old_open
            globals()["IV_SCALPING_THR"] = old_ivt
