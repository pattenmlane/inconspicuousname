"""
v29: v23 + slightly looser IV scalping (IV_SCALPING_THR=0.58, THR_OPEN=0.48) — grid on v23.
"""
...
STRIKE_5000_CAP = 1
MIN_SPREAD = 5
STRIKE_BETA = {5000:0.66, 5100:0.57, 5200:0.43, 5300:0.27, 5400:0.14, 5500:0.06}
...
class Trader:
    def run(self, state: TradingState):
        ...
        for k, sym in zip(STRIKES, VOUCHERS):
            ...
            beta_w = float(STRIKE_BETA.get(k, 0.25))
            # Higher beta strikes react faster to underlying; require slightly less theo edge there.
            edge_scale = max(0.75, min(1.15, 1.0 - 0.25 * (beta_w - 0.4)))
            old_open = globals().get("THR_OPEN", THR_OPEN)
            old_close = globals().get("THR_CLOSE", THR_CLOSE)
            globals()["THR_OPEN"] = float(old_open * edge_scale)
            globals()["THR_CLOSE"] = float(old_close * edge_scale)
