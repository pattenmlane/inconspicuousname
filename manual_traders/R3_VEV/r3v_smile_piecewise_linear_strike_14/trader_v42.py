"""
v41 + short-horizon extract momentum for asymmetric THR_OPEN (Greek-consistent).
Tape: beta_up ~ beta_down for inner strikes (see underlying_up_down_asymmetry_rollup.json)
so we do not split by up/down price response; instead we use prior IV–extract comovement:
spot up / IV often down -> slightly easier to hit bid edge; spot down -> easier offer edge.
"""
...
WARMUP_STEPS = 15
S_EMA_WINDOW = 140
# EMA of (S_t - S_{t-1}) / max(|S_{t-1}|, 1); scales THR_OPEN for bid vs ask separately
MOMENTUM_EMA_WINDOW = 25
THR_OPEN_MOM_COEF = 0.12
WING_CAP = 40
STRIKE_5000_CAP = 1
MIN_SPREAD = 5
...
def _scalp_orders(
    ...
    if switch_mean >= IV_SCALPING_THR:
        if cur_diff - wall_mid + best_bid - mean_diff >= (thr_open_sell + low_adj) and max_sell > 0:
            asks.append((best_bid, max_sell))
        if cur_diff - wall_mid + best_bid - mean_diff >= THR_CLOSE and pos > 0:
            asks.append((best_bid, pos))
        elif cur_diff - wall_mid + best_ask - mean_diff <= -(thr_open_buy + low_adj) and max_buy > 0:
...
def _momentum_thrs(
    u_mom: float, base: float, coef: float
) -> tuple[float, float]:
    """widen bid threshold when u_mom>0, widen ask (sell) when u_mom<0; clamp to [0.36,0.60]."""
    s_sell = base * (1.0 - coef * u_mom)
    s_buy = base * (1.0 + coef * u_mom)
    lo, hi = 0.36, 0.60
    return (max(lo, min(hi, s_sell)), max(lo, min(hi, s_buy)))
...
class Trader:
    def run(self, state: TradingState):
        ...
        if ubb is None or uba is None:
            ...
            return {}, 0, json.dumps(store, separators=(",", ":"))
        S = 0.5 * (ubb + uba)
        prev_s = ema.get("S_last_raw")
        if prev_s is not None and float(prev_s) > 0:
            mom = (float(S) - float(prev_s)) / max(abs(float(prev_s)), 1.0)
        else:
            mom = 0.0
        _ema(ema, "u_mom", MOMENTUM_EMA_WINDOW, float(mom))
        ema["S_last_raw"] = float(S)
        ...
        u_mom = float(ema.get("u_mom", 0.0))
        thr_sell, thr_buy = _momentum_thrs(u_mom, THR_OPEN, THR_OPEN_MOM_COEF)
