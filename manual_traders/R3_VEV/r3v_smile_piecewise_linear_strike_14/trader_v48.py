"""
v41 + extract-shock gate (Greek thread): tape shows L1 voucher spreads barely widen on
large extract mid jumps (see tape_extract_shock_spread_widen.json), so microstructure does
not protect fair-value; scale up THR_OPEN/THR_CLOSE when |S-S_prev| is large, weighted by
BS gamma (convexity risk under spot moves).
"""
...
# Raw extract mid tick-to-tick; tape rarely has >=8 tick jumps but 4 catches more activity.
EXTRACT_SHOCK_ABS = 4.0
# thr *= 1 + SHOCK_THR_MULT * min(1, gamma * GAMMA_SCALE) when shock
SHOCK_THR_MULT = 0.14
GAMMA_SCALE = 8000.0
