"""
vouchers_final_strategy: joint 5200+5300 L1 <= 2, extract + 5200/5300 only.

v52 used aggressive (taker-style) market-side orders; STRATEGY.txt notes mid forward
edge can vanish after spread + worse fills. v54: in **tight** regime, post **passive**
limits one tick *inside* the BBO (when spread >= 2) or join the touch (when spread=1)
— still driven by the same EMA of tick Δ(extract wall_mid) as a directional *intent*.

Wide regime: same flattening as v52 (only when not tight). No HYDROGEL.
"""
...
def _ema(st: dict[str, float], k: str, w: int, x: float) -> float:
    a = 2.0 / (w + 1.0)
    old = float(st.get(k, 0.0))
    n = a * x + (1.0 - a) * old
    st[k] = n
    return n
...
def _passive_buy_price(bb: int, ba: int) -> int:
    """One tick better than best bid if possible, else join bid."""
    if ba - bb >= 2:
        return min(bb + 1, ba - 1)
    return bb


def _passive_sell_price(bb: int, ba: int) -> int:
    if ba - bb >= 2:
...
class Trader:
    def run(self, state: TradingState):
        ...
        d52, d53 = depths.get(S5200), depths.get(S5300)
        ...
        if tight:
            if ema_m > MOMENTUM_LONG and p_u < LIM_U - Q_EXTRACT:
                px = _passive_buy_price(ubb, uba)
                add(
                    U,
                    [Order(U, px, min(Q_EXTRACT, LIM_U - p_u))],
                )
            elif ema_m < -MOMENTUM_SHORT and p_u > -LIM_U + Q_EXTRACT:
                px = _passive_sell_price(ubb, uba)
                add(
                    U,
                    [Order(U, px, -min(Q_EXTRACT, LIM_U + p_u))],
                )
            for sym in VEV_TIGHT:
                ...
                if ema_m > MOMENTUM_LONG and pv < LIM_V - Q_VEV:
                    px = _passive_buy_price(bb, ba)
                    add(sym, [Order(sym, px, min(Q_VEV, LIM_V - pv))])
                elif ema_m < -MOMENTUM_SHORT and pv > -LIM_V + Q_VEV:
                    px = _passive_sell_price(bb, ba)
                    add(sym, [Order(sym, px, -min(Q_VEV, LIM_V + pv))])
        else:
            if p_u > 0:
