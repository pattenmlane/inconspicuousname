"""
Iteration 1: same WLS quadratic smile + ridge prior as v0; parameter sweep —
lower edge threshold (0.18 vs 0.35), lower min vega (0.03), passive VEV limits
at touch vs fair (buy at min(bid, floor(fair)-off)), buffer IV every 3 steps,
slightly larger voucher clips. See results.json iteration notes.
"""
    ...
    def run(self, state: TradingState):
        ...
        for sym in VOUCHERS:
            ...
            if vg < _MIN_VEGA:
                continue
            edge = mid - fair
            thr = _EDGE_FRAC * float(max(spr, 1))
            if abs(edge) < thr:
                continue
            lim = LIMITS.get(sym, 300)
            p0 = int(pos.get(sym, 0))
            off = max(1, int(_EDGE_FRAC * float(spr)))
            orders_sym: list[Order] = []
            if edge < -thr and p0 < lim:
                tgt = int(math.floor(fair)) - off
                px = min(bbp, tgt)
                if px > 0:
        ...
        # Light extract MM (same microstructure style as hydrogel)
        if bb_u is not None and ba_u is not None and ba_u > bb_u + 1:
