"""
Iteration 14: parent v10. Same WLS smile + BS fair, theta+spread-scaled edges.

**Execution signal** uses *level-1 volume-weighted microprice* for edge = micro - fair
(imbalance of bid/ask size at the touch) instead of (bid+ask)/2. Backtest still
crosses at best bid/ask. Tape analysis: microprice has slightly lower mean |error|/vega
vs mid in microprice_smile_mispricing_iter14.json.
"""
...
def best_bid_ask_l1(
    depth: OrderDepth | None,
) -> tuple[int | None, int | None, int | None, int | None]:
    """(best_bid, bid_vol_pos, best_ask, ask_vol_pos). Volumes are positive."""
    bbp, bap = best_bid_ask(depth)
    if depth is None or bbp is None or bap is None:
        return None, None, None, None
    bv = int(depth.buy_orders[bbp])
    av = abs(int(depth.sell_orders[bap]))
    return bbp, bv, bap, av


def l1_microprice(d: OrderDepth | None) -> float | None:
    bbp, bv, bap, av = best_bid_ask_l1(d)
    if bbp is None or bv is None or bap is None or av is None or bv + av == 0:
        return None
...
class Trader:
    def run(self, state: TradingState):
        ...
        for sym in VOUCHERS:
            ...
            mark = float(micro) if micro is not None else mid
            K = float(sym.split("_")[1])
            m = math.log(K / S) / sqrtT
