"""
Iteration 17: parent v16. Add extract-shock and beta-aware execution controls:
- tape analysis shows lag-0 extract->voucher propagation (lag-1 ~0),
- edge quality degrades sharply in large |dS| shocks,
- higher-beta strikes (5000-5100 / ITM) carry better move-through.
Keep same WLS smile fair + theta/spread gates; only adjust trade aggressiveness.
"""
...
_SHOCK_ABS_MID = 1.5
_SHOCK_ABS_LARGE = 2.0
_SHOCK_GATE_MID = 1.15
_SHOCK_GATE_LARGE = 1.35
_SHOCK_CLIP_MID = 0.8
_SHOCK_CLIP_LARGE = 0.55
_BETA_NEUTRAL = 0.35
_BETA_GAIN = 0.45
...
def strike_beta(sym: str) -> float:
    # Estimated from ROUND_3 tape regression dC_t ~ beta * dS_t.
    return {
        "VEV_4000": 0.745,
        "VEV_4500": 0.662,
        "VEV_5000": 0.653,
...
class Trader:
    def run(self, state: TradingState):
        ...
        dS_abs = abs(S - float(prev_S)) if isinstance(prev_S, (int, float)) else 0.0
        ...
        for sym in VOUCHERS:
            ...
            if dS_abs >= _SHOCK_ABS_LARGE:
                shock_gate = _SHOCK_GATE_LARGE
                shock_clip = _SHOCK_CLIP_LARGE
            elif dS_abs >= _SHOCK_ABS_MID:
                shock_gate = _SHOCK_GATE_MID
                shock_clip = _SHOCK_CLIP_MID
            else:
                shock_gate = 1.0
                shock_clip = 1.0
            beta = strike_beta(sym)
            # Favor high-beta vouchers when extract shock is large; damp low-beta wings.
            beta_mult = max(0.7, min(1.3, 1.0 + _BETA_GAIN * (beta - _BETA_NEUTRAL)))
            regime_mult = shock_gate / beta_mult
            edge = mark - fair
            e_ov = _EDGE_OVER_VEGA_MIN * th_m * regime_mult
            if abs(edge) / max(vg, 1e-6) < e_ov:
                continue
            thr = _EDGE_FRAC * float(spr) * th_m * regime_mult
            if abs(edge) < thr:
                continue
            ...
            if not core:
                cap = max(1, cap // 2)
            cap = max(1, int(round(float(cap) * shock_clip * beta_mult)))
