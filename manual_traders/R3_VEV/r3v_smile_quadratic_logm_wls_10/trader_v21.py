"""
Iteration 21: parent v19. Data-grounded tail shock hard-stop from
shock_tail_by_symbol_iter21: both core and wing vouchers show negative edge
quality proxy in |dS|>=2.0 regime, so skip voucher entries in that tail and
trade unchanged logic otherwise.
"""
...
class Trader:
    def run(self, state: TradingState):
        ...
        for sym in VOUCHERS:
            ...
            in_tail_shock = abs_dS >= _SHOCK_TAIL_ABS_DS
            if _TAIL_FREEZE_VOUCHERS and in_tail_shock:
                continue
            shock_gate = _SHOCK_TAIL_GATE_MULT if in_tail_shock else 1.0
            clip_mult = _SHOCK_TAIL_CLIP_MULT if in_tail_shock else 1.0
            edge = mark - fair
