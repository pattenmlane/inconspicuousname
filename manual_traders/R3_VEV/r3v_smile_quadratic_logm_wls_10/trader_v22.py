"""
Iteration 22: parent v19 (WLS smile + micro + theta/spread + shock tail).

Shared thesis (round3work/vouchers_final_strategy/STRATEGY.txt): when VEV_5200 and
VEV_5300 both have top-of-book spread <= TH=2 at the same time, treat as risk-on:
size up extract + VEV clip; when either is wide, scale down (Sonic-style regime).
No HYDROGEL_PACK — focus PnL on extract + vouchers only.
"""
...
VEV_5300 = "VEV_5300"
_JOINT_SPREAD_TH = 2
_EXTRACT_MM_TIGHT = 20
_EXTRACT_MM_WIDE = 6
_VEV_GATE_SIZE_ON = 1.25
...
_SHOCK_TAIL_ABS_DS = 2.0
_SHOCK_TAIL_GATE_MULT = 1.35
_SHOCK_TAIL_CLIP_MULT = 0.7
...
def top_of_book_spread(depth: OrderDepth | None) -> int | None:
    bbp, bap = best_bid_ask(depth)
    if bbp is None or bap is None:
        return None
    return int(bap - bbp)


def joint_tight_gate(depths: dict[str, Any]) -> bool:
    """True iff s5200<=TH and s5300<=TH (STRATEGY.txt)."""
    s0 = top_of_book_spread(depths.get(VEV_5200))
    s1 = top_of_book_spread(depths.get(VEV_5300))
    if s0 is None or s1 is None:
        return False
    return s0 <= _JOINT_SPREAD_TH and s1 <= _JOINT_SPREAD_TH


def l1_microprice(d: OrderDepth | None) -> float | None:
    bbp, bap = best_bid_ask(d)
    if d is None or bbp is None or bap is None:
        return None
    bv = int(d.buy_orders[bbp])
    av = abs(int(d.sell_orders[bap]))
    if bv + av == 0:
        return None
    return (float(bbp) * float(av) + float(bap) * float(bv)) / float(bv + av)
...
class Trader:
    def run(self, state: TradingState):
        ...
        abs_dS = abs(S - float(prev_S)) if isinstance(prev_S, (int, float)) else 0.0
        gate_on = joint_tight_gate(depths)
        ...
        for sym in VOUCHERS:
            ...
            if spr < _MIN_VEV_SPREAD:
                continue
            mid = 0.5 * float(bbp) + 0.5 * float(bap)
            micro = l1_microprice(d)
            if micro is not None and math.isfinite(micro):
                mark = (1.0 - _MARK_BLEND) * mid + _MARK_BLEND * float(micro)
            else:
                mark = mid
            K = float(sym.split("_")[1])
            m = math.log(K / S) / sqrtT
            ...
            shock_gate = _SHOCK_TAIL_GATE_MULT if abs_dS >= _SHOCK_TAIL_ABS_DS else 1.0
            clip_mult = _SHOCK_TAIL_CLIP_MULT if abs_dS >= _SHOCK_TAIL_ABS_DS else 1.0
            edge = mark - fair
            e_ov = _EDGE_OVER_VEGA_MIN * th_m * shock_gate
            if abs(edge) / max(vg, 1e-6) < e_ov:
                continue
            thr = _EDGE_FRAC * float(spr) * th_m * shock_gate
            if abs(edge) < thr:
                continue
            ...
            if not core:
                cap = max(1, cap // 2)
            cap = max(1, int(round(float(cap) * clip_mult)))
            if gate_on:
                cap = max(1, int(round(float(cap) * _VEV_GATE_SIZE_ON)))
        ...
        if ba_u > bb_u + 1:
            ...
            up = int(pos.get(U, 0))
            mm = _EXTRACT_MM_TIGHT if gate_on else _EXTRACT_MM_WIDE
            ub = bb_u + 1
            ua = ba_u - 1
            if ub < ua:
                uo: list[Order] = []
                if up < ulim:
                    uo.append(Order(U, ub, min(mm, ulim - up)))
                if up > -ulim:
                    uo.append(Order(U, ua, -min(mm, ulim + up)))
                if uo:
                    result[U] = uo
