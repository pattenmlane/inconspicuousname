"""
Iteration 6: same WLS quadratic-IV smile + BS fair (parent: v5).
- After prior warmup, **smoothly blend** pooled-tape WLS (prior) and online ridge coefficients
  instead of a hard switch at step 120.
- EMA of extract mid step-changes: when |EMA(dS)| is large, **widen** edge/vega gates slightly and
  **shrink** clip size (small grid sweep on v5) so we trade less on stale fair during sharp moves.
  DTE from round3work/plotting helpers: dte_effective = (8-csv_day) - floor(ts/100)/10000.
"""
...
_BLEND_END_STEPS = 300
_EMA_DS_ALPHA = 0.08
_MOVE_NORM = 2.5
_MOVE_EDGE_MULT = 0.1
_MOVE_CAP_SCALE = 0.15
_GEL_SIZE = 22
_EXTRACT_MM = 10
_WARMUP_STEPS = 10
...
def blend_w(step: int) -> float:
    if step < _PRIOR_ONLY_STEPS:
        return 0.0
    if step >= _BLEND_END_STEPS:
        return 1.0
    return float(step - _PRIOR_ONLY_STEPS) / float(max(1, _BLEND_END_STEPS - _PRIOR_ONLY_STEPS))


def blended_abc(
    a_on: float, b_on: float, c_on: float, w: float
) -> tuple[float, float, float]:
    w = min(1.0, max(0.0, w))
    o = 1.0 - w
    return o * _PA + w * a_on, o * _PB + w * b_on, o * _PC + w * c_on
...
class Trader:
    def run(self, state: TradingState):
        ...
        last = store.get(_KEY_S)
        ema = float(store.get(_KEY_EMA, 0.0))
        if isinstance(last, (int, float)) and math.isfinite(float(last)):
            d_s = S - float(last)
            ema = _EMA_DS_ALPHA * d_s + (1.0 - _EMA_DS_ALPHA) * ema
        move_score = min(1.0, abs(ema) / _MOVE_NORM)
        edge_mult = 1.0 + _MOVE_EDGE_MULT * move_score
        ...
        w_ol = blend_w(step)
        fa, fb, fc = blended_abc(a, b, c, w_ol)

        vega_min = _MIN_VEGA
        e_ov = _EDGE_OVER_VEGA_MIN * edge_mult
        ef = _EDGE_FRAC * edge_mult
        ...
        for sym in VOUCHERS:
            ...
            if not core:
                cap = max(1, cap // 2)
            cap = max(1, int(cap * (1.0 - _MOVE_CAP_SCALE * move_score)))
