"""
Iteration 7: parent v5; replace hard prior vs online switch with **smooth linear blend** from
pooled WLS to ridge-fit coeffs between steps 120 and 280 (grid vs v6 blend window).
No extract-move gating (ablation: v6 regressed overall).
"""
    ...
    def run(self, state: TradingState):
        ...
        for sym in VOUCHERS:
            ...
            if vg < _MIN_VEGA:
                continue
            edge = mid - fair
            if abs(edge) / max(vg, 1e-6) < _EDGE_OVER_VEGA_MIN:
                continue
