"""
Iteration 8: parent trader_v5 (best PnL so far). Same WLS quadratic IV in m, BS fair, touch cross.

Greek add-on: Black-Scholes call |theta| (r=0) at fitted sigma(m) as a *time risk* scale for
execution — higher |theta| widens the edge fraction and vega edge gate slightly so we are pickier
when the fair is moving quickly in calendar time. Grid from tape medians: see greek_buckets_iter8.json.
"""
...
class Trader:
    def run(self, state: TradingState):
        ...
        for sym in VOUCHERS:
            ...
            th_m = theta_edge_multiplier(th_abs)
            edge = mid - fair
