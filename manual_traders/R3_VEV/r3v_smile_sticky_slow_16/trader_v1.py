"""
Round 3 — sticky slow smile (EWMA quadratic IV in m_t); v1: no standalone extract MM (v0 bled
there); book-aware half-spread floor; slightly faster smile tracking (alpha sweep winner offline).

Same DTE/T as trader_v0 (round3description + plot_iv_smile intraday winding).
"""
...
class Trader:
    def run(self, state: TradingState):
        ...
        for v in VOUCHERS:
            ...
            if bb is None or ba is None:
                continue
            half_spread = max(1.0, 0.45 * float(ba - bb))
            atm_w = math.exp(-0.5 * m_t * m_t)
            base_width = 1.0 + 1.4 * (1.0 - atm_w)
            shift = base_width + half_spread + w * max(-EDGE_CLIP, min(EDGE_CLIP, edge)) * 0.055
