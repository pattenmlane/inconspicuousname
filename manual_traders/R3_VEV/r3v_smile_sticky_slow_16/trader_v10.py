"""
Round 3 — v2 with **double EWMA** on the quadratic smile coefficients (slow “outer” layer).

First layer: EMA(0.045) of raw vega-weighted polyfit, same as v2 (stored in ema_coeff).
Second layer: EMA(0.12) of the first layer (stored in ema2_coeff); all fair IV/BS/vega use ema2.

DTE: round3description + intraday winding (dte_effective). Wing liquidity filter as v2.
"""
...
EWMA_ALPHA = 0.045
# Outer slow layer on the already-smoothed coeff vector (thesis: fair chases every tick even less)
EWMA2_ALPHA = 0.12
...
class Trader:
    def run(self, state: TradingState):
        ...
        ema2 = store.get("ema2_coeff")
        if not isinstance(ema2, list) or len(ema2) != 3:
            ema2 = [float(ema[0]), float(ema[1]), float(ema[2])]
        a2 = EWMA2_ALPHA
        ema2 = [(1 - a2) * float(ema2[i]) + a2 * float(ema[i]) for i in range(3)]
        store["ema2_coeff"] = ema2
        c2, c1, c0 = float(ema2[0]), float(ema2[1]), float(ema2[2])
