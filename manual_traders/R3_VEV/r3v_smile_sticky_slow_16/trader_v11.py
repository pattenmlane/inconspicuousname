"""
Round 3 — family 12 pivot: smile_cubic_spline_logk with wing regularization + sticky nodes.

- Fit smile in x=log(K/S)/sqrt(T) using cubic spline over strike nodes.
- Wing regularization: blend per-node IV toward quadratic baseline before spline (lambda=0.15),
  with natural spline boundary conditions to reduce extrapolation noise.
...
NODE_EWMA_ALPHA = 0.045
WING_REG_LAMBDA = 0.15
...
class Trader:
    def run(self, state: TradingState):
        ...
        spline = CubicSpline(x_sort, y_sort, bc_type="natural")
