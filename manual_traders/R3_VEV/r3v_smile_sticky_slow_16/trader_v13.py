"""
Round 3 — family 12 variant: spline logk with synthetic wing anchors + sticky nodes.

- Fit smile in x=log(K/S)/sqrt(T) using cubic spline over strike nodes.
- Wing regularization: blend per-node IV toward quadratic baseline (lambda=0.10), then add synthetic anchor nodes
  25% outside observed logk range from quadratic baseline before natural spline fit.
- Sticky update: EWMA on per-strike node IVs so fair does not chase every tick.

Products: HYDROGEL_PACK, VELVETFRUIT_EXTRACT, VEV_* only. DTE as round3description + intraday wind.
"""
...
SHOCK_ABS_DS = 2.0
CORE_MAX_SPREAD_FOR_ACTION = 3.0
CORE_TAKE_EDGE = 0.6
CORE_TAKE_SIZE = 6
...
class Trader:
    def run(self, state: TradingState):
        ...
        prev_s = store.get("prev_s_mid")
        dS = 0.0
        if isinstance(prev_s, (int, float)) and math.isfinite(float(prev_s)):
            dS = float(s_mid) - float(prev_s)
        ...
        for v in VOUCHERS:
            ...
            fair_iv = float(spline(x))
            fair_iv = max(1e-4, min(8.0, fair_iv))
            fair = bs_call_price(float(s_mid), float(K), T, fair_iv, 0.0)
            mid = float(mids_v[v])
            vega = bs_vega(float(s_mid), float(K), T, fair_iv, 0.0)
            edge = mid - fair
            ...
            half_spread_floor = max(0.5 if is_core else 1.0, (0.25 if is_core else 0.35) * float(ba - bb))
            atm_w = math.exp(-0.5 * x * x)
            base_width = 0.8 + 1.2 * (1.0 - atm_w) if is_core else 1.0 + 1.4 * (1.0 - atm_w)
            shift = base_width + half_spread_floor + w * max(-EDGE_CLIP, min(EDGE_CLIP, edge)) * (0.04 if is_core else 0.055)

            pos_v = int(pos.get(v, 0))
            lim = LIMITS[v]

            ol: list[Order] = []
            # Contract-by-contract propagation logic: core strikes respond immediately to S shocks.
            if is_core and half_spread <= CORE_MAX_SPREAD_FOR_ACTION and abs(dS) >= SHOCK_ABS_DS:
                if dS > 0 and edge > CORE_TAKE_EDGE:
                    avail = int(-dv.sell_orders.get(ba, 0))
                    tq = min(CORE_TAKE_SIZE, max(0, lim - pos_v), max(0, avail))
                    if tq > 0:
                        ol.append(Order(v, int(ba), tq))
                elif dS < 0 and edge < -CORE_TAKE_EDGE:
                    avail = int(dv.buy_orders.get(bb, 0))
                    tq = min(CORE_TAKE_SIZE, max(0, lim + pos_v), max(0, avail))
                    if tq > 0:
                        ol.append(Order(v, int(bb), -tq))
            ...
            q_buy = min(QUOTE_SIZE_VEV + (4 if is_core and abs(dS) >= SHOCK_ABS_DS else 0), lim - pos_v)
            q_sell = min(QUOTE_SIZE_VEV + (4 if is_core and abs(dS) >= SHOCK_ABS_DS else 0), lim + pos_v)
            if edge > 1.5:
