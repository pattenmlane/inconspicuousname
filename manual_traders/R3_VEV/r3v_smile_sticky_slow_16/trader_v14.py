...
class Trader:
    def run(self, state: TradingState):
        ...
        for v in VOUCHERS:
            ...
            q_buy = min(QUOTE_SIZE_VEV + (2 if is_core and abs(dS) >= SHOCK_ABS_DS else 0), lim - pos_v)
            q_sell = min(QUOTE_SIZE_VEV + (2 if is_core and abs(dS) >= SHOCK_ABS_DS else 0), lim + pos_v)
            if edge > 1.5:
