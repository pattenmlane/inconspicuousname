"""
Round 3 — family 12: v12 spline + poly blend for fill-friendly quotes (no taker).

- Same as v12: cubic spline in x=log(K/S)/sqrt(T), wing regularization, anchor nodes, sticky node IV EWMA.
- New: vega-weighted quadratic fit on the same sticky node IVs with coefficient EWMA (v2-style) — fair is a
  blend in *price* between BS(spline IV) and BS(poly IV) so |mid−fair| behavior moves toward the quadratic
  regime that previously produced passive fills, without discarding the spline’s wing shaping.
- Slightly more aggressive width floor vs v12 to improve passive trade rate under --match-trades worse.
