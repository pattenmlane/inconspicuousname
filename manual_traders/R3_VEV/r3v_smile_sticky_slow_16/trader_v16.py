"""
Round 3 — family 12: v15 + strike-bucket grid on poly blend w(K) (sweep, same width as v15).

- Same as v15 except per-strike w: 0.42 in 5000-5300, 0.32 at 5400/5500, 0.30 on deep wings (4k/4.5k/6k/6.5k),
  0.35 default. Falsified on backtest: total 18,920 < v15 18,968; wing w<0.35 increased adverse 5400/4000 flow.
"""
...
POLY_W_CORE_HI = 0.42
POLY_W_MID_54_55 = 0.32
POLY_W_DEEP_WING = 0.30
POLY_W_DEFAULT = 0.35
...
def poly_blend_weight_for_strike(K: int) -> float:
    if 5000 <= K <= 5300:
        return float(POLY_W_CORE_HI)
    if K in (5400, 5500):
        return float(POLY_W_MID_54_55)
    if K in (4000, 4500, 6000, 6500):
        return float(POLY_W_DEEP_WING)
    return float(POLY_W_DEFAULT)
...
class Trader:
    def run(self, state: TradingState):
        ...
        # Quote loop — same width rule as v15; only bw varies by strike
        for v in VOUCHERS:
            if v not in mids_v:
                continue
            dv = depths.get(v)
            ...
            bw = poly_blend_weight_for_strike(K)
