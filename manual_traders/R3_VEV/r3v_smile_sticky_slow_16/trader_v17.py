"""
Round 3 — family 12: v15 + core-only higher poly weight.

- Same as v15 (spline, sticky nodes, EMA poly, width) except: w=0.40 only for 5000-5300 (tape: largest
  poly/spline |edge| ratio there; underlying beta meaningful per underlying_propagation_by_strike).
- All other strikes keep w=0.35 to match v15 on wings (v16 showed lowering w on wings hurt vs v15).
"""
...
def poly_blend_weight_for_strike(K: int) -> float:
    if 5000 <= K <= 5300:
        return float(POLY_W_CORE)
    return float(POLY_W_BASE)
