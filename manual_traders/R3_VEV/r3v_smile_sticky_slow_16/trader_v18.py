"""
Round 3 — family 12: v15 + extract-shock-gated *core-only* quote tightening (passive only).

- Same spline + sticky node IV + poly blend (w=0.35) and hydrogel as trader_v15.
- Track prior VELVETFRUIT_EXTRACT mid in traderData; shock = |S - S_prev| >= 2 (same scale as prior propagation work).
- On shock only, for strikes 5000-5300: slightly lower half_spread_floor coeff (0.27 vs 0.30) and vega edge mult
  (0.048 vs 0.05) to try crossing integer tick barriers where v17’s w-only tweak was invisible; wings unchanged.

Tape (extract_shock_spread_core_wing.json): core VEV median top spread stays 3 on shock timestamps vs calm;
deep wings stay ~16 — so we do not widen/tighten wings on shock; core-only adjustment is the conservative lever.
"""
...
# v15 baseline: floor max(0.5, 0.3*book), edge_mult 0.05
CORE_SHOCK_SPREAD_COEF = 0.27
CORE_SHOCK_EDGE_MULT = 0.048
DEFAULT_SPREAD_COEF = 0.3
DEFAULT_EDGE_MULT = 0.05
...
class Trader:
    def run(self, state: TradingState):
        ...
        shock = False
        if isinstance(s_prev, (int, float)) and math.isfinite(float(s_prev)):
            shock = abs(float(s_mid) - float(s_prev)) >= SHOCK_ABS_DS
        ...
        for v in VOUCHERS:
            ...
            book_spread = float(ba - bb)
            if shock and K in CORE_STRIKES:
                half_spread_floor = max(0.5, CORE_SHOCK_SPREAD_COEF * book_spread)
                edge_mult = CORE_SHOCK_EDGE_MULT
            else:
                half_spread_floor = max(0.5, DEFAULT_SPREAD_COEF * book_spread)
                edge_mult = DEFAULT_EDGE_MULT

            atm_w = math.exp(-0.5 * x * x)
            base_width = 1.0 + 1.4 * (1.0 - atm_w)
            shift = base_width + half_spread_floor + wq * max(-EDGE_CLIP, min(EDGE_CLIP, edge)) * edge_mult
