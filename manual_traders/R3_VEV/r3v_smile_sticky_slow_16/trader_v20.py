"""
Round 3 — joint voucher book + extract (tight two-leg gate from shared strategy).

- **Regime:** trade VELVETFRUIT_EXTRACT + all VEV_* only when **VEV_5200** and **VEV_5300** both have
  top-of-book spread (ask1−bid1) ≤ 2 at this timestamp — otherwise no quotes (empty regime).
- **No HYDROGEL_PACK** (PnL objective: extract + vouchers only per team brief).
- Fairing: same sticky spline + poly blend as v15 (IV nodes EWMA, wing-regularized cubic spline, BS fair).

DTE: round3 work csv day 0→8d at open, intraday wind in t_years_effective.
"""
...
class Trader:
    def run(self, state: TradingState):
        ...
        if bb_u is not None and ba_u is not None:
            ...
            if ua <= ub:
                ua = ub + 1
            qu = min(U_QUOTE_SIZE, 200 - pos_u)
            qus = min(U_QUOTE_SIZE, 200 + pos_u)
            uo: list[Order] = []
            if qu > 0 and ub > 0:
