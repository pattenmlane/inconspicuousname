"""
Round 3 — sticky EWMA smile (slow coeff updates) + selective taker leg on tight books.

v3 keeps v2 core but adds:
- if |mid - fair| is large AND top spread is tight, cross one level with bounded size;
- still skips wide-wing quotes unless edge compensates;
- keeps slow hydrogel EWMA market making.
"""
...
# New selective taker parameters from take_edge_grid analysis.
TAKE_EDGE_THRESHOLD = 2.0
TAKE_MAX_SPREAD = 2.0
TAKE_SIZE = 6
...
class Trader:
    def run(self, state: TradingState):
        ...
        for v in VOUCHERS:
            ...
            # Selective taker on large edge with tight spread.
            if half_spread <= TAKE_MAX_SPREAD and abs(edge) >= TAKE_EDGE_THRESHOLD:
                if edge > 0:
