"""
Round 3 — sticky EWMA smile (slow coeff updates), conservative execution.

v4 keeps the same thesis as v2:
- slow EWMA updates of smile coefficients to avoid chasing every tick,
- voucher quoting off BS fair,
- wide-wing liquidity filter.

After v3 (broad taker leg) underperformed badly, v4 only allows small taker clips near ATM.
"""
...
# v4: only near-ATM taker clips on very tight markets
TAKE_EDGE_THRESHOLD = 2.6
TAKE_MAX_SPREAD = 1.0
TAKE_SIZE = 3
ATM_ABS_MT_MAX = 0.55
...
class Trader:
    def run(self, state: TradingState):
        ...
        for v in VOUCHERS:
            ...
            # Conservative taker: only near-ATM and very tight spread.
            if abs(m_t) <= ATM_ABS_MT_MAX and half_spread <= TAKE_MAX_SPREAD and abs(edge) >= TAKE_EDGE_THRESHOLD:
                if edge > 0:
                    qty = min(TAKE_SIZE, lim - pos_v)
                    avail = int(-dv.sell_orders.get(ba, 0))
                    qty = min(qty, max(avail, 0))
                    if qty > 0:
                        ol.append(Order(v, int(ba), qty))
                else:
                    qty = min(TAKE_SIZE, lim + pos_v)
                    avail = int(dv.buy_orders.get(bb, 0))
                    qty = min(qty, max(avail, 0))
                    if qty > 0:
                        ol.append(Order(v, int(bb), -qty))
