"""
Round 3 — v2 + **slower extract anchor** for the entire smile/BS chain.

Idea: IV smile is fit in m_t = log(K/S)/sqrt(T). Using tick micro-mid for S makes m_t
and implied-IV inversions chase noise; v6 EWMA-smooths S (VELVETFRUIT_EXTRACT) with the
same rate as the smile-coeff EMA (0.045), then uses S_ema for implied vol, polyfit, fair
price, and vega. Mid quotes still use raw voucher mids. Wing liquidity filter unchanged.

DTE: round3work/round3description.txt + plot_iv_smile-style intraday winding (dte_effective).
No standalone extract orders (as v1+).
"""
...
EWMA_ALPHA = 0.045
# Smooth underlying for smile/PnL-decomposition only; match EWMA_alpha per anchor_ewma_smile_stability.json grid
S_EWMA_ALPHA = 0.045
...
class Trader:
    def run(self, state: TradingState):
        ...
        prev = store.get("S_ema")
        if not isinstance(prev, (int, float)) or not math.isfinite(float(prev)):
            s_ema = float(s_mid)
        else:
            a = S_EWMA_ALPHA
            s_ema = (1.0 - a) * float(prev) + a * float(s_mid)
        store["S_ema"] = s_ema
        S = float(s_ema)
        ...
        for v in VOUCHERS:
            ...
            if m is None:
                continue
            K = float(v.split("_")[1])
            iv = implied_vol_call(float(m), S, K, T, 0.0)
            if iv is None:
                continue
            m_t = math.log(K / S) / sqrtT
            xs.append(m_t)
            ys.append(iv)
            vegas.append(bs_vega(S, K, T, iv, 0.0))
            mids_v[v] = float(m)
        ...
        for v in VOUCHERS:
            ...
            fair = bs_call_price(S, float(K), T, fair_iv, 0.0)
            mid = mids_v[v]
            vega = bs_vega(S, float(K), T, fair_iv, 0.0)
            edge = mid - fair
            ...
            shift = base_width + half_spread_floor + w * max(-EDGE_CLIP, min(EDGE_CLIP, edge)) * 0.055
