"""
family13 / realized_vol_regime variation:
- Signal = reversed RV-IV gap blended with spread-stress indicator (validated by worse-fill sweep).
- Stressed regime: core strikes only (5000/5100/5200) + tighter inventory rails.
- Calm regime: broader participation (4500..5500 around core), wider rails.

All IV computations use BS implied vols from mids with T=(8-d)/365 proxy context from
round3description mapping (day-index->TTE). Runtime strategy uses rolling realized vol; reverse sign after sweep showed prior sign was anti-alpha in this tape
from underlying mids and option IV snapshots from core strikes.
"""
...
class Trader:
    def run(self, state: TradingState):
        ...
        # RV from underlying log returns
        rv = 0.0
        if len(mids_hist) >= 20:
        ...
        # Core IV + stress proxy from spread pressure
        core_mids = {}
        ...
        # normalize spread stress in [0,1]-ish
        stress_raw = (avg_spread - 3.0) / 10.0
        ...
        orders = {}
        # Mean-reversion on rich/cheap relative to iv-core anchor via synthetic theo premium
        # if signal high (rv > iv + stress), buy convexity-like shape; if low, sell.
        action = 0
