"""
family13 / realized_vol_regime variation C-relaxed (analysis-led):
- Signal = reversed (RV-IV gap + stress blend), calibrated under worse fills.
- Stress proxy includes spread + depth thinning on core strikes.
- Regime logic:
  * stress: core-only {5000,5100,5200} with tighter rails.
  * calm: participation focused on response-efficient strikes from tape analysis
    {5100,5200,5300} (best beta/spread efficiency), avoiding very wide wings.
- Underlying-response finding used: voucher response is mostly contemporaneous
  (lag/lead ~ 0), so we avoid predictive lag logic and trade on current state only.
"""
...
CALM_SET = ["VEV_5100", "VEV_5200", "VEV_5300"]
...
GAP_ENTER_CALM = 0.04
GAP_EXIT_CALM = 0.015
GAP_ENTER_STRESS = 0.09
GAP_EXIT_STRESS = 0.035
W_GAP = 0.9
W_STRESS = 0.1
...
class Trader:
    def run(self, state: TradingState):
        ...
        all_products = sorted(set(CORE + CALM_SET + [U]))
        syms = {p: _sym(state, p) for p in all_products}
        ...
        # RV proxy: EWMA of abs log-returns.
        rv = 0.0
        if len(u_hist) >= 20:
            rets = [abs(math.log(u_hist[i] / u_hist[i - 1])) for i in range(1, len(u_hist)) if u_hist[i - 1] > 0 and u_hist[i] > 0]
            ew = 0.0
            for r in rets:
                ew = EWMA_ALPHA * r + (1.0 - EWMA_ALPHA) * ew
            rv = ew * SCALE_ANNUAL

        # Core IV + stress from spread and depth-thinning.
        core_mid, spread_sum, depth_inv_sum, cnt = {}, 0.0, 0.0, 0
        for p in CORE:
            s = syms.get(p)
            if s is None:
                continue
        ...
        # Reverse-sign blend validated in family13 sweeps.
        ...
        universe = CORE if regime == "stress" else CALM_SET
