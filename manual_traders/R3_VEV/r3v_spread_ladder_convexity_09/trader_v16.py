"""
family13 / realized_vol_regime: parent v15.

v16 data-led tweak (ablation): add small extra stress on **up** shocks only
(|ΔU|>=SHOCK_DU and dU>0), on top of v15. Tape shows 5200 spread change more
positive on up than down at the same threshold; this version still hurt PnL vs
v15 on backtest — see results iteration 16 vs 15.
"""
...
SHOCK_DU = 2.5
SHOCK_STRESS = 0.10
# Up-shock microstructure premium (tape: VEV_5200 mean spread change more positive on up vs down)
SHOCK_UP_EXTRA = 0.05
...
class Trader:
    def run(self, state: TradingState):
        ...
        if len(mids_hist) >= 1 and mids_hist[-1] > 0:
            du_signed = float(u_mid) - float(mids_hist[-1])
        du = abs(du_signed)
        ...
        shock = du >= SHOCK_DU
        up_shock = shock and du_signed > 0.0
        stress_raw = stress_base + (SHOCK_STRESS if shock else 0.0) + (SHOCK_UP_EXTRA if up_shock else 0.0)
        ...
        td["du_signed"] = round(du_signed, 4)
        td["shock"] = int(shock)
        td["up_shock"] = int(up_shock)
