...
v17: when |Δextract| passes the shock threshold, negative (down) extract moves
get an extra stress bump (signed-asymmetry in next-tick spread dynamics on
5000/5100/5200/5300; see analysis_shock_spread_by_sign_v16.json). v16 tested
up-only extra and reduced PnL vs v15.
"""
...
class Trader:
    def run(self, state: TradingState):
        ...
        shock = du >= SHOCK_DU
        down_shock = shock and du_signed < 0.0
        stress_raw = stress_base + (SHOCK_STRESS if shock else 0.0) + (SHOCK_DOWN_EXTRA if down_shock else 0.0)
