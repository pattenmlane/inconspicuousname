...
v19 data-led tweak: tape shows on large |Δextract| (>=3 ticks) average next-tick
...
BROAD = ["VEV_5100", "VEV_5200", "VEV_5300"]
...
# Smile interaction term: dbump correlates with dU (analysis_smile_shock_v18.json).
BUMP_COEF = 8.0
...
class Trader:
    def run(self, state: TradingState):
        ...
        if not isinstance(mids_hist, list):
            mids_hist = []
        prev_bump = float(td.get("prev_bump", 0.0))
        ...
        bump = 0.0
        if len(ivs) >= 3:
            # CORE order fixed: 5000,5100,5200
            bump = ivs[1] - 0.5 * (ivs[0] + ivs[2])
        dbump = bump - prev_bump
        ...
        signal = -(W_GAP * rv_iv_gap + W_STRESS * stress) + BUMP_COEF * dbump
        ...
        td["bump"] = round(bump, 6)
        td["dbump"] = round(dbump, 6)
        td["prev_bump"] = bump
