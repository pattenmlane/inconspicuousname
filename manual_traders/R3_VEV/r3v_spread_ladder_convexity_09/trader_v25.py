"""
v25: vouchers_final_strategy only (joint VEV_5200 + VEV_5300 gate, TH=2, no HYDROGEL).

- Sonic: gate is **streak-smoothed**—first tick of joint-tight is often noise; we ramp clip with
  `tight_streak` and only apply **full** v23-size flow when `tight_streak >= STREAK_FULL`
  (fewer false triggers / cleaner n per ORIGINAL_DISCORD_QUOTES context).
- inclineGod: size scales with **s_prod = s5200 * s5300** (both small => stronger co-tightness).
"""
...
BASE_CLIP_U = 20
WIDE_UNWIND = 3
# Streak: consecutive joint-tight ticks (resets on wide). Sonic: fewer false fires.
STREAK_FULL = 2
# s_prod 1->4: both=1x1=1 (max co-tight) ... both=2x2=4 (loosest still-tight)
S_PROD_COEF = 0.07
...
def _streak_mult(streak: int) -> float:
    ...
    if streak >= STREAK_FULL:
        return 1.0
    # first tight tick: partial size; at STREAK_FULL: full (2-bar persistence)
    return 0.5
