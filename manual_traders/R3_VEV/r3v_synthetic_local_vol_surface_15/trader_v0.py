...
Concept: each tick builds a smooth implied-vol slice σ̂(log K) (cubic spline on live IV knots);
every N ticks we treat as a surface "refresh" for persisted IV seeds only. Quote when
|mid − BS(S,K,T,σ̂)| exceeds hurdle × half-spread (worse fills).
"""
...
class Trader:
    def run(self, state: TradingState):
        ...
        for v, (mid, hsp, bb, ba) in mids.items():
            ...
            resid = mid - theo
            hurdle = RESID_HURDLE_MULT * max(hsp, 0.5)
            if abs(resid) < hurdle:
                continue
