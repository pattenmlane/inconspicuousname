...
Family12 baseline: smile cubic spline in logK with uniform pseudo-knot grid over observed logK range.
"""
...
KNOT_FAMILY = "adaptive_core_biased"
...
def _pseudo_knot_grid(xmin: float, xmax: float, family: str, core_density: float = 1.0) -> np.ndarray:
    if family == "uniform":
        return np.linspace(xmin, xmax, 12)
    if family == "core_biased":
    ...
    # adaptive core-biased: when core books are tighter, densify near ATM more aggressively
...
def _fit_family_spline(
    ...
    grid = _pseudo_knot_grid(xmin, xmax, family, core_density)
    try:
...
class Trader:
    def run(self, state: TradingState):
        ...
        # tighter core books -> larger core_density -> more ATM knot concentration
        mean_core_hsp = float(np.mean(core_hsp)) if core_hsp else 1.5
        core_density = 1.0 + np.clip((1.5 - mean_core_hsp) / 2.0, -0.35, 0.45)
        if len(knots_iv) >= 4:
