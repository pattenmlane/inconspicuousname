...
Family12 uniform + tape-driven shock tiering (not v22):
after |ΔS|≥THR, scale residual hurdle by strike using lead-lag co-movement:
high lag-0 corr (5000/5100) → slightly *tighter* hurdle; low lag-0 / more OTM (5400/5500) → *wider*.
Lead/lag shows same-tick response dominates; v22’s uniform beta-widen was too blunt.
"""
...
SHOCK_COOLDOWN_TICKS = 8
# From underlying_leadlag_spread_elasticity.json mean_corr_lag0 (5000–5500): high→tighten, low→widen
SHOCK_TIER_MULT = {
    5000: 0.96,
    5100: 0.97,
    5200: 1.0,
...
class Trader:
    def run(self, state: TradingState):
        ...
        prev_u = store.get("prev_u_mid")
        shock_left = int(store.get("shock_left", 0))
        ...
        if isinstance(prev_u, (int, float)) and math.isfinite(float(prev_u)):
            if abs(float(S) - float(prev_u)) >= UNDERLYING_SHOCK_THR:
                shock_left = SHOCK_COOLDOWN_TICKS
        if shock_left > 0:
            shock_left -= 1
        store["prev_u_mid"] = float(S)
        store["shock_left"] = int(shock_left)
        ...
        for v, (mid, hsp, bb, ba) in mids.items():
            ...
            if shock_left > 0:
                hurdle *= float(SHOCK_TIER_MULT.get(k, 1.0))
            if abs(resid) < hurdle:
                continue
