"""
vouchers_final_strategy — **minimal** Sonic surface: **only** the two gate books + extract.

- Joint gate: both VEV_5200 and VEV_5300 spread ≤ 2 (else flat).
- Trade **only** VELVETFRUIT_EXTRACT, VEV_5200, VEV_5300 (inclineGod: per-book spreads; gate legs
  are the ones Sonic names explicitly).

Same wall_mid / Frankfurt + passive MM as v28, smaller passive clip to limit adverse selection.
"""
...
class Trader:
    def run(self, state: TradingState):
        ...
        for sym in TRADE_SET:
            ...
            if b is None:
                continue
            for o in _mm_orders(sym, b, int(pos.get(sym, 0)), plim, MAX_CLIP):
