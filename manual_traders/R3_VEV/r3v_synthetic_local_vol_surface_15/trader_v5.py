...
Parameter sweep (parent v3): add **closed-form call gamma** to the residual hurdle:
hurdle = 1.28*half_spread + 0.001*vega + 0.08 * S * |gamma| (lighter than an initial overshoot
that produced no trades). Surface fit unchanged: all-ten-strike IV knots; trade allowed strikes only.
"""
...
# S*|gamma| is O(1) for these shorts; add a small add-on to the spread-vega hurdle.
GAMMA_HURDLE_COEF = 0.08
...
def _gamma(S: float, K: float, T: float, sig: float, r: float = 0.0) -> float:
    if T <= 0 or sig <= 1e-12:
        return 0.0
    v = sig * math.sqrt(T)
    d1 = (math.log(S / K) + (r + 0.5 * sig * sig) * T) / v
...
def _fit_iv_spline(xs: np.ndarray, ivs: np.ndarray, s: float) -> UnivariateSpline | None:
    if xs.size < 4:
        return None
    order = np.argsort(xs)
    xo = xs[order]
    ...
    if k < 1:
        return None
    try:
        return UnivariateSpline(xo, vo, k=k, s=s)
    except Exception:
        return None
...
class Trader:
    def run(self, state: TradingState):
        ...
        if len(knots_iv) >= 4:
            spl = _fit_iv_spline(np.asarray(knots_x, float), np.asarray(knots_iv, float), SPLINE_SMOOTH)
        ...
        for v, (mid, hsp, bb, ba) in mids.items():
            ...
            gam = _gamma(S, k, T, sig_hat, 0.0)
            hurdle = (
                RESID_HURDLE_MULT * max(hsp, 0.5)
                + VEGA_HURDLE_COEF * veg
                + GAMMA_HURDLE_COEF * S * abs(gam)
            )
            if abs(resid) < hurdle:
                continue
