...
Concept (v6, parent v3): same vega-weighted spread hurdle and allowed strikes; the IV slice
is still built from **all 10** strike knots, but **wing knots** (4000, 4500, 6000, 6500) get
lower weight in the smoothing spline (weighted least squares) so far strikes bias the
mid-book σ̂(x) less than in v3, without dropping data like v4.
"""
...
WING_SPLINE_WEIGHT = 0.35  # v3 uses implicit 1.0 everywhere
