"""
r3v_velvet_beta_hedge_02 — iteration 1.

Tilt VEV quotes from wall_mid using short-horizon extract return + z (same concept as v0).
Changes vs v0: anchor to observable wall_mid (reduces off-market quotes); no directional
extract taker overlay (was the main PnL drag); slightly smaller sizes; safer spread clamp.
"""
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        for v in VOUCHERS:
            ...
            spread = max(1, ba2 - bb2)
            if spread < 2:
                continue
            K = float(v.split("_")[1])
            vega = vega_only(u_mid, K, T)
            sc = min(max(vega / self.VEGA_REF, 0.15), 2.5)
            shift = tilt_px * sc
            center = wm2 + shift
            half = max(1, min(spread // 3, 6))
            bid_p = int(round(center - half))
            ask_p = int(round(center + half))
            bid_p = max(bb2 + 1, min(bid_p, ba2 - 2))
            ask_p = min(ba2 - 1, max(ask_p, bb2 + 2))
            if bid_p >= ask_p:
                bid_p, ask_p = bb2 + 1, ba2 - 1
            if bid_p >= ask_p:
                continue
            p0 = int(pos.get(v, 0))
            lim = LIMITS[v]
