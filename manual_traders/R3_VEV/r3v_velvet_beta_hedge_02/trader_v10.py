"""
r3v_velvet_beta_hedge_02 — iteration 10 (parent: v4).

Tilt and size scaled by |BS call delta| at smile IV so more ATM exposure tracks extract;
vega still caps tilt magnitude. Same TTE and inside-quote frame as v4.
"""
...
def greeks_at_smile(S: float, K: float, T: float) -> tuple[float, float]:
    sig = iv_smile(S, K, T)
    if not math.isfinite(sig) or sig <= 0:
        return 0.0, 0.0
    return bs_delta(S, K, T, sig, 0.0), bs_vega(S, K, T, sig, 0.0)
...
class Trader:
    ...
    VEV_SIZE = 19
    HYDRO_SIZE = 11
    # blend: tilt scale = (vega/VEGA_REF) * (DELTA_W + (1-DELTA_W)*|delta|)
    DELTA_W = 0.55
    SIZE_DELTA_W = 0.5
    ...
    def run(self, state: TradingState):
        ...
        for v in VOUCHERS:
            ...
            if ba2 <= bb2 + 1:
                continue
            K = float(v.split("_")[1])
            de, ve = greeks_at_smile(u_mid, K, T)
            ad = abs(float(de))
            vscale = min(max(ve / self.VEGA_REF, 0.12), 2.6)
            dsc = self.DELTA_W + (1.0 - self.DELTA_W) * ad
            sc = min(max(vscale * dsc, 0.12), 2.8)
            ...
            p0 = int(pos.get(v, 0))
            lim = LIMITS[v]
            base_sz = self.VEV_SIZE
            dsz = int(round(base_sz * (self.SIZE_DELTA_W + (1.0 - self.SIZE_DELTA_W) * ad)))
            dsz = max(6, min(28, dsz))
            qb = min(dsz, max(0, lim - p0))
            qs = min(dsz, max(0, lim + p0))
            lo: list[Order] = []
            if qb > 0:
