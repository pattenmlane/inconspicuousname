"""
r3v_velvet_beta_hedge_02 — iteration 2 (parameter grid arm A).

Same thesis; microstructure: quote one tick inside each side, then shift both prices by
integer(round(tilt * vega_scale)) so fills track extract momentum without leaving the book.
"""
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        for v in VOUCHERS:
            ...
            bb2, ba2, _wm2 = _synth(bb, ba, wm)
            if ba2 <= bb2 + 1:
                continue
            K = float(v.split("_")[1])
            sc = min(max(vega_only(u_mid, K, T) / self.VEGA_REF, 0.12), 2.8)
