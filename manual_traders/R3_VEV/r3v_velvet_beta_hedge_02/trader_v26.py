"""
r3v_velvet_beta_hedge_02 — iteration 26 (parent: v22).

Family-10 smile_quadratic_logm_wls: dynamic smile coeffs via weighted quadratic fit on live IV points.
"""
...
def _iv_from_mid(mid: float, S: float, K: float, T: float) -> float:
    intrinsic = max(S - K, 0.0)
    if mid <= intrinsic + 1e-6 or T <= 0 or S <= 0 or K <= 0:
        return float("nan")

    def f(sig: float) -> float:
        return _bs_call(S, K, T, sig) - mid
...
def _fit_smile_coeffs(state: TradingState, depths: dict[str, OrderDepth], S: float, T: float, mode: str, fallback: list[float]) -> list[float]:
    xs, ys, ws = [], [], []
    for v in VOUCHERS:
        d = depths.get(v)
        if d is None:
        ...
        K = float(v.split("_")[1])
        iv = _iv_from_mid(mid, S, K, T)
        if not (math.isfinite(iv) and 0.01 < iv < 4.0):
            continue
        spread = float(ba - bb)
        if spread <= 0:
            continue
        logm = math.log(K / S)
        w = 1.0 / spread
        if mode == "spread_vega_hybrid":
            vg = max(vega_only(S, K, T), 1e-6)
            w *= math.sqrt(vg)
        xs.append(logm)
        ys.append(iv)
    ...
    X = np.vstack([np.square(xs), xs, np.ones(len(xs))]).T
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        coeffs = _fit_smile_coeffs(state, depths, u_mid, T, "spread_vega_hybrid", _COEFFS)
        ...
        for v in VOUCHERS:
            ...
            K = float(v.split("_")[1])
            # vega from dynamically weighted smile coeffs
            m_t = math.log(K / u_mid) / math.sqrt(T) if T > 0 else 0.0
            sig_dyn = float(np.polyval(np.asarray(coeffs, dtype=float), m_t))
            vg_dyn = bs_vega(u_mid, K, T, sig_dyn, 0.0) if sig_dyn > 0 else 0.0
            sc = min(max(vg_dyn / self.VEGA_REF, 0.12), 2.6)
            sh = int(round(tilt_px * sc * sm))
            base_bid = bb2 + 1
            base_ask = ba2 - 1
