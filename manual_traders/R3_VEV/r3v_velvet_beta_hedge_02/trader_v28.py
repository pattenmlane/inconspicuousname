"""
r3v_velvet_beta_hedge_02 — iteration 28 (parent: v22).

Strike-specific tilt scale + mild lag term from beta_l1 response.
"""
...
# From analysis_v8_underlying_propagation.json
BETA0_BY_STRIKE = {4000:0.76,4500:0.68,5000:0.67,5100:0.59,5200:0.45,5300:0.28,5400:0.13,5500:0.06,6000:0.0,6500:0.0}
BETAL1_BY_STRIKE = {4000:0.09,4500:0.12,5000:0.10,5100:0.09,5200:0.07,5300:0.04,5400:0.02,5500:0.01,6000:0.0,6500:0.0}
SPREAD_SHOCK_PLUS = {4000:-0.013,4500:-0.033,5000:-0.015,5100:-0.011,5200:0.009,5300:-0.002,5400:0.0005,5500:0.0066,6000:0.0,6500:0.0}
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        prev_ret = float(store.get("prev_ret", 0.0)) if isinstance(store.get("prev_ret", 0.0), (int, float)) else 0.0
        ...
        for v in VOUCHERS:
            ...
            b0 = BETA0_BY_STRIKE.get(strike, 0.0)
            b1 = BETAL1_BY_STRIKE.get(strike, 0.0)
            ss = SPREAD_SHOCK_PLUS.get(strike, 0.0)
            resp = 0.45 + 0.75 * b0 + 0.35 * b1
            if ss > 0:
                resp *= 0.90
            sh = int(round((tilt_px * resp + 0.20 * prev_ret * b1) * sc * sm))
            base_bid = bb2 + 1
            base_ask = ba2 - 1
