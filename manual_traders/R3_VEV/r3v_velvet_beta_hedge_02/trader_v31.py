"""
r3v_velvet_beta_hedge_02 — iteration 31 (parent: v22).

Tight 5200/5300 gate: when both near-ATM books have spread<=2, boost tilt by
~10% to lean harder into the regime (analysis_v9, round3work tight-gate story).
"""
...
def _synth_spread(depth: OrderDepth) -> int | None:
    bb, ba, wm = _book(depth)
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        tight_gate = 1.0
        if self.TIGHT_5200_5300_GATE and self.G5200 in depths and self.G5300 in depths:
            s52 = _synth_spread(depths[self.G5200])
            s53 = _synth_spread(depths[self.G5300])
            if s52 is not None and s53 is not None and s52 <= 2 and s53 <= 2:
                tight_gate = self.TIGHT_GATE_MULT
        ...
        for v in VOUCHERS:
            ...
            sh = int(round(tilt_px * sc * sm * tight_gate))
            base_bid = bb2 + 1
            base_ask = ba2 - 1
