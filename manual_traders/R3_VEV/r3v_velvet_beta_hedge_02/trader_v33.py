"""
r3v_velvet_beta_hedge_02 — iteration 33 (parent: v22).

Shock guard: when |extract 1-tick return| is large (|ret1|>1.0), apply extra
10% spread-style damp to tilt (analysis_v10: V4000 |delta mid| tracks extract
shocks; avoid over-amplifying the tilt on a single big extract step).
"""
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        for v in VOUCHERS:
            ...
            sh = int(round(tilt_px * sc * sm * shock))
            base_bid = bb2 + 1
            base_ask = ba2 - 1
