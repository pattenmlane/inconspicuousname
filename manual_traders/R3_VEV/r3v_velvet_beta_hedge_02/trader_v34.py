"""
r3v_velvet_beta_hedge_02 — iteration 34 (parent: v22).

Faster path to surface moves: add a small weight on 2-tick extract return (ret2)
in the same bounded tilt blend to reduce lag vs single-tick noise.
"""
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        ret2 = (hist[-1] - hist[-3]) if len(hist) >= 3 else 0.0
        z20 = 0.0
        if len(hist) >= 5:
        ...
        tilt = (
            self.W_RET * clip(ret1 / self.TILT_RET_SCALE, -1.0, 1.0)
            + self.W_RET2 * clip(ret2 / self.TILT_RET_SCALE, -1.0, 1.0)
            + self.W_Z * clip(z20 / self.TILT_Z_SCALE, -1.0, 1.0)
        )
