"""
vouchers_final_strategy (STRATEGY.txt + ORIGINAL_DISCORD_QUOTES.txt) — v37.

Sonic: VEV "flow" is gated on book state — per-event edge when both 5200 and 5300 are tight.
inclineGod: book width (BBO spread) is the state variable for the 5200/5300 legs.

We always quote (bid/ask execution, mid plots are not PnL): one-tick inside on all VEV + extract.
When joint gate on (s_5200<=2 and s_5300<=2 on BBO), size_mult=TIGHT Mult; when off, WIDE mult < 1
(reduced participation / noise, not a full hard stop — to avoid v36 zero-fill on outer strikes).
No hydrogel. No smile/tilt/legacy stack.
"""
