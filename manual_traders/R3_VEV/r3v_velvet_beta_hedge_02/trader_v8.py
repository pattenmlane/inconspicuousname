"""
r3v_velvet_beta_hedge_02 — iteration 8.

Strike-filtered tilt with quoting at best levels (not inside) to ensure participation; hydrogel disabled.
"""
    ...
    def run(self, state: TradingState):
        ...
        for v in VOUCHERS:
            ...
            if base_bid >= base_ask:
                continue
            bid_p = max(bb2, min(base_bid + sh, ba2 - 1))
            ask_p = min(ba2, max(base_ask + sh, bb2 + 1))
            if bid_p >= ask_p:
                continue
            p0 = int(pos.get(v, 0))
            lim = LIMITS[v]
