...
"""
Family 12 v18: v15 spline + inventory-skewed fair for execution.

v15 backtest replay (v15_spline_delta_beta_scaled tradeHistory) shows many
VEV submission fills happen away from flat inventory (buckets with |pos|/300
> 10% are common), so a standard MM inventory overlay may reduce adverse
path-dependence: shift execution theo (not the spline) against inventory,
scaled by the same response metric as edge/lot (|delta| + beta proxy) so
high-sensitivity names skew more. Keeps v15 burst/calm, spline IV, and
delta/beta scaling; does not re-fit the smile on position.
"""
...
EMA_KEY = "fam12_spline_inv_skew_v18"
...
# Inventory skew: execution theo -= INV_SKEW_ALPHA * (pos/lim) * (1 + INV_RESP_BOOST * resp)
VEV_LIM = 300
# Best grid on this tape: MIN=36, moderate α (v15 remains +7181 — inventory overlay not a win here).
INV_SKEW_ALPHA = 0.40
INV_RESP_BOOST = 0.55
INV_SKEW_CAP = 1.45
MIN_ABS_POS_SKEW = 36
...
def _inv_skew_amount(pos_k: int, resp: float) -> float:
    if abs(pos_k) < MIN_ABS_POS_SKEW:
        return 0.0
    u = pos_k / float(VEV_LIM)
    u = max(-1.0, min(1.0, u))
    raw = INV_SKEW_ALPHA * u * (1.0 + INV_RESP_BOOST * resp)
    if raw > 0.0:
        return min(raw, INV_SKEW_CAP)
    return max(raw, -INV_SKEW_CAP)
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        for k, m in mid_by_k.items():
            ...
            skew = _inv_skew_amount(pos_k, resp)
            theo_exec = theo - skew
