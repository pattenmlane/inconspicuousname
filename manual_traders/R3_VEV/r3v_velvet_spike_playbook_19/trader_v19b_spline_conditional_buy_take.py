"""
Family 12 v19b: v15 spline with conditional buy-taking.

v19 (full buy-take off) was too restrictive and lost profitable participation.
This variant restores buy-taking only in burst windows on high-response strikes,
while retaining sell-taking and passive quotes everywhere.
"""
...
EMA_KEY = "fam12_spline_conditional_buy_take_v19b"
...
# Asymmetric taking controls from v15/v19 analysis.
ENABLE_BUY_TAKE = True
ENABLE_SELL_TAKE = True
BUY_TAKE_MIN_RESP_BURST = 0.70
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        for k, m in mid_by_k.items():
            ...
            allow_buy_take = ENABLE_BUY_TAKE and (burst and resp >= BUY_TAKE_MIN_RESP_BURST)
            if allow_buy_take and ba <= bid_px and buy_cap > 0:
                q = max(0, min(buy_cap, abs(int(getattr(depths[sym], "sell_orders", {}).get(ba, 0)))))
                if q > 0:
                    orders.append(Order(sym, int(ba), int(q)))
                    buy_cap -= q
            if ENABLE_SELL_TAKE and bb >= ask_px and sell_cap > 0:
