...
"""
Family 12 v21: v15 smile MM + VELVETFRUIT_EXTRACT, gated on joint VEV book tightness.

Shared thesis (round3work/vouchers_final_strategy/STRATEGY.txt): when VEV_5200 and
VEV_5300 both have L1 spread <= TH (2 ticks) at the same time, short-horizon extract
dynamics are more favorable; when either is wide, treat as a different (risk-off)
regime. Here we *only* run v15 VEV + a simple extract mean-reversion when the joint
gate is on; when off, we stand down (no HYDROGEL).
"""
...
# Extract (only when joint gate is true)
EXTRACT_MID_WINDOW = 50
EXTRACT_EDGE = 1
EXTRACT_LIM = 200
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        ex_hist = store.get("ex_hist")
        if not isinstance(ex_hist, list):
            ex_hist = []
        ex_hist = [float(x) for x in ex_hist if isinstance(x, (int, float))][-EXTRACT_MID_WINDOW:]
        ...
        ex_hist.append(S)
        ex_hist = ex_hist[-EXTRACT_MID_WINDOW:]
        ...
        # VELVETFRUIT_EXTRACT: mean reversion in joint-tight regime
        if len(ex_hist) >= EXTRACT_MID_WINDOW:
            mean = sum(ex_hist) / float(len(ex_hist))
            pos_e = int(pos.get(sym_u, 0))
            d_e = depths[sym_u]
            b_b = max((getattr(d_e, "buy_orders", {}) or {}).keys(), default=None)
            b_a = min((getattr(d_e, "sell_orders", {}) or {}).keys(), default=None)
            if b_b is not None and b_a is not None:
                e_orders: list[Order] = []
                max_b = EXTRACT_LIM - pos_e
                max_s = EXTRACT_LIM + pos_e
                if b_a < mean - float(EXTRACT_EDGE) and max_b > 0:
                    q0 = min(abs(int((getattr(d_e, "sell_orders", {}) or {}).get(b_a, 0))), max_b)
                    if q0 > 0:
                        e_orders.append(Order(sym_u, int(b_a), int(q0)))
                if b_b > mean + float(EXTRACT_EDGE) and max_s > 0:
                    q0 = min(abs(int((getattr(d_e, "buy_orders", {}) or {}).get(b_b, 0))), max_s)
                    if q0 > 0:
                        e_orders.append(Order(sym_u, int(b_b), -int(q0)))
                if e_orders:
                    out[sym_u] = (out.get(sym_u) or []) + e_orders
