"""
vouchers_final_strategy — v25 (tune v24): same Sonic hard gate; softer inclineGod quality.

Spread imbalance |s5200-s5300| in the tight box is usually 0 (see r3_spread_imbalance_by_day.json);
when it is 2, downweight only slightly. No extract gating on imbalance (v24 was too strict on
tapes where imb==2 is common in-box).
"""
...
def _quality(s5200: int, s5300: int, s_ext: int) -> tuple[float, float]:
    ...
    q_leg = max(0.7, 1.0 - 0.12 * imb)
    q_ext = max(0.75, 1.0 - 0.2 * (float(s_ext) / float(max(MAX_EXTRACT_SPREAD, 1))))
    q = max(0.45, min(1.0, q_leg * q_ext))
    return imb, q
