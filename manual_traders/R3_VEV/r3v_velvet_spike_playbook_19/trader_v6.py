"""
Velvet spike playbook v6: v5 + targeted wing control in bursts.

- Burst cluster slightly tighter (ATM_REL_BURST=0.22): deep-ITM VEV_4000 sits just
  outside |K/S-1|<=0.22 on tape (see analysis), so burst quotes focus on ATM ring
  without that wing (widen-on-spike thesis, less global-smile misfit).
- Per-order size cap in burst only to limit full-depth adverse fills in volatile regime.
- Same calm + low-z flattening as v5.
"""
