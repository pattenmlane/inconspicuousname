"""
Velvet spike playbook v7: v5 + burst-only per-order size cap.

- Same thresholds and ATM_REL_BURST=0.24 as v5 (keeps VEV_4000 in cluster when S~5200).
- Caps each Frankfurt scalp order at BURST_MAX_LOT in burst regime to reduce full-size
  adverse fills under --match-trades worse.
- Calm and low-z behavior unchanged from v5.
"""
