"""
Velvet spike playbook v8: v5 + two-tier burst size caps (core vs ring).

- Same wide burst cluster (ATM_REL_BURST=0.24) and calm 0.10 as v5/v7.
- In burst: 80 lots per order on core (|K/S-1|<=0.10), 40 on the outer burst ring
  (0.10 < |K/S-1| <= 0.24). Ring strikes have much lower BS vega on tape in bursts
  (see analysis_outputs/burst_tier_greeks.json); cap tighter there to "pull wings"
  in size while still quoting.
- Calm and low-z behavior match v5.
"""
...
def _burst_lot_for_strike(S: float, K: int) -> int:
    rel = abs(float(K) / S - 1.0) if S > 0 else 0.0
    if rel <= float(ATM_REL_CALM):
        return int(BURST_LOT_CORE)
    return int(BURST_LOT_RING)
        ...
        for strike in STRIKES:
            ...
            if very_calm:
                ...
                if pos_sym > 0:
                    ...
                    bids = []
                    asks = []
            lot_cap = _burst_lot_for_strike(S, strike) if burst else 10_000
            ol: list[Order] = []
            for px, q in asks:
