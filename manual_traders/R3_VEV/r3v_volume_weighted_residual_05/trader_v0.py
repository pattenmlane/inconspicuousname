"""
r3v_volume_weighted_residual_05 — Fade IV minus volume-weighted WLS smile residual.

Thesis: after fitting a quadratic IV smile in m_t = log(K/S)/sqrt(T), per-strike
residuals that are high vs the smile tend to mean-revert (fade rich IV).

Tape timing (round3work/round3description.txt + intraday wind from
plot_iv_smile_round3): CSV day 0→8d at open, 1→7d, 2→6d;
dte_eff = dte_open - (timestamp//100)/10000; T = dte_eff/365; r=0.

Backtester note: traderData is reset each day. We infer csv_day (0/1/2) from
VELVETFRUIT_EXTRACT mid at session open (distinct anchors on R3 historical tapes).
"""
...
# R3 historical tapes: extract mid at timestamp 0 (see analysis.json).
...
def fit_smile_residuals(
    ...
    """Return strike -> (iv, residual, weight) for each voucher with valid IV."""
    if S <= 0 or T <= 0:
        return {}
    sqrtT = math.sqrt(T)
    xs: list[float] = []
...
class Trader:
    def run(self, state: TradingState):
        ...
        orders: dict[str, list[Order]] = {p: [] for p in [H, U] + VOU}
        ...
        for sym in VOU:
            ...
            q = min(MAX_Q, max(1, int(25.0 / max(veg, 1e-6))))
            q = min(q, lim - pos, lim + pos)

            if resid > RESID_ENTER and pos > -lim + q:
                price = bb
                orders[sym].append(Order(sym, price, -q))
        ...
        # Hydrogel: simple spread capture (allowed product; uncorrelated hedge-lite).
        dh = state.order_depths.get(H)
        if dh is not None:
            pos_h = state.position.get(H, 0)
            bbh, bah = _best_bid_ask(dh)
            if bbh is not None and bah is not None and bah - bbh >= 2 * HYDRO_EDGE:
