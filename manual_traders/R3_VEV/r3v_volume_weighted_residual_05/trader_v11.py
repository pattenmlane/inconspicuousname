"""
r3v_volume_weighted_residual_05 v11 — Parent: v8 (best PnL so far).

Thesis unchanged: fade IV minus **volume-weighted WLS** smile residual
(m_t = log(K/S)/sqrt(T)) on VEV_5100..VEV_5500.

v11 sweep (vs v8): keep voucher logic the same, but add hydrogel risk wind-down:
- disable new HYDROGEL quoting after timestamp >= 800000
- flatten residual hydrogel position in final stretch (timestamp >= 900000)
This keeps the options thesis intact while reducing day-end hydrogel drag that has
appeared on day 2 in recent runs.
"""
...
HYDRO_STOP_NEW_TS = 800000
HYDRO_FLATTEN_TS = 900000
    ...
    def run(self, state: TradingState):
        ...
        if dh is not None:
            ...
            if bbh is not None and bah is not None:
                if ts >= HYDRO_FLATTEN_TS:
                    if pos_h > 0:
                        orders[H].append(Order(H, bbh, -min(abs(pos_h), HYDRO_Q)))
                    elif pos_h < 0:
                        orders[H].append(Order(H, bah, min(abs(pos_h), HYDRO_Q)))
                elif ts < HYDRO_STOP_NEW_TS and (bah - bbh >= HYDRO_MIN_SPREAD):
