"""
r3v_volume_weighted_residual_05 v12 — Parent: v8 (best PnL in branch).

Thesis: fade IV minus **volume-weighted WLS** smile residual (same as v8).

v12 grid (vs v8, motivated by high lag-1 persistence in raw residuals on tape):
- Keep EMA(α=0.12) on raw residual; exits still on **raw** resid vs RESID_EXIT.
- **Innovation / momentum gate** on *new* entries: only fade in the direction the
  smoothed mispricing is still **building** (EMA step vs previous EMA) by at least
  INNOV_MIN, so we avoid fading noise that is already mean-reverting within a tick.
- First EMA update per strike: no innovation gate (same as v8 entry behavior once
  EMA and liquidity gates pass).
"""
...
def _update_res_ema(store: dict[str, Any], sym: str, raw_resid: float) -> tuple[float, float, float | None]:
    """Return (new_ema, innovation vs previous EMA, previous EMA or None)."""
    ...
    if prev is not None and isinstance(prev, (int, float)) and math.isfinite(float(prev)):
        prev_f = float(prev)
        e = EMA_ALPHA * raw_resid + (1.0 - EMA_ALPHA) * prev_f
    else:
        e = raw_resid
    innov = (e - prev_f) if prev_f is not None else 0.0
    ema_map[key] = e
    store[_EMA_RES_KEY] = ema_map
    return e, innov, prev_f
...
class Trader:
    def run(self, state: TradingState):
        ...
        for sym in TRADE_VOU:
            ...
            res_ema, innov, prev_ema = _update_res_ema(store, sym, resid)
            bb, ba = _best_bid_ask(d)
            if bb is None or ba is None or ba - bb > MAX_SPREAD:
                continue
            K = float(sym.split("_")[1])
            ...
            if abs(resid) < RESID_EXIT and abs(pos) > 0:
                ...
                build_short = res_ema > RESID_ENTER and (prev_ema is None or innov > INNOV_MIN)
                build_long = res_ema < -RESID_ENTER and (prev_ema is None or innov < -INNOV_MIN)
                if build_short and pos > -lim + q and top_sum >= MIN_TOP_SUM_VOL:
                    orders[sym].append(Order(sym, bb, -q))
                elif build_long and pos < lim - q and top_sum >= MIN_TOP_SUM_VOL:
