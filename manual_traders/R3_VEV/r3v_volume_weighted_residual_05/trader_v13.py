"""
r3v_volume_weighted_residual_05 v13 — family10 smile_quadratic_logm_wls variation:
refit cadence (FAST baseline).

Same thesis and guardrails as v8. Only model-maintenance change:
  * FAST cadence: refit WLS smile every tick (equivalent to v8 behavior).
"""
...
_COEF_KEY = "wls_coef"
_LAST_REFIT_KEY = "last_refit_ts"
_REFIT_MODE = "fast"
_REFIT_EVERY = 100  # used in slow mode
_REFIT_MOVE = 3.5  # used in conditional mode, extract mid move threshold
...
def residuals_from_coeffs(
    S: float, T: float, depths: dict[str, OrderDepth], coeffs: tuple[float, float, float]
) -> dict[str, tuple[float, float, float]]:
    out: dict[str, tuple[float, float, float]] = {}
    if S <= 0 or T <= 0:
        return out
    ...
    for sym in TRADE_VOU:
        ...
        w = float(max(bv + av + 1, 1))
        m_t = math.log(K / S) / sqrtT
        pred = c0 * m_t * m_t + c1 * m_t + c2
        out[sym] = (iv, float(iv - pred), w)
    return out


def _load_coeffs(store: dict[str, Any]) -> tuple[float, float, float] | None:
    raw = store.get(_COEF_KEY)
    if isinstance(raw, list) and len(raw) == 3:
        try:
            c = tuple(float(x) for x in raw)
            if all(math.isfinite(v) for v in c):
...
def _store_coeffs(store: dict[str, Any], coeffs: tuple[float, float, float]) -> None:
    store[_COEF_KEY] = [float(coeffs[0]), float(coeffs[1]), float(coeffs[2])]
...
class Trader:
    def run(self, state: TradingState):
        ...
        coeffs = _load_coeffs(store)
        last_refit = int(store.get(_LAST_REFIT_KEY, -10**9))
        prev_s = float(store.get("prev_extract_mid", S))
        # FAST cadence: always refit; this is the baseline family-10 setting.
        should_refit = True
        if _REFIT_MODE == "slow":
            should_refit = (state.timestamp - last_refit) >= _REFIT_EVERY
        elif _REFIT_MODE == "conditional":
            should_refit = abs(S - prev_s) >= _REFIT_MOVE
        if should_refit or coeffs is None:
            new_coeffs = fit_smile_coeffs(S, T, state.order_depths)
            if new_coeffs is not None:
                coeffs = new_coeffs
                _store_coeffs(store, coeffs)
                store[_LAST_REFIT_KEY] = int(state.timestamp)
        store["prev_extract_mid"] = float(S)
        residuals = residuals_from_coeffs(S, T, state.order_depths, coeffs) if coeffs is not None else {}
