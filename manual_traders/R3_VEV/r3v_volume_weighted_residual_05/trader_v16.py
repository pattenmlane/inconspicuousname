"""
r3v_volume_weighted_residual_05 v16 — family 10 variant (non-duplicate).

Core family stays the same: smile_quadratic_logm_wls residual fade.

Distinct variation vs prior cadence-only tests:
- Refit schedule = cadence + event trigger:
  * regular refit every REFIT_CADENCE ticks
  * forced refit on large underlying moves (|dS| >= DS_EVENT_TRIGGER)
  * forced refit if fit is stale beyond REFIT_STALE_MAX
- This changes assumptions from time-only cadence to event-responsive surface updates.
"""
...
_FIT_COEF_KEY = "fit_coef_vev_strip"
_FIT_TS_KEY = "fit_last_ts"
_LAST_S_KEY = "last_underlying_mid"
...
def _read_last_fit(store: dict[str, Any]) -> tuple[list[float] | None, int | None]:
    c = store.get(_FIT_COEF_KEY)
    ts = store.get(_FIT_TS_KEY)
    if isinstance(c, list) and len(c) == 3 and all(isinstance(x, (int, float)) for x in c):
        c_out = [float(c[0]), float(c[1]), float(c[2])]
    else:
        c_out = None
    ts_out = int(ts) if isinstance(ts, (int, float)) else None
    return c_out, ts_out


def _write_last_fit(store: dict[str, Any], coef: list[float], ts: int) -> None:
    store[_FIT_COEF_KEY] = [float(coef[0]), float(coef[1]), float(coef[2])]
    store[_FIT_TS_KEY] = int(ts)


def _calc_raw_inputs(
    S: float, T: float, depths: dict[str, OrderDepth]
) -> list[tuple[str, float, float, float, float, int, int]]:
    """
    Build per-symbol rows:
      (sym, iv, m_t, weight, vega, best_bid, best_ask)
    """
    if S <= 0 or T <= 0:
        return []
    sqrtT = math.sqrt(T)
    ...
    for sym in TRADE_VOU:
        ...
        m_t = math.log(K / S) / sqrtT
        vg = vega(S, K, T, iv)
        rows.append((sym, iv, m_t, w, vg, int(bb), int(ba)))
    return rows


def _fit_coef_from_rows(rows: list[tuple[str, float, float, float, float, int, int]]) -> list[float] | None:
    if len(rows) < 5:
        return None
    xs = np.array([r[2] for r in rows], dtype=float)
    ys = np.array([r[1] for r in rows], dtype=float)
    ws = np.array([r[3] for r in rows], dtype=float)
    X = np.c_[xs ** 2, xs, np.ones(len(xs))]
    wsqrt = np.sqrt(ws)
    coef, *_ = np.linalg.lstsq(X * wsqrt[:, None], ys * wsqrt, rcond=None)
