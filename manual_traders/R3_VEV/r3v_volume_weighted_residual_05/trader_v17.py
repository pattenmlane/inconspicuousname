"""
r3v_volume_weighted_residual_05 v17 — analysis-driven propagation tweak.

Thesis unchanged: fade IV minus volume-weighted WLS smile residual.

New assumption from tape analysis (underlying_to_voucher_response.json):
- VEV_5200..VEV_5400 show strongest and most stable immediate response to underlying
  moves with tight calm spreads.
- After |dS| shocks (~2+ ticks per 100ms), spreads tend to worsen; avoid opening
  fresh voucher risk right on shock ticks.

Implementation delta vs v8:
- Trade/fitting strip narrowed to VEV_5200..VEV_5400.
- Add shock-aware entry gate (abs(dS) <= SHOCK_SKIP_ABS_DS for new entries).
- Keep same risk/execution guardrails otherwise.
"""
...
TRADE_VOU = [f"VEV_{k}" for k in (5200, 5300, 5400)]
    ...
    def run(self, state: TradingState):
        ...
        for sym in TRADE_VOU:
            ...
            if abs(resid) < RESID_EXIT and abs(pos) > 0:
                ...
                res_ema > RESID_ENTER
                and pos > -lim + q
                and top_sum >= MIN_TOP_SUM_VOL
                and abs_ds <= SHOCK_SKIP_ABS_DS
            ):
