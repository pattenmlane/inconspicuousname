"""
r3v_volume_weighted_residual_05 v18 — Parent: v8 (best PnL on this branch).

Same thesis: fade IV minus **volume-weighted WLS** smile residual in
m_t=log(K/S)/sqrt(T); trade strip VEV_5100..VEV_5500; hydrogel passive; same
guardrails as v8.

v18 (analysis-driven, not a random nudge): contract **size multipliers** from
underlying→voucher response scores in
`analysis_outputs/underlying_to_voucher_response.json`, mapped in
`analysis_outputs/response_score_to_size_mult.json`. High response + tight calm
spread (5200/5300/5400) gets up to +15% size; weaker 5500 gets -15%, per tape
ranking.
"""
...
# From response_score_to_size_mult.json (5100–5500 strip)
_SIZE_MULT = {
    "VEV_5200": 1.15,
    "VEV_5300": 1.09,
    "VEV_5400": 1.07,
...
class Trader:
    def run(self, state: TradingState):
        ...
        for sym in TRADE_VOU:
            ...
            K = float(sym.split("_")[1])
            veg = vega(S, K, T, iv)
            sm = _SIZE_MULT.get(sym, 1.0)
            q = min(MAX_Q, max(1, int((20.0 * sm) / max(veg, 1e-6))))
