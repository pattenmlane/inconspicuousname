"""
r3v_volume_weighted_residual_05 v20 — parent v8 + 5400 stress passivation (analysis-grounded, behavior-changing).
...
def _entry_sell_price(bb: int, ba: int, stress: bool, sym: str) -> int:
    if stress and sym == SYM_5400 and ba - bb > 1:
        return min(bb + 1, ba - 1)
    return bb


def _entry_buy_price(bb: int, ba: int, stress: bool, sym: str) -> int:
    if stress and sym == SYM_5400 and ba - bb > 1:
...
class Trader:
    def run(self, state: TradingState):
        ...
        if isinstance(prev, (int, float)) and math.isfinite(float(prev)):
            stress = abs(S - float(prev)) >= SHOCK_ABS_DS
        else:
            stress = False
        store[_LAST_S_KEY] = S
        ...
        for sym in TRADE_VOU:
            ...
            if abs(resid) < RESID_EXIT and abs(pos) > 0:
                ...
                p = _entry_sell_price(bb, ba, stress, sym)
                orders[sym].append(Order(sym, p, -q))
            elif res_ema < -RESID_ENTER and pos < lim - q and top_sum >= MIN_TOP_SUM_VOL:
                p = _entry_buy_price(bb, ba, stress, sym)
                orders[sym].append(Order(sym, p, q))
