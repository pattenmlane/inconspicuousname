"""
r3v_volume_weighted_residual_05 v21 — asymmetric 5400 stress quoting by shock direction.

In v8 backtests, **our** `SUBMISSION` fills (see tradeHistory) are concentrated on
**HYDROGEL_PACK** and **VEV_5400**; other strip names are mostly other participants in the
tape, so gating 5500 does nothing for us.

`underlying_to_voucher_response.json` ranks **VEV_5400** high on dV~dS vs extract but still
has microstructure under shocks (|dS|>=2 ticks, same scale as the analysis). On those ticks
we keep the same EMA/liquidity/residual gates, but nudge the working **limit** for new 5400
entries one level **toward the mid** (less eager to print at the touch): sell at
min(bb+1, ba-1) instead of bb; buy at max(ba-1, bb+1) instead of ba. Flatten path unchanged
(RESID_EXIT) still uses bid/ask touch for urgency.
"""
...
def _entry_sell_price(bb: int, ba: int, dS: float, stress: bool, sym: str) -> int:
    # Asymmetry from v21 analysis: under stress-up (dS>0), residual->next idio slope is stronger
    # for VEV_5400, so keep short entry at touch; otherwise remain one-tick passive.
...
def _entry_buy_price(bb: int, ba: int, dS: float, stress: bool, sym: str) -> int:
    # Long entries in stress stay passive (weaker stress-down predictive slope than stress-up).
    if stress and sym == SYM_5400 and ba - bb > 1:
