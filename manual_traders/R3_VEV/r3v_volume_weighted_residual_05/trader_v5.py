"""
r3v_volume_weighted_residual_05 v5 — Parent: v3 (best PnL).

Thesis: fade IV minus **volume-weighted WLS** smile residual.

v5 change (grid from v3 after iteration 3 analysis):
- **Fit** the smile on every traded voucher that passes the spread filter (all ten
  VEV_*), so the surface anchor uses the full chain (richer m_t cross-section).
- **Act** on the same liquid strip as v3: VEV_5000..VEV_5500 only.
- Slight threshold tweak: easier exit so positions do not get stuck
  (RESID_EXIT=0.005 vs 0.004), RESID_ENTER=0.0135.
- Tighter top-of-book spread for **entries** (<=6) on the trade strip; exit still
  allows up to MAX_SPREAD=8 for flattening.
"""
...
TRADE_VOU = [f"VEV_{k}" for k in (5000, 5100, 5200, 5300, 5400, 5500)]
FIT_VOU = VOU
...
def fit_smile_residuals(
    ...
    """WLS fit on FIT_VOU; return residuals for symbols that were used in the fit."""
    if S <= 0 or T <= 0:
        return {}
    sqrtT = math.sqrt(T)
    xs: list[float] = []
...
class Trader:
    def run(self, state: TradingState):
        ...
        for sym in TRADE_VOU:
            ...
            strict_spread = spr <= MAX_SPREAD_ENTRY

            if resid > RESID_ENTER and pos > -lim + q and strict_spread:
                orders[sym].append(Order(sym, bb, -q))
            elif resid < -RESID_ENTER and pos < lim - q and strict_spread:
