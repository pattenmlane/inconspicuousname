"""
r3v_volume_weighted_residual_05 v6 — Parent: v3 (best total PnL so far).

Same thesis: fade IV minus volume-weighted WLS smile residual in
m_t=log(K/S)/sqrt(T), weights = 1 + top-of-book volume.

v6 vs v3 (grid / post v5): VEV_5000 was a repeated large loss in broad-strip tests;
  **do not trade** 5000. **Fit** the smile on all ten VEVs (so m_t support stays
  stable) but **act** only on VEV_5100..VEV_5500. Hydrogel passive leg unchanged
  from v3.
"""
