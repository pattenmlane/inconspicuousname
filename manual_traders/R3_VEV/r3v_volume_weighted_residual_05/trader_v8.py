"""
r3v_volume_weighted_residual_05 v8 — Parents: v7 (best PnL) + Frankfurt-style EMA (see Prosperity3 Frankfurt mean-reversion windows).

Thesis unchanged: fade IV minus **volume-weighted WLS** smile residual
(m_t = log(K/S)/sqrt(T)); trade strip VEV_5100..VEV_5500 only; hydrogel passive.

v8 grid (vs v7):
- EMA smooth per-strike **raw** residual in traderData (α=0.12) to reduce one-tick
  noise when deciding new entries; exits still use **raw** residual for faster flatten.
- **Volume conviction** gate: new entries only if top-of-book size (bid+ask) ≥ 28
  (aligns with WLS weight = 1+volume; thin books skipped).
- Slightly stricter **RESID_ENTER**=0.0145 on the EMA signal, **RESID_EXIT**=0.0045
  on raw.
"""
