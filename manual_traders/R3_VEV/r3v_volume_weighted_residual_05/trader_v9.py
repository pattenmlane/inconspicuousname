"""
r3v_volume_weighted_residual_05 v9 — Parent: v8 (current best PnL).

Thesis: fade IV minus **vega × volume** weighted WLS smile residual.

v9 (grid vs v8):
- WLS loss weights: (1 + book_sum)^1.25 * (1 + 0.002*vega) so high-vega ATM points
  and liquid books anchor the smile more (matches strategy name: volume + convex
  risk weight).
- EMA on residual α=0.10 (slightly smoother entries than v8).
- RESID_ENTER=0.014, RESID_EXIT=0.004, min book sum 24 for new entries.
"""
...
# WLS: combine book depth with vega (convexity) — strategy_concept: volume * Greek weighting
WLS_VOL_EXP = 1.25
WLS_VEGA_SCALE = 0.002
...
def fit_smile_residuals(
    ...
    for sym in TRADE_VOU:
        ...
        book = float(max(bv + av + 1, 1))
        veg = vega(S, K, T, iv)
        w = (book**WLS_VOL_EXP) * (1.0 + WLS_VEGA_SCALE * max(veg, 0.0))
