"""v11: v10 + extract-shock asymmetry on VEV passive offsets (up-shock wider, down-shock tighter).

Distinct from parent family-13 exploration:
- Parent emphasized smile-theo mispricing takes and optional theta/gamma scaling.
- Here: minimal RV-IV regime only, but calm aggressions require L1 imbalance confirmation;
  stressed regime disables hydro participation (risk-off) and widens passive quotes.

Core wide-book passive VEV thesis remains.
"""
...
# Regime + execution controls (sweep these)
RV_ALPHA = 0.06
IV_ALPHA = 0.06
...
IMBALANCE_GATE_EXTRACT = 0.20
IMBALANCE_GATE_HYDRO = 0.25
SHOCK_MOVE_TICKS = 4.0
UP_SHOCK_EXTRA_OFF = 1
DOWN_SHOCK_REDUCE_OFF = 1
...
def _imbalance(od: OrderDepth) -> float:
    ...
    if bb is None or ba is None:
        return 0.0
    bv = abs(int(od.buy_orders.get(bb, 0)))
    av = abs(int(od.sell_orders.get(ba, 0)))
    den = bv + av
    if den <= 0:
        return 0.0
    return (bv - av) / den
...
class Trader:
    def run(self, state: TradingState):
        ...
        if mid_h is not None:
            h_ema = mid_h if h_ema is None else (1.0 - HYDRO_EMA_ALPHA) * float(h_ema) + HYDRO_EMA_ALPHA * mid_h
        data["h_ema"] = h_ema

        prev_mid_u = data.get("prev_mid_u")
        rv_ema = float(data.get("rv_ema", 0.0))
        last_move_ticks = 0.0
        if mid_u is not None and prev_mid_u and prev_mid_u > 0:
            ret = math.log(mid_u / float(prev_mid_u))
            rv_ema = (1.0 - RV_ALPHA) * rv_ema + RV_ALPHA * (ret * ret)
            last_move_ticks = mid_u - float(prev_mid_u)
        ...
        if mid_u is not None:
            for sym in TARGET_VEV:
                ...
                if m is None:
                    continue
                iv = bs_call_iv(mid_u, STRIKE[sym], T, m)
                if iv is None:
                    vev_delta[sym] = 0.0
                    vev_vega[sym] = 0.0
                    continue
                iv_vals.append(iv)
                vev_delta[sym] = bs_call_delta(mid_u, STRIKE[sym], T, iv)
                vev_vega[sym] = bs_call_vega(mid_u, STRIKE[sym], T, iv)
        ...
        data["rv_ema"] = rv_ema
        data["iv_ema"] = iv_ema
        rv_iv_diff = math.sqrt(max(rv_ema, 0.0)) - float(iv_ema)
        ...
        for sym in TARGET_VEV:
            ...
            lim = LIMITS[sym]
            shock_adj = 0
            if last_move_ticks >= SHOCK_MOVE_TICKS:
                shock_adj = UP_SHOCK_EXTRA_OFF
            elif last_move_ticks <= -SHOCK_MOVE_TICKS:
                shock_adj = -DOWN_SHOCK_REDUCE_OFF
            off = max(1, DEEP_OFFSET[sym] + (1 if stressed else 0) + shock_adj)
            bid_px = bb + off
            ask_px = ba - off
            if bid_px >= ask_px:
                continue
            q = PASSIVE_SIZE if not stressed else max(1, PASSIVE_SIZE - 1)
            if lim - pos > 0:
                orders[sym].append(Order(sym, int(bid_px), min(q, lim - pos)))
            if lim + pos > 0:
                orders[sym].append(Order(sym, int(ask_px), -min(q, lim + pos)))
        ...
        if od_u is not None and e_ema is not None:
            ...
            if bb is not None and ba is not None:
                ...
                if not stressed:
                    if buy_room > 0 and (fair - ba) >= CALM_AGGR_EDGE_EXTRACT and imb >= IMBALANCE_GATE_EXTRACT:
                        orders[UNDER].append(Order(UNDER, ba, min(CALM_AGGR_SIZE_EXTRACT, buy_room)))
                    if sell_room > 0 and (bb - fair) >= CALM_AGGR_EDGE_EXTRACT and imb <= -IMBALANCE_GATE_EXTRACT:
                        orders[UNDER].append(Order(UNDER, bb, -min(CALM_AGGR_SIZE_EXTRACT, sell_room)))

        # Distinct risk-off behavior: in stressed regime, skip hydro entirely.
        if not stressed and od_h is not None and h_ema is not None:
            sp = _spread(od_h)
            if sp is not None and sp >= HYDRO_WIDE_MIN:
                ...
                if bb is not None and ba is not None:
                    ...
                    if buy_room > 0 and (fair - ba) >= CALM_AGGR_EDGE_HYDRO and imb >= IMBALANCE_GATE_HYDRO:
                        orders[HYDRO].append(Order(HYDRO, ba, min(CALM_AGGR_SIZE_HYDRO, buy_room)))
                    if sell_room > 0 and (bb - fair) >= CALM_AGGR_EDGE_HYDRO and imb <= -IMBALANCE_GATE_HYDRO:
