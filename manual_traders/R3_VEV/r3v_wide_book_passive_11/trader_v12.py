"""v12: v8 (wide-book deep passive) + tape-grounded Greek/IV and skew-conditioned depth.

Analysis (analysis_outputs_v11_lead_lag_skew.json):
- Same-timestamp extract vs VEV return corr is 0; lag-1+ shows strong comovement — use
  EMA-smoothed extract as S* for BS IV and greeks to reduce look-ahead.
- IV(4000)-IV(4500) skew correlates ~-0.79 with mean L1 spread: high skew <-> tighter
  book — add +1 to passive depth when EMA IV skew is high.
- TTE proxy for BS: 6d/365y in-file (trader has no DTE; matches prior iterations).

Swept: IV_SKEW_HIGH, S_IV_EMA blend (see sweep_iter12_skew_depth.json).
"""
...
# Smoothed spot for IV/greeks (addresses lag-0=0 comovement artifact on tape)
S_IV_EMA = 0.30
# EMA of IV(4000)-IV(4500) skew
IV_SKEW_EMA_ALPHA = 0.10
IV_SKEW_HIGH = 0.34
    ...
    def run(self, state: TradingState):
        ...
        if mid_u is not None:
            if e_iv is None:
                ...
                e_iv = (1.0 - S_IV_EMA) * float(e_iv) + S_IV_EMA * mid_u
        data["e_iv"] = e_iv
        ...
        S_star = float(e_iv) if e_iv is not None else (float(mid_u) if mid_u is not None else 0.0)

        sk_ema = data.get("sk_ema")
        iv4_now: Optional[float] = None
        iv45_now: Optional[float] = None
        for sym in TARGET_VEV:
            odv = state.order_depths.get(sym)
            if odv is None or S_star <= 0:
                continue
            m = _mid(odv)
            if m is None:
                continue
            iv = bs_call_iv(S_star, STRIKE[sym], T, m)
            if sym == "VEV_4000" and iv is not None:
                iv4_now = iv
            elif sym == "VEV_4500" and iv is not None:
                iv45_now = iv
        if iv4_now is not None and iv45_now is not None:
            sk_inst = iv4_now - iv45_now
            if sk_ema is None:
                sk_ema = sk_inst
            else:
                sk_ema = (1.0 - IV_SKEW_EMA_ALPHA) * float(sk_ema) + IV_SKEW_EMA_ALPHA * sk_inst
        data["sk_ema"] = sk_ema

        vev_delta: Dict[str, float] = {}
        vev_vega: Dict[str, float] = {}
        if S_star > 0 and mid_u is not None:
            for sym in TARGET_VEV:
                odv = state.order_depths.get(sym)
                if odv is None:
                    continue
                ...
                if m is None:
                    continue
                K = STRIKE[sym]
                iv = bs_call_iv(S_star, K, T, m)
                if iv is not None:
                    vev_delta[sym] = bs_call_delta(S_star, K, T, iv)
                    vev_vega[sym] = bs_call_vega(S_star, K, T, iv)
                else:
                    vev_delta[sym] = 0.0
                    vev_vega[sym] = 0.0
        ...
        depth_extra = 0
        if sk_ema is not None and sk_ema > IV_SKEW_HIGH:
            depth_extra = 1
        ...
        for sym in TARGET_VEV:
            ...
            pos = _pos(state, sym)
            lim = LIMITS[sym]
            off = max(1, DEEP_OFFSET[sym] + depth_extra)
            bid_px = bb + off
            ask_px = ba - off
            if bid_px >= ask_px:
                continue
