"""v15: v14 + strike-specific shock beta center shifts (4000 > 4500 from tape).

v7: v5 + focused sweep — WIDE_SPREAD_MIN=15, deep passive 4000/4500 offsets 4/5, PASSIVE_SIZE=3; extract delta+vega skew.

Tape: HYDROGEL_PACK L1 spread mean ~15.7–15.74 and median 16 across days 0–2 (analyze_tapes.py → tape_vev4000_4500_stats.json hydrogel_pack). Backtest grid: {14,15}×{1,2,3} and 16–17×sizes; best merged PnL at WIDE_MIN 14–15 with size 3 (36,500); use 15 to match VEV spread gate.
"""
...
# Hedge extract penny quotes: delta exposure + vega from held options (v4 used delta only).
# Grid (sweep_iter5_vega_hedge.json): best total 29,147 with DELTA_SKEW=0.3; VEGA_SKEW 0–0.001
# tied on day 0–1; use small positive vega weight for non-zero Greek hedge.
DELTA_SKEW = 0.3
VEGA_SKEW = 0.0005
SKEW_CAP = 8

# Extract shock -> next-tick VEV drift proxy (analysis_outputs_v12_shock_response.json):
# beta_dV_per_dS ~ 0.54-0.70 on |dS|>=3; shift quote center by recent extract move.
VEV_CENTER_SHIFT_BETA = {"VEV_4000": 0.58, "VEV_4500": 0.52}
VEV_SHOCK_MIN = 4.0
# Next-tick mean VEV spread change after |dS|>=4 is slightly negative (compression ~-0.075
# ticks vs near-zero on quiet; analysis iteration 13). Tighten passive depth 1 tick on shock.
VEV_SHOCK_TIGHTEN_OFF = 1
...
class Trader:
    def run(self, state: TradingState):
        ...
        extract_move = 0.0
        if mid_u is not None and prev_mid_u is not None:
            extract_move = float(mid_u) - float(prev_mid_u)
        ...
        for sym in TARGET_VEV:
            ...
            pos = _pos(state, sym)
            lim = LIMITS[sym]
            off0 = DEEP_OFFSET[sym]
            center_shift = 0
            shock = abs(extract_move) >= VEV_SHOCK_MIN
            if shock:
                center_shift = int(round(extract_move * VEV_CENTER_SHIFT_BETA.get(sym, 0.55)))
            off = max(1, off0 - (VEV_SHOCK_TIGHTEN_OFF if shock else 0))
            bid_px = bb + off + center_shift
            ask_px = ba - off + center_shift
            if bid_px >= ask_px:
                continue
            buy_room = lim - pos
            sell_room = lim + pos
