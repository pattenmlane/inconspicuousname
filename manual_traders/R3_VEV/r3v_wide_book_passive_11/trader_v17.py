"""trader_v17: vouchers_final_strategy (Round 3) — same as v16 + one execution tweak + telemetry.

- Same as v16: joint VEV_5200+VEV_5300 spread<=2 = risk_on (logged); wide passive 4000/4500;
  extract penny MM; delta+vega skew from 4000+4500 only; no HYDROGEL orders; 5200/5300 signal-only.
- *Change vs v16:* `EXTRACT_PASSIVE_SIZE` 6 -> 7 (cap 12) — small, measurable nudge; gate-conditioned
  EMA thresholds from iteration notes never bit in the simulator (VEV_4000 wide book rarely
  coincides with joint tight 5200/5300; one-tick EMA stayed in [-0.5,0.5]), so we do not
  wire broken no-ops into live sizing. We still log `gate_ret_ema` for analysis.
- See round3work/vouchers_final_strategy/STRATEGY.txt.
"""
...
# v16 used 6; nudge for worse-fill PnL on extract+VEV_4000 bundle
EXTRACT_PASSIVE_SIZE = 7

# Causal EMA of one-tick extract changes while 5200+5300 are jointly tight; logged only
...
class Trader:
    def run(self, state: TradingState):
        ...
        if (
            ...
            gate_ret_ema = (1.0 - GATE_RET_EMA_ALPHA) * float(gate_ret_ema) + GATE_RET_EMA_ALPHA * r_tick
