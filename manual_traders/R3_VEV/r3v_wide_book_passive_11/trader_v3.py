"""v3: VEV_4000 keeps deep passive (v1); VEV_4500 uses touch penny-improve (bb+1/ba-1) to get fills."""
...
class Trader:
    def run(self, state: TradingState):
        ...
        net_call_delta = sum(_pos(state, s) * vev_delta.get(s, 0.0) for s in ("VEV_4000", "VEV_4500"))
        skew = max(-8, min(8, int(round(net_call_delta * 0.35))))

        # VEV_4000 deep passive
        sym = "VEV_4000"
        od = state.order_depths.get(sym)
        if od is not None:
            sp = _spread(od)
            if sp is not None and sp >= WIDE_SPREAD_MIN:
                bb, ba = _best_bid_ask(od)
                if bb is not None and ba is not None:
                    off = DEEP_OFFSET_4000
                    bid_px, ask_px = bb + off, ba - off
                    if bid_px < ask_px:
                        pos = _pos(state, sym)
                        lim = LIMITS[sym]
                        br, sr = lim - pos, lim + pos
                        if br > 0:
                            orders[sym].append(Order(sym, int(bid_px), min(PASSIVE_SIZE_4000, br)))
                        if sr > 0:
                            orders[sym].append(Order(sym, int(ask_px), -min(PASSIVE_SIZE_4000, sr)))

        # VEV_4500 touch improve (still passive vs mid; wide spread context)
        sym = "VEV_4500"
        od = state.order_depths.get(sym)
        if od is not None:
            sp = _spread(od)
            if sp is not None and sp >= WIDE_SPREAD_MIN:
                bb, ba = _best_bid_ask(od)
                if bb is not None and ba is not None:
                    bid_px, ask_px = bb + 1, ba - 1
                    if bid_px < ask_px:
                        pos = _pos(state, sym)
                        lim = LIMITS[sym]
                        br, sr = lim - pos, lim + pos
                        if br > 0:
                            orders[sym].append(Order(sym, int(bid_px), min(PASSIVE_SIZE_4500, br)))
                        if sr > 0:
                            orders[sym].append(Order(sym, int(ask_px), -min(PASSIVE_SIZE_4500, sr)))
