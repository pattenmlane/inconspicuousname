"""v5 + second passive layer on 4000/4500: when L1 spread >= WIDE_SPREAD_DEEP, add
bb+off+OFF_DEEP2 / ba-off-OFF_DEEP2 with size PASSIVE_SIZE2; room reserved for both tiers
so limits are not breached. (Grid sweep: any positive GAMMA_SKEW on extract hurt PnL; BS
gamma is computed on tape in analyze_tapes.py, not in extract skew for this file.)

TTE: 6d/365y greek proxy (unchanged from v5).
"""
...
WIDE_SPREAD_MIN = 16
# Extra passive layer only when the book is "extra wide" (tape medians 21/16).
WIDE_SPREAD_DEEP = 20
# Primary deep passive (same as v4/v5 best).
DEEP_OFFSET = {"VEV_4000": 5, "VEV_4500": 6}
# Second tier: one tick deeper in from each side (still inside spread when sp >= 20+2*OFF_DEEP2).
OFF_DEEP2 = 1
PASSIVE_SIZE = 3
PASSIVE_SIZE2 = 2
...
class Trader:
    def run(self, state: TradingState):
        ...
        for sym in TARGET_VEV:
            ...
            buy_room = lim - pos
            sell_room = lim + pos
            q_bu1 = min(PASSIVE_SIZE, buy_room) if buy_room > 0 else 0
            q_se1 = min(PASSIVE_SIZE, sell_room) if sell_room > 0 else 0
            if q_bu1 > 0:
                orders[sym].append(Order(sym, int(bid_px), q_bu1))
            if q_se1 > 0:
                orders[sym].append(Order(sym, int(ask_px), -q_se1))
            if sp is not None and sp >= WIDE_SPREAD_DEEP:
                d = OFF_DEEP2
                bid2 = bb + off + d
                ask2 = ba - off - d
                if bid2 < ask2 and bid2 < ask_px and ask2 > bid_px:
                    br = lim - pos - q_bu1
                    sr = lim + pos - q_se1
                    if br > 0:
                        q2 = min(PASSIVE_SIZE2, br)
                        if q2 > 0:
                            orders[sym].append(Order(sym, int(bid2), q2))
                    if sr > 0:
                        q2 = min(PASSIVE_SIZE2, sr)
                        if q2 > 0:
                            orders[sym].append(Order(sym, int(ask2), -q2))
