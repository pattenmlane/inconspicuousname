"""v7: v5 + focused backtest sweep (sweep_iter7_focused.json): lower spread gate 15, shallower deep passive on 4000 (bb+4) and 4500 (bb+5). Same delta+vega extract skew.

v5 parent: v4 + BS vega on target strikes; extract hedge skew = delta*DELTA_SKEW + pos*vega*VEGA_SKEW.

TTE for greeks: 6d/365y proxy in-file (traderData does not carry tape day). Tape work
(see analysis.json) shows 4000/4500 mean BS vega ~10–13 per 1.0 vol at Round-3 TTEs;
deep ITM delta ~0.994 so both channels are small; vega term nudges extract quotes when
option inventory builds IV sensitivity.
"""
