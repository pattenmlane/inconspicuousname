"""
Fade smile curvature: sell wings vs quadratic IV(m_t) when wings are rich;
buy wings on inversion. Hedges with extract + light hydrogel delta tilt.

TTE: round3work/round3description.txt (historical CSV day -> 8,7,6 d open) plus
intraday wind-down per round3work/plotting/.../plot_iv_smile_round3.py.
"""
...
def _dte_eff(csv_day: int, ts: int) -> float:
    base = 8 - int(csv_day)
    return max(float(base) - _intraday_progress(ts), 1e-6)
...
class Trader:
    def run(self, state: TradingState):
        ...
        wing_set = {4000, 4500, 5400, 5500, 6000, 6500}
        ...
        for v, K, mid, iv, delta in strikes_data:
            ...
            lim = LIMITS[v]
            qty = 0
            if K in wing_set and resid > RICH_THR and pos > -lim + 1:
                qty = -min(MAX_LEG, pos + lim)
            elif K in wing_set and resid < CHEAP_THR and pos < lim - 1:
                qty = min(MAX_LEG, lim - pos)
            if qty == 0:
                continue
            d = state.order_depths[v]
            if qty > 0 and d.sell_orders:
                px = min(d.sell_orders)
                orders[v].append(Order(v, px, qty))
        ...
        if hedge_q != 0 and du.buy_orders and du.sell_orders:
            if hedge_q > 0:
                px = min(du.sell_orders)
                orders[U].append(Order(U, px, hedge_q))
            else:
                px = max(du.buy_orders)
                orders[U].append(Order(U, px, hedge_q))

        # small hydrogel tilt vs extract hedge
