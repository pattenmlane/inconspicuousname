"""
v1: Same smile-curvature thesis as v0; parameter sweep + risk tweaks:
  - Full delta hedge on extract (HEDGE_FRAC=1.0).
  - Trade wings only when |m_t| is large enough (avoid noisy near-ATM wings).
  - Slightly wider IV residual thresholds; smaller max clip; tighter spread filter.
"""
...
class Trader:
    def run(self, state: TradingState):
        ...
        for v in VOUCHERS:
            ...
            xs.append(m_t)
            ys.append(iv)
            strikes_data.append((v, K, mid, iv, bs_delta(mid, S, K, T), m_t))
        ...
        d_net = 0.0
        for v, K, mid, iv, delta, m_t in strikes_data:
            if delta is None:
                continue
            if K not in WING_KS or abs(m_t) < MIN_ABS_MT:
                continue
            resid = iv - iv_hat(m_t)
            ...
            elif resid < CHEAP_THR and pos < lim - 1:
