"""
v11: Signed smile-curvature fade (pivot from v7–v10).

Tape check: quadratic IV(m_t) residuals for 5400/5500 are typically *negative*
(mean around -0.01 to -0.017), so "sell rich wings" on those strikes rarely fires.
VEV_6000 residuals are typically *positive* vs the same parabola — sell that OTM
rich vol; buy 5400/5500 when residual is very negative (cheap vs core fit).

Parent: v2 mechanics (time-value filter for smile fit, BS IV, partial extract hedge).
"""
...
# Short rich *OTM* wing (6000); buy cheap upper ITM/near-ITM wings (5400/5500).
RICH_SHORT_KS = {6000}
RICH_THR = 0.008
CHEAP_LONG_KS = {5400, 5500}
CHEAP_THR = -0.014
EXIT_RICH = 0.003
    ...
    def run(self, state: TradingState):
        ...
        for v in VOUCHERS:
            ...
            if mid <= 0 or tv / mid < MIN_TIME_VALUE_FRAC:
                continue
            ivv = implied_vol(mid, S, K, T)
            if ivv is None:
                continue
            m_t = math.log(K / S) / sqrtT
            xs.append(m_t)
            ys.append(ivv)
            strikes_data.append((v, K, mid, ivv, bs_delta(mid, S, K, T), tv / mid))

        if len(xs) < 6:
            return orders, 0, json.dumps(td)

        coeff = np.polyfit(np.asarray(xs, dtype=float), np.asarray(ys, dtype=float), 2)
        a, b, c2 = float(coeff[0]), float(coeff[1]), float(coeff[2])

        def iv_hat(m: float) -> float:
            return a * m * m + b * m + c2

        d_net = 0.0
        for v, K, mid, ivv, delta, tv_frac in strikes_data:
            if delta is None:
                continue
            m_t = math.log(K / S) / sqrtT
            resid = ivv - iv_hat(m_t)
            ...
            lim = LIMITS[v]
            qty = 0
            if K in RICH_SHORT_KS:
                if pos < 0 and resid < EXIT_RICH:
                    qty = min(MAX_LEG, -pos)
                elif resid > RICH_THR and pos > -lim + 1:
                    qty = -min(MAX_LEG, pos + lim)
            elif K in CHEAP_LONG_KS:
                if resid < CHEAP_THR and pos < lim - 1 and tv_frac >= MIN_TIME_VALUE_FRAC:
