"""
v12: Parent v2 with cubic IV(m_t) core (deg-3 poly) instead of quadratic.
Same wing set, thresholds, and hedging as v2 for a clean model-comparison grid.
Least-squares cubic with >=6 IV points (same as v2 count floor; 6>3 so fit is well-posed).
"""
...
class Trader:
    def run(self, state: TradingState):
        ...
        coeff = np.polyfit(np.asarray(xs, dtype=float), np.asarray(ys, dtype=float), 3)
        d, a, b, c = float(coeff[0]), float(coeff[1]), float(coeff[2]), float(coeff[3])

        def iv_hat(m: float) -> float:
            return d * m * m * m + a * m * m + b * m + c
