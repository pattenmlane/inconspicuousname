"""
v29: Underlying-response-aware variant from v20.
Per-strike clip sizing follows extract->voucher beta buckets from tape analysis.
"""
...
HIGH_RESP_KS = {4000, 4500}
LOW_RESP_KS = {6000, 6500}
...
class Trader:
    def run(self, state: TradingState):
        ...
        for v, K, mid, iv, delta, tv_frac in strikes_data:
            ...
            lim = LIMITS[v]
            leg_cap = MAX_LEG
            if K in HIGH_RESP_KS:
                leg_cap = max(6, MAX_LEG - 4)
            elif K in LOW_RESP_KS:
                leg_cap = max(5, MAX_LEG - 6)
