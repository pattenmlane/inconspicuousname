"""
v32: v31 + skip far-OTM cheap longs when |Δextract| is a shock (reduces VEV_6000 path).
Short-rich and non-far legs unchanged.
"""
...
def _leg_cap_from_delta(dlt: float) -> int:
    if abs(dlt) >= DELTA_HIGH:
        return max(5, MAX_LEG - LEG_TRIM_HIGH)
    return MAX_LEG
...
class Trader:
    def run(self, state: TradingState):
        ...
        for v, K, mid, iv, delta, tv_frac in strikes_data:
            ...
            leg_cap = _leg_cap_from_delta(delta)
            is_shock = abs(dS) >= SHOCK_DS_ABS

            qty = 0
            if resid > RICH_THR and pos > -lim + 1:
                qty = -min(leg_cap, pos + lim)
            elif (
                resid < cheap_thr_eff
                and pos < lim - 1
