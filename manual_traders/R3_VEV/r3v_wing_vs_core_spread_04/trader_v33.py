"""
v33: v30 + persistent shock flag and far-OTM cheap-long guard.

v30/v32 only see one-tick dS. A large extract move can leave the *next* tick
with a small dS while the book is still in a post-shock state where far-OTM
cheap longs misfill. When |dS|>=SHOCK_DS_ABS, set shock_left=SHOCK_PERSIST
steps; else decrement. Skip cheap *long* on VEV_6000/VEV_6500 while shock_left>0
only; same thresholds as v30 otherwise.
"""
    ...
    def run(self, state: TradingState):
        ...
        for v, K, mid, iv, delta, tv_frac in strikes_data:
            ...
            if resid > RICH_THR and pos > -lim + 1:
                ...
                resid < cheap_thr_eff
                and pos < lim - 1
                and tv_frac >= MIN_TIME_VALUE_FRAC
                and not (K in FAR_OTM_BLOCK_KS and is_shock)
            ):
