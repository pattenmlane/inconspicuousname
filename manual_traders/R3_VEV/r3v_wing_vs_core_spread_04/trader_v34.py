"""
v34: v30 + stricter cheap-long threshold for far-OTM strikes only (6000/6500).

Tape analysis (underlying_response_spread_iv_v29): OLS beta of voucher price to extract
moves is ~0 for VEV_6000/VEV_6500 — these legs do not "earn" through delta-like
exposure; cheap-wing buys there still show persistent drag in backtests. Wing
inversion entries at 4000/4500/5400/5500 keep CHEAP_THR=-0.022; only 6000/6500 use
CHEAP_THR_FAR_OTM to require deeper mispricing before lifting offers.
Shock shift on dS (v30) still applies on top of the per-strike base threshold.
"""
...
RICH_THR = 0.010
CHEAP_THR = -0.022
# Grid step from analysis: v30 baseline -0.022; far-OTM -0.026 (tighter) vs other wings
CHEAP_THR_FAR_OTM = -0.026
MAX_LEG = 12
HEDGE_FRAC = 0.28
HYDRO_FRAC = 0.04
MIN_SPREAD_SKIP = 6
...
class Trader:
    def run(self, state: TradingState):
        ...
        for v, K, mid, iv, delta, tv_frac in strikes_data:
            ...
            lim = LIMITS[v]
            base_cheap = CHEAP_THR_FAR_OTM if K in FAR_OTM_CHEAP_KS else CHEAP_THR
            cheap_thr_eff = base_cheap
            if abs(dS) >= SHOCK_DS_ABS:
                cheap_thr_eff = base_cheap + SHOCK_CHEAP_SHIFT
