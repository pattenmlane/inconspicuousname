"""
v35: v30 + smaller clip on cheap longs for VEV_6000/VEV_6500 only (MAX_LEG_CHEAP_FAR=8).

Far-OTM strikes show ~zero extract beta in historical tape (underlying_response_spread_iv_v29)
and chronic negative PnL on VEV_6000 in worse-fill runs while VEV_5400 carries profit.
Capping *long* size on those legs cuts exposure without retuning global CHEAP/RICH; rich-wing
shorts and other cheap longs stay at MAX_LEG=12. Shock cheap shift (v30) unchanged.
"""
...
CHEAP_THR = -0.022
MAX_LEG = 12
# Small parameter sweep: reduce far-OTM long clip only (v35 grid vs v30 MAX_LEG uniform)
MAX_LEG_CHEAP_FAR = 8
HEDGE_FRAC = 0.28
HYDRO_FRAC = 0.04
MIN_SPREAD_SKIP = 6
    ...
    def run(self, state: TradingState):
        ...
        for v, K, mid, iv, delta, tv_frac in strikes_data:
            ...
            if resid > RICH_THR and pos > -lim + 1:
                ...
                leg = MAX_LEG_CHEAP_FAR if K in FAR_CHEAP_LEG_KS else MAX_LEG
                qty = min(leg, lim - pos)
            if qty == 0:
                continue
