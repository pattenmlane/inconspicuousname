"""
v4: Smile-curvature fade with focused tradable wings.
Parent: v2. Only trade upper wings (5400/5500) where IV residuals were stable and
vega meaningful in analysis; avoid low-time-value deep ITM lower wings (4000/4500).
Add explicit short-cover when residual normalizes.
"""
...
class Trader:
    def run(self, state: TradingState):
        ...
        for v, K, mid, iv, delta, tv_frac in strikes_data:
            ...
            elif pos < 0 and resid < EXIT_THR:
                # Cover short wings once curvature edge mean-reverts.
                qty = min(MAX_LEG, -pos)
            elif resid < CHEAP_THR and pos < lim - 1 and tv_frac >= MIN_TIME_VALUE_FRAC:
                qty = min(MAX_LEG // 2, lim - pos)
            if qty == 0:
                continue
