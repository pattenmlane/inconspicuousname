"""
v43 — same thesis as v42; adds inclineGod-style *extract* book filter before paying spread.

Replicate join uses 5200, 5300, extract: all three in sync. We only take the optional
STRATEGY long-extract when joint gate is on **and** extract BBO width ≤ EXTRACT_SMAX
ticks (tighter extract = less taker cost under --match-trades worse).

Smaller clip than v42; else identical edge logic to v42.
"""
...
def _ok_to_buy_extract(du: OrderDepth) -> bool:
    s = _spread_bbo(du)
    return s is not None and s <= EXTRACT_SMAX
