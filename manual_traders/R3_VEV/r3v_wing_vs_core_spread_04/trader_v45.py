"""
v45 — v44 variant: same Sonic joint gate and wide-regime flatten, but **bid-only** on
5200/5300/5400 and extract in the tight regime (no passive asks, so we do not warehouse
**short** VEVs that then bleed under public tape prints — the main v44 *all* pain).

STRATEGY layer-1: risk-on filter; optional long delta via **long vouchers / long extract**
only (Discord optional “long extract / delta” is long-biased, not short-OTM taker from v41–v43).
No smile, no HYDROGEL.
"""
...
class Trader:
    def run(self, state: TradingState):
        ...
        for v in TRADE_VEV:
            ...
            for ord_ in _bid_only(v, d, pos, LIMITS[v], CLIP):
        ...
        if s_u is not None and s_u <= U_SPREAD_MAX and du is not None:
            ...
            for ord_ in _bid_only(U, du, pu, LIMITS[U], U_CLIP):
