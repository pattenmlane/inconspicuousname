"""
v47 — v45 with explicit soft long targets (12 VEV, 20 extract) and **bids at best bid only**.

(Attempt: 1-tick bid improver on >=2-tick books matched v46-style bleed under `worse`;
reverted to **touch** posting like v45 — improver + tight Sonic gate misfired vs zero-fill v45.)
"""
...
CLIP = 5
U_CLIP = 4
# Soft cap on how much long to warehouse from tight-regime **bids** (in addition to product limit)
MAX_VEV_LONG = 12
MAX_U_LONG = 20
MAX_FLAT = 12
...
def _bid_only(
    ...
    if t is None:
        return []
    bb, _ba = t
    bq = min(clip, max(0, long_cap - pos), max(0, lim - pos))
    if bq > 0:
