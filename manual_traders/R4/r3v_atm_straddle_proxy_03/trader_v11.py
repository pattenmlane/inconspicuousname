"""
Round 4 trader v11 — v8 + **Mark 49 seller** on VELVETFRUIT_EXTRACT (Phase 1 cohort).

`r4_ph1_participant_predictivity.json`: Mark 49 as **seller** on extract, K=5, n≈105,
mean fwd mid on extract ≈ +1.82, t≈9.6 (strong positive short-horizon drift). Phase 1 table
uses **all** seller prints on that symbol (not restricted to aggressive; tape has almost no
Mark 49 aggressive sells — see analysis_outputs/r4_ph1_m49_seller_extract_counts.json).

Overlay: days 1–2 only, small clip (3 loose / 4 Sonic-tight), same extract spread skip/cap as v8.
Runs **before** the Mark 67 loop so both can fill same tick if capacity allows. Lead-lag suppress
still skips the whole tick (same as v8).
"""
...
class Trader:
    def run(self, state: TradingState):
        ...
        # --- Mark 49 seller on extract (Phase 1 seller cohort, K=5) ---
        if day < 3 and ex_od and ex_od.sell_orders:
            if any(seller == "Mark 49" and sym == EXTRACT for _b, seller, sym, _p, _q in rows):
                c49 = CLIP_M49_TIGHT if _sonic_tight(state) else CLIP_M49_LOOSE
                q49 = min(c49, POS[EXTRACT] - px)
                if q49 > 0:
                    ba_ex = min(ex_od.sell_orders)
                    out[EXTRACT].append(Order(EXTRACT, int(math.ceil(float(ba_ex))), q49))
                    px += q49
