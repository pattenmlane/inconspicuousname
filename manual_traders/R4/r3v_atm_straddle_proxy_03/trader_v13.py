"""
Round 4 trader v13 — v8 + **smaller Mark67 clip** when Mark 49 sells extract same tick.

Offline (new): `r4_ph1_m67_aggr_fwd_extract_by_m49_overlap.json` — at Mark67 aggressive-buy
timestamps, extract forward mids are still positive both with and without same-tick M49 seller
extract; day-2 K=5 mean is slightly **lower** on overlap (1.66 vs 1.88) suggesting overlap is not
where you want *more* size (v12 falsified). v13 applies **-2** to the v8 clip on overlap, **floor 4**,
days 1–2 only. Lead-lag suppress unchanged.
"""
...
class Trader:
    def run(self, state: TradingState):
        ...
        if day < 3 and m49_sell_ex:
            clip = max(clip - CLIP_TRIM_M49_OVERLAP, CLIP_FLOOR)
