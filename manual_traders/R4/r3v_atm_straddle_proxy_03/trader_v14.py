"""
Round 4 trader v14 — v13 + **Phase 3 Sonic interaction** on Mark49 overlap trim.

v13: days 1–2, if Mark 49 sells extract same tick → Mark67 clip -= 2 (floor 4).
Phase 3 / ph3: Mark67 subset slightly stronger when joint gate tight at trade time.

Hypothesis: on **tight** 5200+5300 books, the crowded M49-overlap tick needs **less** clip reduction
than when wings are wide (liquidity elsewhere). v14 applies **-1** trim when Sonic tight + M49
overlap, **-2** when M49 overlap but Sonic loose (matches v13 in loose case).

Lead-lag suppress unchanged.
"""
...
CLIP_TRIM_M49_OVERLAP_LOOSE = 2
CLIP_TRIM_M49_OVERLAP_TIGHT = 1
CLIP_FLOOR = 4
...
class Trader:
    def run(self, state: TradingState):
        ...
        if day < 3 and m49_sell_ex:
            trim = CLIP_TRIM_M49_OVERLAP_TIGHT if _sonic_tight(state) else CLIP_TRIM_M49_OVERLAP_LOOSE
