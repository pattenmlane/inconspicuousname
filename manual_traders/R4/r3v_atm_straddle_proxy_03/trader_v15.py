"""
Round 4 trader v15 — v13 variant: **asymmetric** M49-overlap clip trim vs Sonic.

v13: always -2 on overlap (days 1-2), floor 4.
v14: -1 if Sonic tight else -2 (all worse same as v13; all-match slightly worse).

v15: **stronger** trim when joint gate is **loose** (surface noisy / execution harder): -3 on overlap
when not Sonic tight; **-1** when overlap and Sonic tight (still reduce a bit vs no trim). Floor 4.
Tests whether offline r4_ph1_m67_overlap_fwd5_by_sonic.json (sparse tight cells) still benefits from
lighter touch when tight.
"""
...
CLIP_TRIM_OVERLAP_TIGHT = 1
CLIP_TRIM_OVERLAP_LOOSE = 3
CLIP_FLOOR = 4
...
class Trader:
    def run(self, state: TradingState):
        ...
        tight = _sonic_tight(state)
        clip = lt if tight else lf
        ...
        if day < 3 and m49_sell_ex:
            trim = CLIP_TRIM_OVERLAP_TIGHT if tight else CLIP_TRIM_OVERLAP_LOOSE
            clip = max(clip - trim, CLIP_FLOOR)
