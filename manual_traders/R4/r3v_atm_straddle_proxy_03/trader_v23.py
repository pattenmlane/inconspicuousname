"""
Round 4 trader v23 — v21 with **lower floor** on M49-overlap clip trim (days 1-2).

v21: after trim, clip floored at **4**. v22 showed -7 plateaus vs -6 because many overlap ticks
already hit 4. v23 uses floor **3** only when M49 seller extract co-tick applies (same trim 0/6).
Tests whether one extra contract on the worst overlap ticks helps under worse fills.
"""
...
CLIP_TRIM_OVERLAP_TIGHT = 0
CLIP_TRIM_OVERLAP_LOOSE = 6
CLIP_FLOOR_M49_OVERLAP = 3
...
class Trader:
    def run(self, state: TradingState):
        ...
        if day < 3 and m49_sell_ex:
            ...
            clip = max(clip - trim, CLIP_FLOOR_M49_OVERLAP)
