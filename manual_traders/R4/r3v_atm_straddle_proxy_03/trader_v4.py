"""
Round 4 trader v4 — v3 + day-3 risk trim + extract spread guard.

Motivation (v3 / analysis iteration 3): day-3 extract PnL negative under worse; Phase 2 showed Mark67
aggressive extract events sit in tighter extract spreads. Here:

- Days 1–2: same as v3 (clip 16 Sonic tight, 10 loose).
- Historical day 3: smaller clips (12 tight / 8 loose) to reduce tail from larger tight fills.
- If extract L1 spread > 10, cap clip to loose size even when Sonic tight (execution / mid caveat).
- If extract L1 spread > 16, skip the Mark67 lift (too wide for the edge we studied).

Tape + counterparty logic unchanged from v0/v3.
"""
