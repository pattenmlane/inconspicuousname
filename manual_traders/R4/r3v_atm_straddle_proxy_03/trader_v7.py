"""
Round 4 trader v7 — v4 + Mark 55 **risk-off** on same tick.

Phase 1 (r4_ph1_adverse_aggressor_fwd20.json): Mark 55 as aggressive **seller** has mildly negative
mean fwd20 on the traded symbol (MM-style hitting bid). If the tape shows Mark 55 aggressively
selling VELVETFRUIT_EXTRACT at the same timestamp, **skip** the Mark 67 long lift (avoid leaning
long into concurrent sell pressure).
...
class Trader:
    def run(self, state: TradingState):
        ...
        if ex_od is not None:
            bb_ex, ba_ex = _bb_ba(ex_od)
            if bb_ex is not None and ba_ex is not None:
                for _b, seller, sym, price, _q in rows:
                    if seller != "Mark 55" or sym != EXTRACT:
                        continue
                    if price <= bb_ex:
                        return out, 0, json.dumps(td)
