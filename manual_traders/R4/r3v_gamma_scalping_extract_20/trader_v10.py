"""
Round 4 — **v8** on day 3 **relaxed** where the tape has **no** Mark01→Mark22
surface print.

- Days **1–2**: identical to **trader_v5** (Sonic gate + Mark 67 extract + 5200/5300 MM).
- **Tape day 3**: same gate + Mark 67; post **VEV_5200/5300** MM **only** on
  requote ticks where **this tick's** `__BT_TAPE_TRADES_JSON__` has **no**
  **Mark 01 → Mark 22** print on **VEV_5200** or **VEV_5300** (otherwise skip
  surface MM for that bucket, `mm_tick` set).

Compares to **v8** (no surface MM all day 3) and **v9** (skip on that print
**every** day — too aggressive). See `r4_day3_joint_tight_m01_m22_coverage.csv`
for how often day-3 joint-tight timestamps carry this print.
"""
...
class Trader:
    def run(self, state: TradingState) -> tuple[dict[str, list[Order]], int, str]:
        ...
        if tape_day == DAY3_CONDITIONAL_SURFACE and mark01_to_22_surface_on_tape(tape):
            store["d3_skip_surface_m01_m22"] = True
            store["mm_tick"] = tick
            return result, conversions, json.dumps(store)
