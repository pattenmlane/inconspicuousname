"""
Round 4 — **v19** + **extract spread filter** for Mark 67 lift (Phase 2 regime).

Same as **trader_v19** (Sonic joint gate, no surface MM tape day 3, **CLIP_EX=20**,
**MAX_EX_POS=120**) but only follow **Mark 67** aggressive extract when
**VELVETFRUIT_EXTRACT** L1 spread **≤ EXTRACT_SPREAD_MAX_M67**.

Rationale: Phase 2 offline (`r4_phase2_mark67_extract_buyagg_k5_tight_by_day.csv`)
conditions Mark67 stats on extract **tight** tertile; per-day quantiles here collapse
to spread **5** for both 33rd/66th, so we use an explicit **stricter** cap (4 ticks)
to test “wider extract = worse follow” under **`worse`**.
"""
...
# Only lift extract when book is at most this wide (ask - bid on VELVETFRUIT_EXTRACT).
EXTRACT_SPREAD_MAX_M67 = 4
...
class Trader:
    def run(self, state: TradingState) -> tuple[dict[str, list[Order]], int, str]:
        ...
        ex_spr = _bbo_spread(d_ex)
        allow_m67 = ex_spr is not None and ex_spr <= EXTRACT_SPREAD_MAX_M67
        if not allow_m67:
            store["m67_skipped_spread"] = True
