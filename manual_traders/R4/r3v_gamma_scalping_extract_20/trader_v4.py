"""
Round 4 — **v3 + counterparty risk-off** on the tape.

Same as **trader_v3** (Sonic joint gate; MM only **VEV_5200** / **VEV_5300** at
BBO+1). Additionally: if this tick’s `__BT_TAPE_TRADES_JSON__` contains a
**Mark 14 → Mark 55** print on **VELVETFRUIT_EXTRACT**, **skip** posting VEV
quotes for this requote interval (Phase 1/2 adverse-selection hypothesis on
that pair, applied only when we would otherwise provide liquidity on the tight
surface).
"""
...
def adverse_extract_on_tape(obs: dict) -> bool:
    ...
    for t in tape:
        if (
            t.get("buyer") == ADV_BUYER
            and t.get("seller") == ADV_SELLER
            and t.get("symbol") == EXTRACT
        ):
            return True
    return False
...
class Trader:
    def run(self, state: TradingState) -> tuple[dict[str, list[Order]], int, str]:
        ...
        if adverse_extract_on_tape(obs):
            store["skip_adverse_extract_print"] = True
            if "__BT_TAPE_DAY__" in obs:
                store["tape_day"] = int(obs["__BT_TAPE_DAY__"])
            return result, conversions, json.dumps(store)
        store["skip_adverse_extract_print"] = False
