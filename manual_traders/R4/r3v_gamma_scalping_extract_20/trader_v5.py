"""
Round 4 — **Sonic joint gate** + **tight-surface MM** (v3) **and** **Mark 67**
aggressive-extract follow (v2).

When joint gate is **on**:
1) If tape shows **Mark 67** lifting **VELVETFRUIT_EXTRACT** at/through the ask,
   buy up to **CLIP_EX** lots (capped by **MAX_EX_POS**), same as v2.
2) On requote cadence, post MM on **VEV_5200** and **VEV_5300** only (v3).

When gate is **off**, no orders. Combines population-level tight surface (Phase 3)
with the strongest counterparty tape signal from Phase 1/2.
"""
