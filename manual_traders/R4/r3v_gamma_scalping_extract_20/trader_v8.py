"""
Round 4 — **v5** with **no VEV surface MM on tape day 3**.

Rationale: `trader_v5` worse PnL is dragged by **VEV_5200/5300** on the last R4
tape day (day 3 in backtester numbering) while **Mark 67** extract follow stays
useful there. This variant keeps **Sonic joint gate** + **Mark 67** aggressive
extract lift every day, but **skips** posting 5200/5300 quotes when
`__BT_TAPE_DAY__ == 3` to test whether surface MM is the marginal loser on
that day only (counterparty-aware thesis unchanged; execution/risk tweak).
"""
...
# Backtester `__BT_TAPE_DAY__` matches `prices_round_4_day_{n}.csv` day index (1,2,3,...).
