"""
Round 4 — Sonic joint gate, **passive** extract bids (v6).

v3/v5 showed aggressive buys at ask lose badly under --match-trades worse.
This variant **only** posts **limit buys at best bid** when the joint gate is
tight (rests liquidity; fills when sell pressure hits bid per order matcher).
When the gate goes **wide**, sell **CLIP** at best ask if long to reduce
inventory (still pays half-spread on exit — experiment).

No VEV/hydro. Limits 200.
"""
