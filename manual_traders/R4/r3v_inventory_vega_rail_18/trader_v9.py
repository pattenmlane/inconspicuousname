"""
Round 4 — v6 passive entry + v4-style **falling-edge** exit (v9).

v6: passive bid every tight tick; sell CLIP at ask **every** wide tick (sparse
worse fills, +252).

v9: same **passive bid** while tight as v6, but **only** sell at best ask on
the **falling edge** (tight -> wide), flattening **full** long position like
trader_v4 — avoids repeated adverse exits across long wide regimes.
"""
...
class Trader:
    def run(self, state: TradingState):
        ...
        if (not now_t) and prev_t and p > 0:
