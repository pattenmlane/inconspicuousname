#!/usr/bin/env python3
"""
Precompute timestamps: **VELVETFRUIT_EXTRACT** trades where aggressor is **buy** (price >= ask)
and **seller == Mark 22** (Phase 1 Tier-A edge: positive short-horizon extract fwd).

Output: precomputed/r4_extract_aggr_buy_m22_print.json
Shape: {"1": [ts, ...], "2": [...], "3": [...]} — unique sorted timestamps.

Run from repo root:
  python3 manual_traders/R4/r3v_jump_gap_filter_17/preprocess_r4_extract_aggr_buy_mark22.py
"""
...
OUT = Path(__file__).resolve().parent / "precomputed" / "r4_extract_aggr_buy_m22_print.json"
...
def main() -> None:
    ...
    for day in DAYS:
        ...
        ts_list = sorted(m.loc[aggr_buy, "timestamp"].astype(int).unique().tolist())
        out[str(day)] = ts_list
