#!/usr/bin/env python3
"""
Precompute timestamps: VELVETFRUIT_EXTRACT aggressive buy (price >= ask) with **Mark 67** as buyer.

Output: precomputed/r4_extract_aggr_buy_m67_print.json

Run from repo root:
  python3 manual_traders/R4/r3v_jump_gap_filter_17/preprocess_r4_extract_aggr_buy_mark67.py
"""
...
OUT = Path(__file__).resolve().parent / "precomputed" / "r4_extract_aggr_buy_m67_print.json"
...
def main() -> None:
    ...
    for day in DAYS:
        ...
        tr = tr[(tr["symbol"] == EXTRACT) & (tr["buyer"] == BUYER)].copy()
