"""
Round 4 iteration 11 — Mark22 aggressive-buy prints: **bid-only** size lift (no fair tilt).

v10 raised clip to 17 **and** +1 fair on both bid/ask at print timestamps → worse PnL than v1.
v11 keeps v1 fair (`fi = round(f) + microprice skew` only). On precomputed print ts, **only**
the buy order uses `mq_base+1`; ask side stays `mq_base` (joint 16/5 or off 5).

Precompute: `r4_extract_aggr_buy_m22_print.json` (same as v9/v10).
"""
...
class Trader:
    def run(self, state: TradingState):
        ...
        mq_bid = mq_base + (1 if on_m22_print else 0)
        mq_ask = mq_base
