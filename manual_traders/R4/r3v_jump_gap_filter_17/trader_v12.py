"""
Round 4 iteration 12 — **union** of Phase-1 aggressive extract buys: Mark **22** or **49** seller.

Same mechanics as **v11** (bid-only +1 clip when joint-on at print ts; no tape fair tilt).
Loads:
- `precomputed/r4_extract_aggr_buy_m22_print.json`
- `precomputed/r4_extract_aggr_buy_m49_print.json`

Run `preprocess_r4_extract_aggr_buy_mark49.py` if m49 JSON is missing.
"""
...
def _load_ts(name: str) -> dict[int, set[int]]:
    p = Path(__file__).resolve().parent / "precomputed" / name
    raw = json.loads(p.read_text(encoding="utf-8"))
    return {int(k): set(int(x) for x in v) for k, v in raw.items()}


_SIG: dict[int, set[int]] | None = None
...
class Trader:
    def run(self, state: TradingState):
        global _SIG
        if _SIG is None:
            a = _load_ts("r4_extract_aggr_buy_m22_print.json")
            b = _load_ts("r4_extract_aggr_buy_m49_print.json")
            _SIG = {d: a.get(d, set()) | b.get(d, set()) for d in (1, 2, 3)}
        ...
        on_print = ts in _SIG.get(day, set())
        ...
        mq_bid = mq_base + (1 if on_print else 0)
        mq_ask = mq_base
