"""
Round 4 iteration 15 — Mark **67** aggressive-buy prints: bid +2 (same as v13 but buyer-side tape).

Precompute: `preprocess_r4_extract_aggr_buy_mark67.py` → `precomputed/r4_extract_aggr_buy_m67_print.json`.
When current ts is a print timestamp: `mq_bid = mq_base + 2`, `mq_ask = mq_base` (v13 parity).
"""
...
def _load_m67_aggr_buy_ts() -> dict[int, set[int]]:
    p = Path(__file__).resolve().parent / "precomputed" / "r4_extract_aggr_buy_m67_print.json"
    raw = json.loads(p.read_text(encoding="utf-8"))
    return {int(k): set(int(x) for x in v) for k, v in raw.items()}


_M67_TS: dict[int, set[int]] | None = None
...
class Trader:
    def run(self, state: TradingState):
        ...
        on_m67_print = ts in _M67_TS.get(day, set())
        ...
        extra = _BID_EXTRA_PRINT if on_m67_print else 0
