"""
Round 4 iteration 16 — **Union** of Phase-1 extract aggr-buy prints: Mark **22** seller OR Mark **67** buyer.

Same bid+2 / ask-base as **v13**; signal = `precomputed/r4_extract_aggr_buy_m22_m67_union_print.json`
(build with `preprocess_r4_extract_aggr_buy_m22_m67_union.py`).

Rationale: M22-only and M67-only sets overlap heavily but **union** adds ticks present in only one list
(63/69/47 vs 37/34/19 for M22 per day).
"""
...
def _load_union_ts() -> dict[int, set[int]]:
    p = Path(__file__).resolve().parent / "precomputed" / "r4_extract_aggr_buy_m22_m67_union_print.json"
...
class Trader:
    def run(self, state: TradingState):
        ...
        extra = _BID_EXTRA_PRINT if on_sig else 0
