"""
Round 4 iteration 2 — **Phase 3**: Sonic joint gate + **surface** on the two gate legs.

Tape (phase3): pooled K=20 forward extract mid is **higher** when both VEV_5200 and VEV_5300
TOB spreads ≤ 2 (Welch t ~7.9 on days 1–3) — aligns with R3 `analyze_vev_5200_5300_tight_gate_r3` convention.

Live logic:
- **Joint tight:** quote **VELVETFRUIT_EXTRACT** (clip 14) **and** **VEV_5200**, **VEV_5300** with
  simple wall-mid fair, make edge 1 / take edge 2, MM 12 (hedgeable surface).
- **Not tight:** extract only, clip 5 (risk-off).

HYDROGEL + other VEVs: **no** orders (keeps Tier-A sim focused on gate legs + extract).
Counterparty IDs still unavailable in `Trader.run`; tape tables in `outputs/phase3/`.
"""
...
def _vev_orders(
    ...
    if bai <= fair - TAKE_EDGE + 1e-9 and pos < lim:
        q = min(MM_SZ + 4, lim - pos)
        if q > 0:
            olist.append(Order(sym, bai, q))
    if bbi >= fair + TAKE_EDGE - 1e-9 and pos > -lim:
        q = min(MM_SZ + 4, lim + pos)
        if q > 0:
            olist.append(Order(sym, bbi, -q))
    bid_p = min(bbi + 1, int(math.floor(fair - MAKE_EDGE)))
    bid_p = max(0, bid_p)
    if bid_p < bai and pos < lim:
        q = min(MM_SZ, lim - pos)
        if q > 0:
            olist.append(Order(sym, bid_p, q))
    ask_p = max(bai - 1, int(math.ceil(fair + MAKE_EDGE)))
