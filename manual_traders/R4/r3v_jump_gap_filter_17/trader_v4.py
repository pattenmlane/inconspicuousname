"""
Round 4 iteration 4 — Phase 3 sim: joint gate + VEV legs **only when TOB spread ≥ 2** on that leg.

Tape (`r4_vev_fillability_under_gate.csv`): under joint tight, both 5200 and 5300 are almost always
**2-tick** wide, but ~5–32% of joint ticks are **1-tick** on a leg — then `best_bid+1` / `best_ask-1`
collapse to the touch and the backtester **never** matches passive quotes to `order_depths`.

v4 skips VEV MM on a leg when its spread is 1 (still satisfies Sonic joint ≤2 when the other is 2).
Same `_vev_orders` as v3 otherwise.
"""
