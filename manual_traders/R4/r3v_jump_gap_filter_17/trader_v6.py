"""
Round 4 iteration 6 — **tape + gate + counterparty window** (no live Mark IDs).

Uses precomputed timestamps: joint Sonic tight (5200+5300 ≤2) **and** within ±5 ticks of a
**Mark 01 → Mark 22** print on **VEV_5300** (`preprocess_r4_m0122_5300_gate_signal.py`).

Tape Phase 3: in that window, **short-horizon fwd on 5300** tended negative → **one-sided**:
when signal **on** and joint gate **on**, place a **single** small **sell** at **best bid** on
VEV_5300 (hit bid / passive sell — matches tape seller side of 01→22 prints). No buy on 5300
in the window. **VEV_5200** not traded here. Extract: same microprice-skew MM as v1 with
gate-sized clip.

Requires backtester env **PROSPERITY4_BACKTEST_DAY** (set by patched `test_runner`).

Precompute file must exist (run preprocess script once):
  manual_traders/R4/r3v_jump_gap_filter_17/precomputed/r4_m0122_5300_window_joint_signal.json
"""
