"""
Round 4 iteration 9 — **Phase 1 counterparty edge on extract** (tape precompute).

Tape (Phase 1): aggressive **buy** on **VELVETFRUIT_EXTRACT** with **Mark 22** as seller →
positive short-horizon extract forward (K=5). No VEV orders.

Precompute: `preprocess_r4_extract_aggr_buy_mark22.py` →
`precomputed/r4_extract_aggr_buy_m22_print.json` (trade timestamps only).

Live rule (conservative): when **current ts** is in that set **and** Sonic **joint tight**,
use **larger extract clip** (18 vs 16) and **+1** fair tilt vs v1 (lean long). Otherwise same
as `trader_v1.py`.

Requires **PROSPERITY4_BACKTEST_DAY** from patched `test_runner`.
"""
...
class Trader:
    def run(self, state: TradingState):
        ...
        tape_long = ts in _M22_TS.get(day, set())
        ...
        tape_sk = 1 if (tape_long and joint) else 0

        pos = int(state.position.get(EXTRACT, 0) or 0)
        lim = LIMITS[EXTRACT]
        if joint and tape_long:
            mq = 18
        else:
