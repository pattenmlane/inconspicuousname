"""
Round 4 — **v26 + Sonic exit discipline**.

Same precomputed triggers as `trader_v26.py` (`outputs/r4_v26_signals.json`: Mark 67
aggressive extract prints where 5200+5300 were **joint tight at the tape print**).

**Change vs v26:** while holding **VEV_5300**, flatten to bid when **either**
(1) the post-trigger **time window** ends, **or** (2) the **joint gate** breaks
(either 5200 or 5300 spread widens past 2) **even if** the window is still active.
v26 only flattened off the window, so inventory could ride through a **wide**
voucher surface — inconsistent with Sonic’s “different regime when books widen”.
...
class Trader:
    def run(self, state: TradingState):
        ...
        if on and joint_tight and sp53 <= MAX_SPREAD_V and q < LIMITS[V5300]:
            avail = abs(d53.sell_orders.get(va, 0))
            dq = min(Q, avail, LIMITS[V5300] - q)
            if dq > 0:
                orders.append(Order(V5300, va, dq))
        elif q > 0 and (not on or not joint_tight):
