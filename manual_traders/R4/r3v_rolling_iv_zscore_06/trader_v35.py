"""
Round 4 — **v26 with intended “no lifts on day 3” guard (DO NOT USE — ineffective).**

**Bug:** `TestRunner` passes **per-day local** `state.timestamp` (CSV clock), **not**
merged-timeline absolute time. The check `timestamp >= 2_000_000` therefore **never**
fires; this file behaves like **v26** (kept for regression / documentation).

Use **`trader_v36.py`** + `outputs/r4_v36_signals.json` to drop tape–day-3 triggers
offline instead.
...
class Trader:
    def run(self, state: TradingState):
        ...
        allow_entry = ts < DAY3_MERGED_TS_MIN
        if on and joint_tight and allow_entry and sp53 <= MAX_SPREAD_V and q < LIMITS[V5300]:
        ...
        td["_d3_skip"] = 0.0 if allow_entry else 1.0
        td["_ts"] = float(ts)
        return result, 0, json.dumps(td)
