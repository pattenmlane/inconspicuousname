"""
Round 4 — **v26 with tape–day-3 Mark67 triggers removed** (correct offline filter).

`trader_v35.py` used merged `state.timestamp >= 2e6`, but the backtester passes
**per-day local** timestamps (same grid as CSV), so that guard never fired (v35
matched v26).

Precomputed triggers: `outputs/r4_v36_signals.json` — same construction as
`r4_v26_signals.json` but **drops** any merged timestamp **>= day_cum_offset["3"]**
(i.e. Mark67 prints on **tape day 3** only). **12** triggers kept vs **27** in v26.

Execution unchanged from `trader_v26.py` (joint gate, W=50_000, Q=6).
...
_SIG_PATH = Path(__file__).resolve().parent / "outputs" / "r4_v36_signals.json"
