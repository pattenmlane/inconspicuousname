"""
Round 4 — **v26 + end-of-tape-day flatten** (carry-risk control).

Same signals and lift logic as `trader_v26.py` (`r4_v26_signals.json`), with merged
`window_active` via `merged_ts_util`. Additionally: on **tape days 1 and 2** only,
if we hold **VEV_5300** long and local `timestamp >= DAY_END - PAD`, **flatten to
bid** even if a post-trigger window is still “active” in merged time (avoids
carrying inventory into the next calendar day where fills were losing in v26).

Day 3: no forced EOD flatten (last day — normal window exit only).
