"""
Round 4 — **EOD flatten tape day 2 only** (before roll to day 3).

Same as fixed `trader_v26.py` plus: on **tape day 2** only, if long **VEV_5300** and
local `timestamp >= 999_400` (PAD=500 before day end), flatten to bid. Skips
day-1 EOD so day-1 PnL matches v26 when windows do not extend past tape end.
...
class Trader:
    def run(self, state: TradingState):
        ...
        eod_flat = day == 2 and ts >= DAY_END - EOD_PAD and q > 0
