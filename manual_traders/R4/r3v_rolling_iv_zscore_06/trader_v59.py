"""
`trader_v50` with **forced post-trigger window W=40_000** (JSON has 50_000).

Tests whether shorter carry after Mark67 improves multi-day PnL (v28 was worse at 25k
with Q=6; Q=5 may differ).
"""
...
def _load_signals() -> tuple[list[int], int, dict[int, int]]:
    ...
    w = int(obj.get("window_ts", _WINDOW))
    if _FORCE_WINDOW is not None:
        w = int(_FORCE_WINDOW)
    _WINDOW = w
