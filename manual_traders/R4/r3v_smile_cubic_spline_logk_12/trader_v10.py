"""
Round 4 trader_v10: **trader_v8** + second **Phase 1** leg: **Mark 67 → Mark 22**
**buy-aggressive** extract prints.

Tape (`r4_trade_markouts_wide.csv`): buy-aggr extract **M67→M22** pooled mean mark_20
**~2.03** (n=75); day stability **weaker on day 2** (~1.19) vs ~2.36 / ~2.68 days 1/3.

Same mechanism as v8 for M67→M49: **omit extract asks** for **M67_LEG_EX_WIN** ticks
after the print, **only when not** `joint_gate_tight`. If **either** leg fires, the
suppression window extends to `max(until_49, until_22)` for that tick’s scheduling.

HYDRO M38→M14 unchanged from v8.
"""
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        if _m67_buyaggr_extract_vs_seller_now(state, "Mark 22"):
