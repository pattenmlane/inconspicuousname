"""
Round 4 trader_v11: **trader_v10** with **M67→M22** ask suppression **off on tape day 3**.

R4 backtest: `os.environ['PROSPERITY4_BACKTEST_DAY']` is the tape day index. **v10** added
+308 extract worse PnL on day1 vs **v8** but **−172** on day3 extract; this variant does not
set `_m6722_ex_until` on **day 3** (no M67→M22 stacking) while **M67→M49** + hydro match
v10. Days 1–2 behavior matches **v10**.
"""
...
def _tape_day() -> int | None:
    """Prosperity4 backtester sets PROSPERITY4_BACKTEST_DAY per tape file."""
    v = os.environ.get("PROSPERITY4_BACKTEST_DAY")
    if v is None or v == "":
        return None
    try:
        return int(v)
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        # Day-3 ablation: do not stack M67→M22 (see trader_v10 day3 extract vs v8).
        if _tape_day() != 3 and _m67_buyaggr_extract_vs_seller_now(state, "Mark 22"):
