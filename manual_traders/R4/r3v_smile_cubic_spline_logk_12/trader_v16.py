"""
Round 4 trader_v16: **trader_v14** + **Phase 2** hydro conservatism in the same **M01→M22**
offline burst ±500 window.

Tape (`r4_phase2_burst_window_mark20.csv`): **HYDROGEL_PACK** in-window mean mark_20 **−0.64**
(n=122) vs **−0.04** out (n=900). **Sim:** widen passive edge (3→5) and shrink clip (20→12)
when `burst_near` and session ≠ 3 (same session ablation as extract burst leg). If **both**
M38→M14 caution and burst apply, use **max** edge and **min** clip (stack conservatively).
"""
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        skip_burst_ask = (
            burst_near and not tight and _tape_session_day(ts) != BURST_EX_SKIP_OFF_SESSION
        )
        if skip_burst_ask:
        ...
        if hd is not None and hd.buy_orders and hd.sell_orders:
            ...
            burst_hydro = burst_near and _tape_session_day(ts) != BURST_EX_SKIP_OFF_SESSION
            if burst_hydro:
                td["_hydro_burst_caution_ticks"] = int(td.get("_hydro_burst_caution_ticks", 0)) + 1
            if hydro_caution and burst_hydro:
                he = max(HYDRO_CAUTION_EDGE, HYDRO_BURST_EDGE)
                hc = min(HYDRO_CAUTION_CLIP, HYDRO_BURST_CLIP)
            elif hydro_caution:
                he, hc = HYDRO_CAUTION_EDGE, HYDRO_CAUTION_CLIP
            elif burst_hydro:
                he, hc = HYDRO_BURST_EDGE, HYDRO_BURST_CLIP
            else:
                he, hc = HYDRO_BASE_EDGE, HYDRO_BASE_CLIP
