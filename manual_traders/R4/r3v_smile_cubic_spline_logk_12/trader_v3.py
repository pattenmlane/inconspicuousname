"""
Round 4 trader_v3: **v0 baseline** + **live** Mark01→Mark22 basket detection from tape.

At each decision tick, if `state.market_trades` lists **≥4** prints with buyer **Mark 01**
and seller **Mark 22** at the **current** timestamp (same definition as Phase 1 burst
orchestration on that ts), treat as **burst_now** and apply the same **VEV_5300**
conservative nudge as trader_v2.

If no market trades are exposed at this ts (empty dict), fall back to offline
±500 windows from `r4_burst_events.csv` (same as v2) so behavior matches v2 on
environments without tape in state.

HYDROGEL_PACK unchanged from v0.
"""
...
# Offline fallback: centers from analysis_outputs/r4_burst_events.csv (R4 days 1–3).
...
def _burst_window_offline(ts: int, day: int) -> bool:
    return _near_sorted_ts(ts, BURST_CENTERS.get(day, []), BURST_WIN)


def _m01_m22_print_count_now(state: TradingState) -> int:
    """Count Mark 01 → Mark 22 prints at current timestamp (all products in market_trades)."""
    n = 0
    ts = int(state.timestamp)
    mt = state.market_trades
    if not mt:
        return 0
    for trades in mt.values():
        for t in trades:
            if int(t.timestamp) != ts:
                continue
            if t.buyer == "Mark 01" and t.seller == "Mark 22":
                n += 1
    return n


def burst_risk(state: TradingState) -> tuple[bool, str]:
    """
    Returns (apply_vev5300_burst_tweak, reason) for logging in traderData counters.
    Live basket burst: >=4 M01→M22 prints this tick. Else offline ±500 fallback.
    """
    day = _tape_day()
    ts = int(state.timestamp)
    c = _m01_m22_print_count_now(state)
    if c >= BURST_MIN_PRINTS:
        return True, "live"
    if _burst_window_offline(ts, day):
        return True, "offline"
    return False, "none"
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        burst, burst_src = burst_risk(state)
        if burst:
            td["_burst_ticks"] = int(td.get("_burst_ticks", 0)) + 1
            td["_burst_last_src"] = burst_src
