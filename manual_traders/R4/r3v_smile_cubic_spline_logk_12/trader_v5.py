"""
Round 4 trader_v5: **trader_v4** (Phase 1 hydro M38→M14 post-print caution) **plus**
**Phase 2** Mark01→Mark22 basket burst conditioning on **VEV_5300** (same as trader_v3).

Burst risk: ≥4 Mark 01→Mark 22 prints at current `state.market_trades` timestamp, **or**
offline ±500 ts window from `r4_burst_events.csv` (R4 days 1–3). In burst window,
widen VEV_5300 take/make edges slightly and shrink MM/take sizes.

Extract quoting unchanged from v0.
"""
...
def burst_risk(state: TradingState) -> tuple[bool, str]:
    day = _tape_day()
    ts = int(state.timestamp)
    if _m01_m22_print_count_now(state) >= BURST_MIN_PRINTS:
        return True, "live"
