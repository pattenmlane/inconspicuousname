"""
Round 4 trader_v6: **trader_v4** + **Phase 1** extract leg after **Mark 67 → Mark 49**
**buy-aggressive** print on **VELVETFRUIT_EXTRACT** (tape: positive mean mark_20_same).

Detection: at current `state.market_trades` timestamp, any **EXTRACT** trade with
buyer Mark 67, seller Mark 49, and **price >= ask** (same aggr rule as Phase 1).

For **M6749_EX_WIN** ticks, add **+1** to the extract fair anchor used for quoting
(`fi` in v0 terms) — passive bids lift slightly and asks require a higher anchor.

**Phase 3 interaction:** Phase 3 tape shows **lower** mean mark_20 for this pair when
**joint_gate_tight** — we apply the **+1 bias only when the gate is NOT tight**
(`not joint_gate_tight(state)` at quote time), so we do not stack the conflicting signal.

HYDRO M38→M14 caution unchanged from v4.
"""
...
def _m67_m49_buyaggr_extract_now(state: TradingState) -> bool:
    """Mark 67 buy-aggressive vs Mark 49 on extract at this timestamp."""
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        if ts <= until6749 and not tight:
            fi += M6749_FAIR_BIAS
            td["_m6749_ex_bias_ticks"] = int(td.get("_m6749_ex_bias_ticks", 0)) + 1
