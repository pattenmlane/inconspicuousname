"""
Round 4 trader_v7: **trader_v4** + **Phase 1** Mark67→Mark49 buy-aggr extract cue (**bid-only**).

Same detection and **250-tick** window as **trader_v6**, same **Phase 3** guard
(bias only when **not** `joint_gate_tight`).

**Difference from v6:** v6 added **+1** to `fi` for both bid and ask anchors; under
`--match-trades worse` that hurt day-3 extract PnL. v7 adds **+1** only to the
**bid** anchor (`fair_i - edge` → effectively one tick more aggressive bid skew);
**ask** side matches v4 (no lifted offer into adverse fills).

HYDRO M38→M14 unchanged from v4.
"""
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        if ts <= until6749 and not tight:
            bid_add = M6749_BID_TICK_ADD
            td["_m6749_ex_bid_bias_ticks"] = int(td.get("_m6749_ex_bid_bias_ticks", 0)) + 1

        ex_pos = state.position.get(EXTRACT, 0)
        result[EXTRACT].extend(
            self._quote_extract(ex, ex_pos, fi, ex_e, max_lot=ex_l, bid_tick_add=bid_add)
        )
    ...
    def _quote_extract(
        ...
        lim = LIMITS[EXTRACT]
        cap = min(max_lot, lim)
        bid_p = min(bbi + 1, fair_i - edge + bid_tick_add)
