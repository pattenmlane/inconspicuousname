"""
Round 4 trader_v8: **trader_v4** + **Phase 1** extract **ask suppression** after
**Mark 67 → Mark 49** **buy-aggressive** print on **VELVETFRUIT_EXTRACT**.

Tape (Phase 1): positive forward mid drift after this leg — hypothesis for sim: avoid
**passive offers** into a rising path (adverse selection on the sell side under
worse-fill), while keeping **bids** unchanged.

Window: **M6749_EX_WIN** ticks after the print. **Phase 3:** suppress asks only when
**not** `joint_gate_tight` (same guard as v6/v7 — weaker tape edge when tight).

Detection: `state.market_trades` EXTRACT at current ts, buyer Mark 67, seller Mark 49,
price >= ask1.
