#!/usr/bin/env python3
"""
Extract trades, buyer_agg only: residual = fwd20_mid_delta - mean(buyer,seller,EXTRACT)
from pooled buyer_agg baseline cell (same Phase-1 forward_mid / snap_at).

Stratify residual by joint-tight (5200+5300<=2) at trade ts. Tests whether 'informed'
buyer_agg edge vs cell mean differs in Sonic tight regime.
"""
...
def main() -> None:
    ...
    out = {
        "method": "buyer_agg extract only; baseline = mean fwd20 per (buyer,seller,EXTRACT) if n>=5 in cell",
    ...
    pth = OUT / "r4_residuals_buyer_agg_joint_gate.json"
