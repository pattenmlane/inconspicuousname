"""
Round 4 post-Phase3 — Sonic joint gate + *wide* extract book (ablation vs v6 tight extract).

Passive buy extract 1 tick inside BBO when 5200+5300 L1<=2 AND extract L1 spread >= 3.
Tests whether edge concentrates when extract is not ultra-tight (inclineGod spread angle).
"""
...
def _extract_wide(depths) -> bool:
    d = depths.get(U)
    if d is None:
        return False
    _, _, sp = _l1_spread(d)
    return sp is not None and sp >= MIN_EXT_SPREAD
...
class Trader:
    def run(self, state: TradingState):
        ...
        if not _joint_tight(depths) or not _extract_wide(depths):
            store[_TD_KEY] = st
