"""
Round 4 post-Phase3 — Sonic joint gate + extract L1 spread in {2, 3} only.

Passive buy extract 1 tick inside BBO (Q=4). Between v2 (no extract filter) and v6
(tight <=2) / v10 (wide >=3): mid-width extract microstructure slice.
"""
...
def _extract_spread_midwidth(depths) -> bool:
    d = depths.get(U)
    if d is None:
        return False
    _, _, sp = _l1_spread(d)
    return sp is not None and 2 <= sp <= 3
...
class Trader:
    def run(self, state: TradingState):
        ...
        if not _joint_tight(depths) or not _extract_spread_midwidth(depths):
            store[_TD_KEY] = st
