"""
Round 4 iteration 13 — Relaxed Sonic gate (sensitivity to threshold).

Passively buy VELVETFRUIT_EXTRACT 1 tick inside BBO (Q=4) when VEV_5200 and VEV_5300
L1 spreads are both **<=3** (vs v2’s <=2). Ablation for how strict the surface gate must be.
"""
...
def _joint_gate_relaxed(depths) -> bool:
    a = depths.get(S5200)
    b = depths.get(S5300)
...
class Trader:
    def run(self, state: TradingState):
        ...
        if not _joint_gate_relaxed(depths):
            store[_TD_KEY] = st
