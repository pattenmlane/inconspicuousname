"""
Round 4 post-Phase3 — Sonic joint gate + inventory skew on VELVETFRUIT_EXTRACT only.

Same joint tight (5200+5300 L1<=2) as v2. When long extract builds past INV_SOFT, prefer
passive sells one tick inside the spread; when short past -INV_SOFT, passive buys to cover;
otherwise default to the v2 passive buy lean when flat/small long.

Counterparty-aware tape motivation: avoid one-sided inventory when providing liquidity in
tight surface regimes (Phase 2 execution theme).
"""
...
def _passive_sell_px(bb: int, ba: int) -> int:
    if ba - bb >= 2:
...
class Trader:
    def run(self, state: TradingState):
        ...
        if p_u > INV_SOFT:
            ...
            if q <= 0:
                return {}, 0, json.dumps(store, separators=(",", ":"))
            px = _passive_sell_px(bb, ba)
            return {U: [Order(U, px, -q)]}, 0, json.dumps(store, separators=(",", ":"))
