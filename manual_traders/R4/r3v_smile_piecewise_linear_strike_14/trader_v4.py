"""
Round 4 post-Phase3 — Sonic joint gate + position-aware buy size only (no passive sells).

Same as trader_v2 when |extract position| is small; reduce passive buy clip when inventory
builds (|pos|>=40 -> Q=2, |pos|>=80 -> Q=1) to cap risk without fading the tape edge with
asks (trader_v3 showed passive sells hurt).
"""
...
TH = 2
Q0 = 4
_TD_KEY = "r4_gate_pos_clip"


def _buy_qty(p_u: int) -> int:
    a = abs(p_u)
    if a < 40:
        return Q0
    if a < 80:
