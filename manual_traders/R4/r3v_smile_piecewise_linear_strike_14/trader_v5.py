"""
Round 4 post-Phase3 — Sonic joint gate + size tilt when flat.

Same passive buy 1 tick inside BBO as v2, but Q=6 when |extract position|<=FLAT_THR else Q=4.
Hypothesis: population tight-regime edge scales with clip when not near limit (ablation vs v2).
"""
...
class Trader:
    def run(self, state: TradingState):
        ...
        p_u = int(pos.get(U, 0))
        q = Q_FLAT if abs(p_u) <= FLAT_THR else Q_DEFAULT
        if p_u >= LIM_U - q:
            store[_TD_KEY] = st
            return {}, 0, json.dumps(store, separators=(",", ":"))
