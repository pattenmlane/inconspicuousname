"""
Round 4 post-Phase3 — Triple microstructure: Sonic joint gate + tight extract book.

Passive buy extract one tick inside BBO only when VEV_5200 and VEV_5300 L1 spreads <=2
AND extract L1 spread <=2 (inclineGod-style surface cleanliness). Q=4. Ablation vs v2.
"""
...
def _extract_tight(depths) -> bool:
    d = depths.get(U)
    if d is None:
        return False
    _, _, sp = _l1_spread(d)
    return sp is not None and sp <= TH
...
class Trader:
    def run(self, state: TradingState):
        ...
        if not _joint_tight(depths) or not _extract_tight(depths):
            store[_TD_KEY] = st
