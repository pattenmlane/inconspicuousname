"""
Round 4 post-Phase3 — Sonic joint gate + *bad hour* blacklist (ablation vs v8 whitelist).

Same passive buy as v2 when joint tight, but **no** orders when hour is in BAD_HOURS:
pooled Welch had mean_tight < mean_loose with p<0.05 on extract fwd20 → hours {1, 2, 10}
(r4_hour_joint_gate_extract_fwd20.json).
"""
...
TH = 2
Q = 4
BAD_HOURS = frozenset({1, 2, 10})
_TD_KEY = "r4_gate_bad_hours"
...
class Trader:
    def run(self, state: TradingState):
        ...
        hour = (ts // 10000) % 24
        if hour in BAD_HOURS or not _joint_tight(depths):
            store[_TD_KEY] = st
