"""
Round 4 trader_v3 — child of v2: same Sonic joint gate and full VEV + extract
one-tick MM, but only quote wing strikes (not 5200/5300) when that symbol's
top-of-book spread is at most _MAX_WING_SPREAD ticks. Reduces MM on wide
one-sided books while keeping core surface active under the gate.
...
def _quote_vev(sym: str, depths: dict[str, OrderDepth]) -> bool:
    if sym in (G5200, G5300):
        return True
    sp = spread(depths.get(sym))
    if sp is None:
        return False
    return sp <= _MAX_WING_SPREAD
...
class Trader:
    def run(self, state: TradingState):
        ...
        lim_u = LIMITS.get(U, 200)
        oo_u = one_tick_mm(U, depths.get(U), int(pos.get(U, 0)), lim_u, _MM_CORE)
        if oo_u:
            result[U] = oo_u

        for sym in VEV_SYMS:
            if not _quote_vev(sym, depths):
                continue
