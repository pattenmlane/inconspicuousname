"""
Round 4 trader_v7 — ablation: Sonic joint gate then one-tick MM on
VELVETFRUIT_EXTRACT, VEV_5200, VEV_5300, and HYDROGEL_PACK only (no wing VEVs).

Isolates hydro + gate legs + extract vs v5 full VEV surface (all strikes).
"""
...
def _mm_size(sym: str) -> int:
    if sym == HY:
        return _MM_HY
    return _MM_EX if sym == U else _MM_CORE
