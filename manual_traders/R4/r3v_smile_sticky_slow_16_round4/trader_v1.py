"""
Round 4 — Phase 3: **Sonic joint gate** on live book + Phase 2 counterparty hooks.

- **Tight gate:** quote **VELVETFRUIT_EXTRACT** (mid\u00b12) only when **VEV_5200** and **VEV_5300** both have
  top spread (ask\u2212bid) **\u2264 2** (same convention as analyze_vev_5200_5300_tight_gate_r3.py / Phase 3 tape).
- **HYDROGEL_PACK** and all **VEV_*** still quoted every tick (gate is about hedging surface for extract).
- Same **market_trades** hooks as trader_v0: skip extract ask on Mark38 aggressive buy; +2 extract bid on Mark67 aggressive buy (when extract is quoted).

Phase 3 tape: pooled extract K=20 fwd mean **higher** when gate tight vs loose (see r4_phase3_gate_fwd_extract.json).
"""
