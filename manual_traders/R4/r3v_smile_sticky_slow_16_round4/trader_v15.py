"""
Round 4 — **v12 + milder tight ask on tape day 1**.

- **v12**: tight extract ask **+3** on tape days **1\u20132**, **+2** on tape day **3** (bid \u22121).
- **v15**: **tape day 1** uses ask **+2** (same as day3); **tape day 2** keeps **+3**; **tape day 3** **+2**.
  Hypothesis: day1 extract PnL under v12 may still be paying for overly passive **+3** asks vs day-open level.
- Sizes and all other logic match **v12** (tight extract size 12 both sides).
"""
...
HALF = 2
HALF_U_TIGHT_BID = 1
HALF_U_TIGHT_ASK_WIDE = 3
HALF_U_TIGHT_ASK_TIGHT = 2
...
class Trader:
    def run(self, state: TradingState):
        ...
        if tape_day == 2:
            half_tight_ask = int(HALF_U_TIGHT_ASK_WIDE)
        else:
            half_tight_ask = int(HALF_U_TIGHT_ASK_TIGHT)
