"""
Round 4 — **v17 + stronger Mark 67 ask widen** (ablation on Phase-1 lift signal).

- **v17** when Sonic **tight** and **Mark 67** **aggressive buy** on extract: add **+1** to tight **ask** half, cap **4**.
- **v18**: same, add **+2**, cap **5** (more passive offer when M67 hits the offer), keeping v12 day schedule
  and Mark38/67 else unchanged.
"""
...
class Trader:
    def run(self, state: TradingState):
        ...
        if tight and m67_buy:
            base_ask = min(base_ask + int(M67_ASK_BUMP), int(HALF_U_TIGHT_ASK_MAX))
