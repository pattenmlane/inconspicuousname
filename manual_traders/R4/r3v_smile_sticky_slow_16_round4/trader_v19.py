"""
Round 4 — **v17 + Mark 67 widen extract bid** (complement to ask-bump; tests orthogonality).

- **v17**: tight extract bid **mid\u22121**; on M67 aggressive buy, ask half **+1** (cap 4); bid size +2.
- **v19**: same as v17, plus when **Sonic tight** and **M67 aggressive buy**, use bid half **2** (i.e. **mid\u22122** on bid)
  instead of **1** \u2014 do **not** lean the **bid** into someone lifting the offer (Phase-1: M67 buy \u2192 positive extract fwd; we test
  **less aggressive bidding** on those ticks). Ask schedule unchanged from v17.
- If this fails, it parallels **v8** (wide bid hurt) but **only** on M67 trigger ticks, not all tight time.
"""
...
class Trader:
    def run(self, state: TradingState):
        ...
        if tight and m67_buy:
            base_ask = min(base_ask + 1, int(HALF_U_TIGHT_ASK_MAX))
