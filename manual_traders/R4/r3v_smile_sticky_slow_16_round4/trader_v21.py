"""
Round 4 — **v19 + Mark 55 aggressive-sell halve extract offer size** (alternative to v20 ask-widen).

- **v20** added **+1** ask half on M55 sell and **hurt** `worse` PnL vs v19.
- **v21**: same prices as **v19**; when **Sonic tight** on extract and **Mark 55** **aggressive sell** (`seller==Mark 55`,
  `price<=bid`), set extract **ask quantity** to **max(1, floor(q_sell/2))** after the usual **base_sz** cap (does not
  touch bid size or M67 bid-widen). Tests **inventory reduction** on sell pressure instead of price skew.
"""
    ...
    def run(self, state: TradingState):
        ...
        for sym in [H, U, *VEVS]:
            ...
            if sym == U and m38_buy:
                q_sell = 0
            if sym == U and tight and m55_sell and q_sell > 0:
                q_sell = max(1, int(q_sell // 2))
