"""
Round 4 — Mark55 + Sonic gate (v4) with **position-size scaled** by rolling joint-tight prevalence.

Tape (`r4_p7_mark55_fwd20_by_roll_tertile.csv`): on day 3, **high** roll-tight tertile has the
**highest** mean fwd20 — so we do **not** zero out in saturation; we **reduce clip** when
`roll_tight_rate` is high to limit execution risk while still participating.

clip = round( CLIP_MAX * (1 - SAT_GAIN * clamp((roll - ROLL_LO)/(ROLL_HI - ROLL_LO), 0, 1)) ),
clamped to [CLIP_MIN, CLIP_MAX]. Per-day `tight_hist` reset when `backtest_day` changes.
"""
