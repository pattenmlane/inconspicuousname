"""
Loose-book extract trades only: among Mark 55 **aggressive** prints, split aggressive **buy**
vs **sell**; mean extract fwd_5 / fwd_20 (same K-row definition as Phase 1).

Also rows with loose + no M55 touch for baseline reference.
"""
...
def main() -> None:
    gate = pd.concat([gate_for_day(d) for d in DAYS], ignore_index=True)
    xf = extract_forwards()
    ...
    tr["aggr_buy"] = tr["price"] >= ask1
    tr["aggr_sell"] = tr["price"] <= bid1
    tr = tr.merge(gate, on=["day", "timestamp"], how="left")
    tr = tr.merge(xf, on=["day", "timestamp"], how="left")
    loose = tr[tr["tight"] == False].copy()
    loose["m55_buy"] = (loose["buyer"] == "Mark 55") & loose["aggr_buy"]
    loose["m55_sell"] = (loose["seller"] == "Mark 55") & loose["aggr_sell"]
    loose["bucket"] = "other_loose"
    loose.loc[loose["m55_buy"] & ~loose["m55_sell"], "bucket"] = "m55_aggr_buy"
    loose.loc[loose["m55_sell"] & ~loose["m55_buy"], "bucket"] = "m55_aggr_sell"
    loose.loc[loose["m55_buy"] & loose["m55_sell"], "bucket"] = "m55_buy_and_sell_same_row"

    by_day = (
        loose.groupby(["day", "bucket"])
        .agg(n=("fwd_5", "count"), m5=("fwd_5", "mean"), m20=("fwd_20", "mean"))
        .reset_index()
    )
    by_day.to_csv(OUT / "r4_extract_loose_m55_side_fwd_by_day.csv", index=False)

    pooled = (
        loose.groupby("bucket")
        .agg(n=("fwd_5", "count"), m5=("fwd_5", "mean"), m20=("fwd_20", "mean"))
        .reset_index()
    )
    pooled.insert(0, "scope", "pooled")
    pooled.to_csv(OUT / "r4_extract_loose_m55_side_fwd_pooled.csv", index=False)
    print("wrote", OUT / "r4_extract_loose_m55_side_fwd_pooled.csv")
