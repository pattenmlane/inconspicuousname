"""
Round 4 — v11 but skip loose soft extract MM only on Mark 55 **aggressive sell** (not buy).

Tape hypothesis (iteration 12 analysis): under loose gate, M55 aggressive **buys** may carry
more positive fwd than aggressive **sells**; v10/v11 skip both — this variant isolates sell-side.
"""
...
def _mark55_aggressive_sell_extract_tick(depths: dict[str, OrderDepth], mt: dict[str, list[Trade]]) -> bool:
    d = depths.get(U)
    if d is None:
        return False
    bb, ba = _book(d)
...
class Trader:
    def run(self, state: TradingState):
        ...
        if joint:
            ...
            skip_u_soft = _mark55_aggressive_sell_extract_tick(depths, mt)
            if not skip_u_soft:
                orders.update(_emit_u(depths, pos, SIZE_U_SOFT))
            td = {"joint": False, "mode": "u_soft_only", "skip_m55_sell": bool(skip_u_soft)}
