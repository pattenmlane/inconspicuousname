"""
Round 4 — v11 + **tight-gate** counterparty damp on extract only.

When joint Sonic gate is **on** and this tick has Mark 55 **aggressive** on extract
(same detection as v10/v11), quote extract at **SIZE_U_TIGHT_M55=12** instead of 18;
VEV and hydro clips unchanged (tests three-way gate × M55 × extract inventory risk).

Loose path identical to v11 (soft U 14, skip on any M55 aggressive extract tick).
"""
...
class Trader:
    def run(self, state: TradingState):
        ...
        orders: dict[str, list[Order]] = {}
        m55 = _mark55_aggressive_extract_tick(depths, mt)
        if joint:
            u_sz = SIZE_U_TIGHT_M55 if m55 else SIZE_U
            orders.update(_emit_u(depths, pos, u_sz))
            orders.update(_emit_hydro(depths, pos, SIZE_H))
            orders.update(_emit_vevs(depths, pos, VOUCHERS_ALL, SIZE_VEV))
            td = {"joint": True, "mode": "full", "u_sz": u_sz, "m55_u": bool(m55)}
        else:
            skip_u_soft = m55
            if not skip_u_soft:
