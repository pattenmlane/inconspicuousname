"""
Round 4 — v15 occupancy (both 5200/5300 spr <= 3) + **Mark55 skip on extract only in wing band**.

**Core:** both spreads <= 2 — full surface (same as v11/v15 joint), no M55 skip on U.

**Wing band:** both <= 3 but **not** core (one or both legs at 3-tick wide) — quote all VEVs + hydro
like v15, but **skip extract** quotes this tick if Mark55 aggressive on extract (same detection
as v10/v11 loose path). Tests counterparty filter where Sonic *tight* thesis is weakest.

**Loose:** both > 3 — v11 loose (soft U=14, M55 skip on extract).
"""
...
class Trader:
    def run(self, state: TradingState):
        ...
        if joint_core:
            ...
            orders.update(_emit_hydro(depths, pos, SIZE_H))
            orders.update(_emit_vevs(depths, pos, VOUCHERS_ALL, SIZE_VEV))
            skip_u = _mark55_aggressive_extract_tick(depths, mt)
            if not skip_u:
                orders.update(_emit_u(depths, pos, SIZE_U))
            td = {"mode": "wing_full_vev_hydro_u_cond_m55", "skip_m55_u": bool(skip_u)}
        else:
            skip_u_soft = _mark55_aggressive_extract_tick(depths, mt)
            if not skip_u_soft:
                orders.update(_emit_u(depths, pos, SIZE_U_SOFT))
            td = {"mode": "u_soft_only", "skip_m55": bool(skip_u_soft)}
