"""
Round 4 — v17 with larger **VEV** clip in **wing_only** band only (SIZE_VEV_WING=18 vs 14).

Core and loose paths unchanged from v17. Tests whether wing-band VEV one-tick MM was
size-saturated at 14 (as v5–v7 on tight path) or has slack when gate legs are wider.
"""
...
class Trader:
    def run(self, state: TradingState):
        ...
        if joint_core:
            ...
            orders.update(_emit_hydro(depths, pos, SIZE_H))
            orders.update(_emit_vevs(depths, pos, VOUCHERS_ALL, SIZE_VEV_WING))
            skip_u = _mark55_aggressive_extract_tick(depths, mt)
            if not skip_u:
                orders.update(_emit_u(depths, pos, SIZE_U))
            td = {"mode": "wing_vev18", "skip_m55_u": bool(skip_u)}
        else:
            skip_u_soft = _mark55_aggressive_extract_tick(depths, mt)
            if not skip_u_soft:
                orders.update(_emit_u(depths, pos, SIZE_U_SOFT))
