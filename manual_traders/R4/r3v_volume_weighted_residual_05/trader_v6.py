"""
Round 4 — **v5 + passive-style exits** (Mark 67 buy-aggressive U × Sonic gate).

Parent: `trader_v5.py`. v5 lost ~**42** over 3 days largely from **paying touch ask** on exits
and **ASK_LIFT_Q** on thin U books.

Changes vs v5:
- **Take profit:** sell at **improved ask** `max(bb+1, ba-1)` when U spread **≥ 2** (strictly
  below touch so `--match-trades worse` can match incoming buys above our limit); at spread **1**
  keep **touch ask** (same as improved = ba).
- **Smaller lift:** `ASK_LIFT_Q = 1` (was 3) on spread-1 entries.
- **Fewer / smaller entries:** `ENTRY_Q = 5`, slightly wider **PROFIT** band before scaling out.

Same infrastructure: raw `market_trades` before `run()` (`test_runner.py`).
"""
...
def _improved_ask(bb: int, ba: int) -> int:
    """Sell limit strictly below touch when possible (worse-match sells need trade price > limit)."""
    if ba - bb >= 2:
