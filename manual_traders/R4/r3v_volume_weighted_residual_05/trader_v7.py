"""
Round 4 — **v6 + gate-off liquidation** (Mark 67 buy-aggressive U × Sonic).

Parent: `trader_v6.py`. v6 analysis noted **flatten-at-bid** when the joint gate opens as a
likely loss source (selling long inventory at the touch bid).

Change vs v6:
- When **not** `tight` and **long** `U`: if U spread **≥ 2**, liquidate at **improved ask**
  (same `_improved_ask` as take-profit) instead of the touch **bid**; if spread **== 1**,
  keep **bid** dump (only worse-fill path for full exit on a 1-tick book).

Entries / in-gate take-profit unchanged from v6.
"""
