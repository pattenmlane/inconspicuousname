"""Round 4 trader v10 — v8 with **larger Tier-A aggression** (more lift, more room).

Same passive + Sonic + VEV_4000 logic as `trader_r4_v8`. Tier-A aggressive buy
at best ask uses **qty 3** (capped by buy room) and requires **buy room >= 8**
before crossing, so passive+aggr still passes limit check cleanly.

`AGGR_MAX_POSITION` stays 160 like v8 (v9 showed 100 never binds on worse path).

See `results.json` iteration 12.
"""
