"""Round 4 trader v13 — v10 + **extract L1 spread gate** on Tier-A aggression.

Same as `trader_r4_v10` (buy at ask qty 3, min buy room 8, cooldown 25) but the
aggressive buy-at-ask fires only when **VELVETFRUIT_EXTRACT** top-of-book spread
**(ask - bid) <= 2**. Passive quotes unchanged.

Rationale: paying the spread is cheaper in ticks when the extract book is tight;
skip aggression on wide extract prints even if Sonic gate is tight.

See `results.json` iteration 15.
"""
    ...
    def run(self, state: TradingState):
        ...
        if od_u is not None and e_ema is not None:
            ...
            if bb is not None and ba is not None:
                ...
                ext_sp = _spread(od_u)
                spread_ok = ext_sp is not None and ext_sp <= EXTRACT_SPREAD_MAX_FOR_AGGR
                data["extract_spread"] = ext_sp
                ...
                if (
                    ...
                    and spread_ok
                    and cooldown_ok
                    and pos <= AGGR_MAX_POSITION
                    and br >= AGGR_MIN_BUY_ROOM
                ):
