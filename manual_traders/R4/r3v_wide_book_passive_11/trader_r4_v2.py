"""Round 4 trader v2 — trader_r4_v1 + **Mark 67 → Mark 22** on **VELVETFRUIT_EXTRACT** (tape).

`TestRunner` now fills `state.market_trades` at each timestamp (see
`imc-prosperity-4-backtester/prosperity4bt/test_runner.py`) so `buyer` / `seller`
from the tape are visible before `Trader.run`.

When **Sonic joint gate** is tight **and** this tick includes a tape print
`Mark 67` → `Mark 22` on extract, increase extract **buy** clip by `SIGNAL_BUMP`
(capped at `EXTRACT_CAP`). Same quote prices as v1.

If backtest `tradeHistory` matches v1, the tape signal rarely coincides with
ticks where buy room exceeds v1's tight size (see offline counts in analysis).
"""
