"""Round 4 trader v3 — v1 + Tier-A **execution** when tape fires.

**Sonic gate:** same as v1 (VEV_5200 & VEV_5300 L1 spread <= 2).

**Tape signal:** `state.market_trades` is populated before `run` (prosperity4bt test_runner);
if any `VELVETFRUIT_EXTRACT` print has buyer **Mark 67** and seller **Mark 22**, set `sig`.

**v3 vs v1:** when `tight` and `sig`, split buy room: up to `TOUCH_BUY_Q` at **best bid `bb`**
first, then the usual `bb+1` bid for `min(ex_buy, br - q_touch)`. Non-signal ticks match v1.
Under worse matching on this tape, merged `tradeHistory` stayed **identical** to v1 (see
`results.json` iteration 5) — the extra touch tier did not change realized fills vs v1's
single bid path in this run.

Sell side unchanged from v1.
"""
...
EXTRACT_SIZE_WIDE = 4
# Primary buy clip bump (same intent as v2; still capped by br)
SIGNAL_BUMP = 2
EXTRACT_CAP = 12
TOUCH_BUY_Q = 4
...
class Trader:
    def run(self, state: TradingState):
        ...
        ex_buy = EXTRACT_SIZE_TIGHT if tight else EXTRACT_SIZE_WIDE
        ex_sell = ex_buy
        if tight and sig:
            ex_buy = min(EXTRACT_CAP, ex_buy + SIGNAL_BUMP)
        ...
        if od_u is not None and e_ema is not None:
            ...
            if bb is not None and ba is not None:
                ...
                if br > 0 and bid_px > 0 and bid_px < ba:
                    if tight and sig and bb < bid_px and int(bb) > 0 and int(bb) < ba:
                        q_touch = min(TOUCH_BUY_Q, br)
                        q_pri = min(ex_buy, max(0, br - q_touch))
                        if q_touch > 0:
                            orders[UNDER].append(Order(UNDER, int(bb), q_touch))
                        if q_pri > 0:
                            orders[UNDER].append(Order(UNDER, bid_px, q_pri))
                    else:
                        orders[UNDER].append(Order(UNDER, bid_px, min(ex_buy, br)))
                if sr > 0 and ask_px > bb:
                    orders[UNDER].append(Order(UNDER, ask_px, -min(ex_sell, sr)))
        ...
        if tight:
            ...
            if od4 is not None:
                ...
                if sp is not None and sp >= 15:
                    bb4, ba4 = _best(od4)
                    if bb4 is not None and ba4 is not None:
                        bid_px = bb4 + 4
                        ask_px = ba4 - 4
                        if bid_px < ask_px:
