"""Round 4 trader v4 — v1 + **Tier-A price skew** (Mark 67 → Mark 22 on extract, Sonic tight).

When `tight` and tape shows Mark **67** buying extract from Mark **22**:
  - **Bid** one tick more aggressive than v1: `bb+2` vs `bb+1` (still capped vs fair floor).
  - **Ask** one tick **higher** vs v1 (lighter offer / long lean): `ceil(fair)+2` floor vs `+1`.
  - **Asymmetric size:** buy clip +1 (cap 12); sell clip max(2, base−2) so we lean inventory long.

Non-signal ticks match v1 quote math and symmetric sizes.
"""
...
class Trader:
    def run(self, state: TradingState):
        ...
        ex_sell = ex_buy
        if tight and sig:
            ex_buy = min(EXTRACT_CAP, ex_buy + SIGNAL_BUY_BUMP)
            ex_sell = max(2, ex_sell - SIGNAL_SELL_TRIM)
        ...
        if od_u is not None and e_ema is not None:
            ...
            if bb is not None and ba is not None:
                ...
                fair = float(e_ema)
                skew = 0
                bid_off = SIGNAL_BID_OFF if (tight and sig) else 1
                fair_ask_extra = 1 + (SIGNAL_ASK_FAIR_EXTRA if (tight and sig) else 0)
                bid_px = min(bb + bid_off, int(math.floor(fair)) - 1 - skew)
                ask_px = max(ba - 1, int(math.ceil(fair)) + fair_ask_extra - skew)
                br = lim - pos
                sr = lim + pos
