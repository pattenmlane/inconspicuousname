"""Round 4 trader v5 — **bid-only** Tier-A skew (ablation vs v4).

Same as v4 on signal detection (Sonic tight + Mark67→Mark22 extract tape).

**vs v4:** keep **v1 ask path** (`max(ba-1, ceil(fair)+1)`) and **v1 sell sizing** (no sell trim).
**vs v1:** on signal only — bid `bb+2` (vs `bb+1`) and buy clip +1 capped at 12.

Hypothesis (from analysis iter 6): ask lift + sell trim in v4 hurt worse PnL; isolate bid aggression.
"""
