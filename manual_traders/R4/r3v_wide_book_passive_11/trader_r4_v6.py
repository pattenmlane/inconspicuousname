"""Round 4 trader v6 — **ask-only** Tier-A skew (ablation vs v4 / complement to v5).

Tier-A: Sonic tight + Mark67→Mark22 on VELVETFRUIT_EXTRACT tape.

**vs v1:** on signal only — ask anchor `max(ba-1, ceil(fair)+2)` instead of `+1`.
**vs v4:** no bid bb+2, no buy bump, no sell trim (isolate **ask lift** effect).
"""
