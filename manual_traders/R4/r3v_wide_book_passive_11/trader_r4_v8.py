"""Round 4 trader v8 — v1 + **aggressive extract** on Tier-A counterparty print.

Tier-A (Phase 3): Sonic joint gate tight **and** this tick's tape on
VELVETFRUIT_EXTRACT includes buyer **Mark 67** / seller **Mark 22**
(`state.market_trades`, populated before `run` in test_runner).

Passive quoting stays v1. Additionally, when signal fires we place a small
**buy at best ask** (cross the spread) to lean into the favorable short-horizon
drift seen offline — sized/cooled so `__enforce_limits` does not drop the
whole product's orders.

See `results.json` iteration 8 for worse vs all vs v1.
"""
...
# Aggressive leg: keep small; reserve room so passive + aggr <= buy room.
AGGR_BUY_Q = 2
AGGR_MIN_BUY_ROOM = 6
