"""
Round 4 v10 — same mechanics as **v1** (lag+100 taker buy 1 lot extract; act-time Sonic gate),
but signals are **pre-filtered on tape**: only Mark67 buy-aggr extract prints where
**joint 5200/5300 tight already holds at fire_ts = print_ts+100** (see
r4_filter_mark67_signals_tight_at_fire.py).

Hypothesis: v1’s act gate at execution often fails; tape filter may concentrate fires
where the book is still tight one step ahead, improving fill rate or PnL per fill under worse.
"""
...
_SIG_PATH = (
    ...
    / "outputs"
    / "phase3"
    / "signals_mark67_buy_aggr_extract_tight_at_fire.json"
)
_FIRE: set[int] = set()
