"""
Round 4 — trader_v9 + (1) inventory skew on VELVETFRUIT_EXTRACT bid/ask vs EMA,
(2) small aggressive buy at best ask when extract BBO tight AND Mark67 tape key hits
(Phase 1: aggressive buy on extract → positive short-horizon fwd mid).
"""
...
INV_SKEW_STEP = 25
INV_SKEW_MAX = 8
# Extract: gentler step (larger inventory before skew) so we do not fight v9 voucher path too hard.
EXTRACT_INV_STEP = 40
EXTRACT_INV_MAX = 3
M67_AGGR_BUY_Q = 22
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        if tight and m67_sig and ba is not None and pu < lim_u - 5:
