"""
Round 4 — trader_v11 hybrid Mark67 take: when extract tight + M67 key, aggressive buy at ask
with qty 8 if Sonic joint is loose, else qty 22 when joint tight (v11 used 22 only under joint).
"""
...
EXTRACT_INV_STEP = 40
EXTRACT_INV_MAX = 3
M67_AGGR_BUY_Q_LOOSE = 8
M67_AGGR_BUY_Q_JOINT = 22
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        if tight and m67_sig and ba is not None and pu < lim_u - 5:
            q_cap = M67_AGGR_BUY_Q_JOINT if joint else M67_AGGR_BUY_Q_LOOSE
            q_take = min(q_cap, lim_u - pu)
            if q_take > 0:
