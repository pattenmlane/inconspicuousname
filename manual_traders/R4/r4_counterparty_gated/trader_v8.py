"""
Round 4 — v6 plus inventory-aware voucher skew: shift bid/ask in integer ticks from mid
based on signed position (fade building longs, encourage flattening). Same Mark01→Mark22
5300 fade under Sonic joint tight and Mark67 JSON lift as v6.
"""
...
_TH = 2.0
# Per-voucher inventory skew: |shift| ticks = min(MAX, |pos| // STEP); long -> bid/ask down.
INV_SKEW_STEP = 50
INV_SKEW_MAX = 3
...
def _inv_skew_ticks(pos: int) -> int:
    """Signed tick shift: long (+) inventory -> negative return (lower bid/ask); short -> raise."""
    if pos == 0:
        return 0
