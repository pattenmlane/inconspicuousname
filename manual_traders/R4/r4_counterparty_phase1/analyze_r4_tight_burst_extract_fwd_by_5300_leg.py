#!/usr/bin/env python3
"""VELVETFRUIT_EXTRACT forward mid at joint-tight + M01→M22 burst, split by
whether M01→Mark22 prints on VEV_5300 (Round 4 days 1–3).

Cross-asset analogue to r4_burst_m01_m22_5300_leg_fwd_multik.json: same timestamps,
but fwd on extract mid series aligned to VEV_5300 price-row timestamps (union ts).
"""
...
OUT = Path(__file__).resolve().parent / "r4_tight_burst_extract_fwd_by_5300_leg.json"
DAYS = (1, 2, 3)
TH = 2.0
KS = (5, 10, 20, 50)
BURST_MIN = 4
        ...
        with open(p, newline="") as f:
            ...
            for row in r:
                by[(d, int(row["timestamp"]))].append(
                    (str(row["symbol"]), str(row["buyer"]).strip(), str(row["seller"]).strip())
                )
    return by
...
def main() -> None:
        ...
        with open(path, newline="") as f:
            ...
            for row in r:
                ...
                if sym == S5200:
                    ...
                    tss.append(ts)
                    ex_mid.append(float(row["mid_price"]))

        if len(tss) < max(KS) + 2:
            continue
        ds = str(day)
        for ts in sorted(set(sp53.keys()) & set(sp52.keys())):
            if sp52[ts] > TH or sp53[ts] > TH:
                continue
            rows = tr.get((day, ts), [])
            if not burst_base(rows):
                continue
            wing = on_5300_leg(rows)
            for k in KS:
                fk = fwd(tss, ex_mid, ts, k)
                if fk is None:
                    continue
                for name in ("any_m01_m22_burst",):
                    key = (name, k)
                    pooled[key].append(fk)
                    per_day[ds][key].append(fk)
                if wing:
    ...
    def pack(d: dict[tuple[str, int], list[float]]) -> dict:
        return {
            name: {f"K{k}": stat(d.get((name, k), [])) for k in KS}
    ...
    out = {
        ...
        "KS": list(KS),
        "pooled": pack(pooled),
        "per_day": {d: pack(per_day[d]) for d in ("1", "2", "3")},
        "note": "Timestamps are VEV_5300 price rows where joint tight + burst; extract fwd uses extract mid series (may differ row index from 5300 series at same wall ts).",
    }
