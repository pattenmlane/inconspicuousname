#!/usr/bin/env python3
"""
VEV surface (5100-5500): aggressive sell (price<=bid1) fwd+20 by passive buyer,
split by Sonic joint tight. Same tight merge as r4_v9_hydro_aggr_sell_tight.py.
"""
...
def main() -> None:
    ...
    path_rows = OUT / "r4_v16_vev_aggrsell_fwd20_rows.csv"
    df.to_csv(path_rows, index=False)
    sub = df[df["tight"]]
    summ = (
        sub.groupby(["symbol", "buyer"])["fwd20"]
        .agg(["count", "mean", "median"])
        .reset_index()
        .sort_values(["mean", "count"], ascending=[True, False])
    )
    path_summ = OUT / "r4_v16_vev_aggrsell_tight_summary_by_symbol_buyer.csv"
    summ.to_csv(path_summ, index=False)
    print("Tight gate only — aggressive sell (price<=bid1); buyer = passive on bid")
    print(summ.to_string(index=False))
    print("Wrote", path_rows, path_summ)
