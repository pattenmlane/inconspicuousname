#!/usr/bin/env python3
"""HYDROGEL_PACK aggressive-buy fwd20 by passive seller, split by Sonic joint gate (R4)."""
...
def main() -> None:
    ...
    for d in DAYS:
        ...
        for _, r in tr.iterrows():
            ...
            tight = bool(r["tight"])
            rows.append({"day": d, "tight": tight, "seller": seller, "fwd20": fwd})
            counts.append({"day": d, "tight": tight})

    df = pd.DataFrame(rows)
    df.to_csv(OUT / "r4_v7_hydro_aggrbuy_fwd20_rows.csv", index=False)
    summ = df.groupby(["tight", "seller"])["fwd20"].agg(["count", "mean"]).reset_index().sort_values(["tight", "mean"])
    summ.to_csv(OUT / "r4_v7_hydro_aggrbuy_summary_by_tight_seller.csv", index=False)
    pd.DataFrame(counts).groupby(["day", "tight"]).size().reset_index(name="n_aggr_buy_hydro").to_csv(
        OUT / "r4_v7_hydro_aggrbuy_counts_by_day_tight.csv", index=False
    )
