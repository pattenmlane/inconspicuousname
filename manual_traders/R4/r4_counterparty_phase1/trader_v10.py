"""
Round 4 — **v9 + extend hydro bid-skip passive set to Mark 14** (tight gate only).

Offline summary `r4_v9_hydro_aggrsell_summary_by_tight_buyer.csv`: under **joint tight**,
aggressive hydro sells with passive **Mark 14** as buyer have mean fwd+20 **-0.46** on
n=126 (vs Mark 22 n=3, very negative but rare). `trader_v10` widens `SKIP_HYDRO_BUY_PASSIVE`
from {Mark 22} to {Mark 22, Mark 14} for the same `_skip_buy_aggr_sell` rule as v9.
"""
...
SKIP_HYDRO_BUY_PASSIVE = frozenset({"Mark 22", "Mark 14"})
...
class Trader:
    def run(self, state: TradingState):
        ...
        td["skip_h_buy_m14_m22"] = int(skip_h_buy)
