"""
Round 4 — **v11 + inventory-aware clip under joint tight** (Phase 2 style).

When Sonic joint gate is **on** and **|position| >= POS_TH** for a product, use **half** the
base clip for orders that **increase** absolute inventory (long: cap buys; short: cap sells).
Fills the rest of the stack unchanged (counterparty sell-skips, hydro buy-skip, wide trim).
"""
...
CLIP_H = 4
WIDE_TRIM = 2
# Skew when |pos| is large (tight gate only; units = contracts)
POS_TH = 50
...
def _skewed_clips(base: int, cur: int) -> tuple[int, int]:
    """Return (clip_for_buys, clip_for_sells) after inventory skew when |cur| is large."""
    if abs(cur) < POS_TH:
        return base, base
    if cur > 0:
        ...
        if tight:
            for p in tight_products:
                ...
                if p == U:
                    ...
                    base = CLIP_V
                buy_clip, sell_clip = _skewed_clips(base, cur)
                buy_px, sell_px = _passive_prices(bb, ask)
                can_buy = max(0, min(buy_clip, lim - cur))
                can_s = max(0, min(sell_clip, lim + cur))
