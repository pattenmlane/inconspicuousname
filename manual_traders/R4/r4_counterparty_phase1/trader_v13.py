"""
Round 4 — **v11 + inventory clip skew on VELVET only** (joint tight).

`trader_v12` applied half-clip skew to every product when |pos|>=50, which **hurt** PnL
(material drop on VEV_5300). `trader_v13` keeps v11's clips for HYDRO + VEVs and applies
the same v12 **skew** only to **VELVETFRUIT_EXTRACT** (`|pos|>=POS_TH` -> half clip on
the side that increases |inventory|).
"""
...
def _velvet_skewed_clips(base: int, cur: int) -> tuple[int, int]:
    if abs(cur) < POS_TH:
        return base, base
    if cur > 0:
    ...
    def run(self, state: TradingState):
        ...
        if tight:
            for p in tight_products:
                ...
                cur = int(pos.get(s, 0))
                if p == U:
                    bu, se = _velvet_skewed_clips(CLIP_U, cur)
                    can_buy = max(0, min(bu, lim - cur))
                    can_s = max(0, min(se, lim + cur))
                elif p == HY:
                    clip = CLIP_H
