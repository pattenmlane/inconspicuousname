"""Round 4: v12 + **skip / trim** on high Mark 22 sell qty on VEV_5300 in burst.

Tape (r4_phase2_burst_m22_5300_qty_vs_fwd.json): at joint tight + M01→M22 burst,
Mark22 sell quantity on 5300 in the print batch p75=4. High bucket mean fwd20 5300
~-0.15 vs low ~+0.9.

- Enter long 5300 only if burst active AND m22_sell_qty_5300 < M22_HIGH (default 4).
- Exit: after MIN_HOLD, if burst ended **or** m22_sell pressure is high (early trim).
  Same join-buy and ask-1 exit as v12.
"""
...
def _m22_sell_qty_5300(rows: list[tuple]) -> int:
    return sum(int(q) for sym, _b, s, _p, q in rows if sym == S5300 and s == "Mark 22")
...
class Trader:
    def run(self, state: TradingState):
        ...
        br = _burst_m01_m22(csv_day, ts)
        rows = _TR.get((csv_day, ts), [])
        m22q = _m22_sell_qty_5300(rows)
        regime = tight and br
        bad_m22 = regime and m22q >= M22_HIGH
        allow_entry = regime and m22q < M22_HIGH
        if regime:
        ...
        should_exit = p53 > 0 and (tick - last_active_tick) >= MIN_HOLD_TICKS and (not regime or bad_m22)
        if should_exit:
