"""Round 4: v12 + **Sonic pair-grid** filter (Phase 3 spread vs fwd panels).

Tape r4_phase3_spread_vs_fwd_panels.json: under joint tight, mean fwd20 on VEV_5300
for (s5200,s5300)=(1,2) is **negative** on days 1–2 and weakly negative day 3; other
integer cells (1,1), (2,1), (2,2) show non-negative pooled 5300 fwd in the table.

v16: same as v12 (burst, join-buy, ask-1 exit, LOT=6, MAX_POS=90) but only **buy**
when L1 spreads rounded to ints (s5200, s5300) are in **ALLOW** and not (1,2).

Exit unchanged vs v12 (regime off + MIN_HOLD).
"""
...
def _spr_pair_int(depth) -> tuple[int, int] | None:
...
def _joint_tight(depth) -> bool:
    pr = _spr_pair_int(depth)
    if pr is None:
        return False
    s5200, s5300 = pr
    return s5200 <= int(TH) and s5300 <= int(TH)


def _pair_allowed(depth) -> bool:
    pr = _spr_pair_int(depth)
    if pr is None:
        return False
    s5200, s5300 = pr
    if s5200 > int(TH) or s5300 > int(TH):
        return False
    if (s5200, s5300) == (1, 2):
        return False
    return True
...
class Trader:
    def run(self, state: TradingState):
        ...
        pair_ok = _pair_allowed(d)
        active = tight and burst
        can_buy = active and pair_ok
        if active:
