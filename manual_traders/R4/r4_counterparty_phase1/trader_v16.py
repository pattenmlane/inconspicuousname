"""
Round 4 — **v11 + tight-gate VEV buy-skip vs Mark14 aggressive sells (5300/5400/5500)**.

Tape (`r4_v16_vev_aggrsell_tight_summary_by_symbol_buyer.csv`): under **joint tight**,
**aggressive sell** (price<=bid1) with passive buyer **Mark 14** has **negative** mean
fwd+20 on **VEV_5300** (n=28, -0.25), **VEV_5400** (n=12), **VEV_5500** (n=7). **5200**
Mark14 slice is ~flat (n=33). Skip **buy** quotes on 5300/5400/5500 for the tick when
that pattern appears (tight only); symmetric to hydro Mark22 buy-skip in v9.
"""
...
# Tight + tape: Mark14 passive on aggr sell -> negative fwd20 on these strikes (not 5200)
VEV_SKIP_BUY_ON_M14_AGGR_SELL = frozenset({"VEV_5300", "VEV_5400", "VEV_5500"})
SKIP_VEV_BUY_PASSIVE = frozenset({"Mark 14"})
    ...
    def run(self, state: TradingState):
        ...
        if tight:
            for p in tight_products:
                ...
                skip_vev_buy = bool(
                    tight
                    and p in VEV_SKIP_BUY_ON_M14_AGGR_SELL
                    and _skip_buy_aggr_sell(state, s, SKIP_VEV_BUY_PASSIVE)
                )
                if skip_vev_buy:
                    skip_vev_m14_buy_any = True
                if (
                    not (p == HY and skip_h_buy)
                    and not skip_vev_buy
                    and can_buy > 0
                ):
        ...
        td["skip_vev_m14_buy"] = int(skip_vev_m14_buy_any)
