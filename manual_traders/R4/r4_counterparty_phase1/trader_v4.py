"""Round 4: gate + M01→M22 burst — **passive** bid join on VEV_5300 (vs v3 ask lift).

Tape 2x2 (r4_gate_burst_5300_fwd_2x2.json): tight+burst mean fwd20 ~+0.33 on n=291.
v3 lost at touch-ask; v4 posts **limit buy at best bid** when joint gate and burst
(fills when offer hits bid / book depletion). Flatten long at bid when regime drops.

Same (csv_day, timestamp) trade index as v3.
"""
...
class Trader:
    def run(self, state: TradingState):
        ...
        q = min(LOT, LIMITS[S5300] - p53, 24)
        if q > 0:
            o[S5300].append(Order(S5300, bid, q))
            td["last_sig"] = tick
