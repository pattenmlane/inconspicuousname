"""Round 4: v5 entry + **minimum hold after regime ends** before flatten.

Tape (r4_tight_burst_5300_fwd_horizons.json): under tight+burst, VEV_5300 forward
mid mean rises with horizon (K10 ~+0.28, K20 ~+0.33). v5 sold as soon as gate
or burst dropped.

Change vs v5: track last tick where joint gate+burst was active; only sell at bid
when inactive AND (tick - last_active_tick) >= MIN_HOLD_TICKS. Ablations on this
tape: MIN_HOLD=10 matches v5 PnL (~-132 worse); MIN_HOLD=20 worsened (~-168).
Same join-buy price, LOT, cooldown, max pos as v5.
"""
...
class Trader:
    def run(self, state: TradingState):
        ...
        if not active and p53 > 0 and d53.buy_orders:
