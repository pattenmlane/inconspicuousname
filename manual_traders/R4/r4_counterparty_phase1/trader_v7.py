"""
Round 4 — **v4 + gated HYDROGEL_PACK MM** (Mark 14/22/49 adverse skip on hydro sells).

Extends `trader_v4.py`: when Sonic joint gate is **tight**, also two-sided passive quote
**HYDROGEL_PACK** (clip 4). Skip **hydro sell** quotes for the tick when tape shows
**aggressive buy** (price >= ask1) with passive seller in **Mark 14, Mark 22, Mark 49**
— Phase 1 hydro row shows Mark14 passive worst mean fwd20 on aggressive buys; 22/49 are
the duopoly partners on hydro in round4work summary.

Wide regime: trim hydro inventory with same small unwind as VEV/U.
"""
...
class Trader:
    def run(self, state: TradingState):
        ...
        skip_h = bool(h_sym and _skip_sell_aggr_buy(state, h_sym, SKIP_HYDRO_SELL))
        td["skip_u_sell"] = int(skip_u)
        td["skip_h_sell"] = int(skip_h)
        ...
        if tight:
            for p in tight_products:
                ...
                skip_sell = (p == U and skip_u) or (p == HY and skip_h)
                if skip_sell:
                    continue
                if can_s > 0:
