"""Round 4: v6 + **improved passive exit** (mirror of v5 entry).

v5 buys at min(bid+1, ask) when spread>=2 else bid. v6/v5 sell at bid only.

Exit: limit sell at max(ask-1, bid) when spread>=2 else at ask (join ask on 1-tick
book — same as ask-1=bid). Symmetric microstructure to join-buy.

Regime / hold / entry unchanged vs v6 (gate+burst, MIN_HOLD=10, join buy).
"""
...
def _improve_sell_price(d53) -> int | None:
    """Passive-leaning limit sell: one tick inside spread when wide, else touch ask."""
    if not d53.buy_orders or not d53.sell_orders:
        return None
    bid = max(d53.buy_orders)
    ask = min(d53.sell_orders)
