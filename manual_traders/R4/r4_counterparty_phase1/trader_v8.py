"""Round 4: v7 entry filter — only long when M01→Mark22 leg is **on VEV_5300**.

v7 fires on joint tight + any-symbol burst (>=4 rows, some M01→M22). Tape analysis
(r4_burst_m01_m22_5300_leg_fwd20.json) compares fwd20 when that leg hits 5300 vs
basket-only bursts; v8 requires the 5300 leg for counterparty alignment with the
traded wing.

Exit / join-buy / MIN_HOLD same as v7.
"""
...
def _burst_m01_m22_on_5300(csv_day: int, ts: int) -> bool:
    ...
    if not any(b == "Mark 01" and s == "Mark 22" for _sym, b, s, _p, _q in rows):
        return False
    return any(
        sym == S5300 and b == "Mark 01" and s == "Mark 22"
        for sym, b, s, _p, _q in rows
    )
...
class Trader:
    def run(self, state: TradingState):
        ...
        burst = _burst_m01_m22_on_5300(csv_day, ts)
