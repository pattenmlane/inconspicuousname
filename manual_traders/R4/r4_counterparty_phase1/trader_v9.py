"""Round 4: v7 entry filter — M01→Mark22 **basket-only** (exclude when leg is on VEV_5300).

Tape (r4_burst_m01_m22_5300_leg_fwd20.json): at joint tight, pooled mean VEV_5300
fwd20 is **+0.70** when M01→M22 appears only on other symbols (n=160), vs **-0.12**
when that leg also prints on VEV_5300 (n=131). v9 enters only on the basket-only
subset; same exit/join-buy/MIN_HOLD as v7.
"""
...
def _burst_m01_m22_basket_only_no_5300_leg(csv_day: int, ts: int) -> bool:
...
class Trader:
    def run(self, state: TradingState):
        ...
        burst = _burst_m01_m22_basket_only_no_5300_leg(csv_day, ts)
