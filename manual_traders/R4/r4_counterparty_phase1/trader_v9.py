"""
Round 4 — **v7 + hydro bid skip vs Mark22 aggressive sells (tight gate only)**.

Tape analysis (`r4_v9_hydro_aggrsell_summary_by_tight_buyer.csv`): **aggressive sell** on
HYDROGEL (price <= bid1) with passive **Mark 22** as buyer has **very negative** mean
hydro fwd+20, especially under **joint tight** (mean ~ -6.67, n=3).

`trader_v9` = `trader_v7` plus: when **tight**, skip **HYDROGEL buy** quotes for the tick
when tape shows aggressive sell through bid with passive buyer **Mark 22** only
(symmetric adverse-selection to the existing hydro sell-skip on aggressive buys).
"""
...
# Tight-gate only: tape under-tight shows Mark22 passive worst on aggressive sells
...
def _skip_buy_aggr_sell(state: TradingState, listing_sym: str, passive_buyers: frozenset) -> bool:
    """Aggressive sell (price <= bid1) with passive buyer in set."""
    mt = getattr(state, "market_trades", None) or {}
    if not isinstance(mt, dict):
        return False
    lst = mt.get(listing_sym)
    ...
    def run(self, state: TradingState):
        ...
        if tight:
            for p in tight_products:
                ...
                skip_sell = (p == U and skip_u) or (p == HY and skip_h_sell)
                if skip_sell:
                    continue
                if can_s > 0:
