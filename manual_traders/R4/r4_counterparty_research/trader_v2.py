"""
Round 4 counterparty-aware MM (research → execution probe).

Base: vouchers_final_strategy gate (VEV_5200 & VEV_5300 spread <= 2) + intrinsic + DTE-scaled
time value (same fair as manual_traders/R3_VEV/.../trader_v24.py).

Round 4: infer tape day from ts=0 VELVETFRUIT_EXTRACT mid; DTE proxy dte = (5 - day) - intraday
(TTE steps down across sim days 1–3).

Counterparty (execution-time): at the start of each tick, ``state.own_trades`` still holds fills
from the previous timestamp’s match step. If we lifted sell liquidity on a VEV and the tape
seller was Mark 22 (backtester sets ``buyer='SUBMISSION'``, ``seller`` from the market trade),
widen that strike’s quotes next tick (+1 edge, slightly smaller size). This mirrors Phase 1’s
“Mark 22 sells / adverse selection” theme without needing same-tick tape bursts (not visible in
``TradingState`` before we act).

HYDROGEL_PACK: light mid MM (wider edge than U).
"""
...
# R4 historical open mids (prices_round_4_day_{1,2,3} timestamp 0)
ANCHOR = {
    5245.0: 1,
    5267.5: 2,
    5295.5: 3,
...
def _vev_bought_from_m22_last_tick(own_trades: dict[str, list]) -> set[str]:
    """Our buy fills where the passive seller on tape was Mark 22 (previous timestamp)."""
    out: set[str] = set()
