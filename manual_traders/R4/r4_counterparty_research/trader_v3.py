"""
Round 4 counterparty-aware MM — v3 extends v2.

Same base as trader_v2 (Sonic gate, DTE-scaled fair, hydro MM).

Counterparty (prior tick ``own_trades``):
- Buy VEV from passive Mark 22 seller: widen next tick (+1 edge, -2 size) [same as v2].
- Sell VEV to aggressive Mark 22 buyer: widen next tick (+1 edge, -2 size) [new in v3].
  Backtester labels: ``quantity < 0``, ``buyer == Mark 22``, ``seller == SUBMISSION``.
"""
...
def _vev_mark22_pressure_syms(own_trades: dict[str, list]) -> set[str]:
    """Strikes where last tick we either bought from M22 (passive sell) or sold to M22 (aggressive buy)."""
    out: set[str] = set()
    for sym, lst in own_trades.items():
        ...
        for t in lst:
            q = int(t.quantity)
            if q > 0 and getattr(t, "buyer", None) == SUBMISSION and t.seller == MARK_22:
                out.add(sym)
            elif q < 0 and t.buyer == MARK_22 and getattr(t, "seller", None) == SUBMISSION:
                out.add(sym)
    return out
...
class Trader:
    def run(self, state: TradingState):
        ...
        m22_syms = _vev_mark22_pressure_syms(state.own_trades or {})
        ...
        td["mark22_vev_pressure_prev_tick"] = sorted(m22_syms)
