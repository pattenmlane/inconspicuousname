"""
Round 4 MM — v4 = v2 + Phase-3-aligned extract tilt.

Phase 3 (r4_phase3_gate_fwd_u_k20_pooled.json): mean forward extract mid move at K=20 rows
is higher when Sonic joint gate (5200+5300 spread <= 2) is tight vs loose. v4 applies that
only to VELVETFRUIT_EXTRACT quoting (VEVs and hydro unchanged from v2):

- Tight gate: extract edge 0 (was 1 in v2), size 24 (was 22).
- Loose gate: extract edge 4 (was 3 in v2), size 7 (was 8).

Mark 22 post-buy widen on VEVs unchanged from trader_v2.
"""
...
# Extract-only overrides vs v2 schedule (v2 used same eu,su as VEV tight/wide)
MM_EDGE_U_TIGHT_EXTRACT = 0
SIZE_U_TIGHT_EXTRACT = 24
MM_EDGE_U_LOOSE_EXTRACT = 4
SIZE_U_LOOSE_EXTRACT = 7
...
class Trader:
    def run(self, state: TradingState):
        ...
        if tight:
            ...
            eu_u, su_u = MM_EDGE_U_LOOSE_EXTRACT, SIZE_U_LOOSE_EXTRACT
