"""
Round 4 — **v10**: same logic as **v8**; **HYDROGEL_PACK** half and size read from **env**
so sweeps do not require file edits.

- **R4_HYDRO_HALF** (default ``2.2``): hydro half-spread in ticks (float).
- **R4_HYDRO_SIZE** (default ``14``): per-side size clip (int).

Extract path unchanged from **v8** / **v3**. **No VEV** orders.
"""
