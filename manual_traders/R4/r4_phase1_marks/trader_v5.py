"""
Round 4 — **v5**: **v4** + optional **EXTRA_VEV_HALF** on **VEV_5200/5300** quotes only.

Tape (`outputs/phase5_01_22_bbo_spreads_at_prints.*`): at Mark01→Mark22 prints, mean
**s5300** is often **≈2** (threshold). Hypothesis: widen VEV half-spread for `worse`.

**Grid on `worse`:** **EXTRA_VEV_HALF** ∈ {−0.5, −0.25, 0, 0.5, 1.0, 1.5, 2.0} and
**SIZE_VEV_ATM** ∈ {8, 12, 20, 32, 40} — **all** totals **3382** (same as **v3**;
VEV lines **0** under adverse matching). **`all`:** with **EXTRA=0.5**, VEV PnL **0**
(total **6036** = extract-only); **v4** (`EXTRA=0`) gets large VEV **`all`** PnL.

Locked **EXTRA_VEV_HALF = 0.5** as the explored default; **prefer `trader_v4`**
(`EXTRA=0`) for `all`-mode hedge surface.

**No HYDROGEL_PACK**, no other VEVs.
"""
...
WING_KM_SQ = 280.0
SIZE_VEV_ATM = 20
# Added to VEV_5200/5300 half (can be negative = tighter; grid in results.json)
EXTRA_VEV_HALF = 0.5
...
class Trader:
    def run(self, state: TradingState):
        ...
        store["extra_vev_half"] = EXTRA_VEV_HALF
        ...
        for p in QUOTE_VEVS:
            ...
            half = BASE_VEV_HALF + WING_KM_SQ * (km**2) + EXTRA_VEV_HALF
