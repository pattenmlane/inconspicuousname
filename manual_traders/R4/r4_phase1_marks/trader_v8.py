"""
Round 4 — **v8**: **v3** (joint gate + scaled extract half) + **HYDROGEL_PACK** mid MM when tight.

**Tape motivation:** ``analyze_r4_gate_pair_stability.py`` / ``phase8_*`` — **Mark14↔Mark38**
**HYDROGEL_PACK** prints show **higher mean fwd_EXTRACT_20** under **joint tight** than
loose (pooled Welch **p<0.02**), with **positive delta on all three** R4 days for both
directions. This tests whether adding a **hydro** quote leg under the same Sonic gate
changes **sim** PnL vs **v3** (VEV objective is unchanged; hydro is exploratory).

**No VEV** orders. Requires **VEV_5200**, **VEV_5300**, **VELVETFRUIT_EXTRACT**, **HYDROGEL_PACK**
in book when gate is tight.
"""
