"""Round 2 submission osmium: ``MeanReversionMarketMaker`` from ``round2work/litests/potential3.py``.

Canonical copy for upload lives in ``round2submission/`` (synced from
``round2work/submission_exports_p2_variants/potential3_osmium_only.py``).
Rolling-window mean, Bayesian blend toward 10_000, mean-reversion skew, edge 1.5.
``bid()`` returns 0 (same as litests ``potential3``).
"""
