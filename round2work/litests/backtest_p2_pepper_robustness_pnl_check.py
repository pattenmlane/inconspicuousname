#!/usr/bin/env python3
"""
Compare PnL: ``potential2_pepper_only_pre_robustness_snapshot`` (legacy) vs
``round2submission/potential2_pepper_only.py`` (current).

* All days under ``Prosperity4Data`` (rounds 1 and 2)
* Every Round 2 day-29 zip + ``combined_all_including_extra`` (if present)

Exits 0 only if pepper (and total) PnL match exactly on every run. ``match_trades=worse``.

From repo root::

  PYTHONPATH="$PWD/imc-prosperity-4-backtester:$PWD/imc-prosperity-4-backtester/prosperity4bt:$PWD/round2work/litests" \\
  python3 round2work/litests/backtest_p2_pepper_robustness_pnl_check.py
"""
...
def _pnl_pepper(d: dict[str, float]) -> float:
    return float(d.get(PEPPER, 0.0))


def _pnl_total(d: dict[str, float]) -> float:
...
def run_day29_tape(
    ...
    root = Path(tempfile.mkdtemp(prefix="p2pepchk_"))
    try:
...
def main() -> None:
    ...
    ap.add_argument(
        "--current",
        type=Path,
        default=REPO / "round2submission" / "potential2_pepper_only.py",
    )
    ap.add_argument(
        "--legacy",
        type=Path,
        default=HERE / "potential2_pepper_only_pre_robustness_snapshot.py",
    )
    ...
    if not cur_path.is_file():
        raise SystemExit(f"Missing current trader: {cur_path}")
    if not leg_path.is_file():
        raise SystemExit(f"Missing legacy snapshot: {leg_path}")

    _bootstrap_paths()

    TraderCur = load_trader_from_path(cur_path, "_p2_pepper_robust_current")
    TraderLeg = load_trader_from_path(leg_path, "_p2_pepper_robust_legacy")
    ...
    def check(label: str, run: Callable[[type], dict[str, float]]) -> None:
        ...
        pl, pc = _pnl_pepper(d_leg), _pnl_pepper(d_cur)
        tl, tc = _pnl_total(d_leg), _pnl_total(d_cur)
        rows.append((label, pl, pc, tl, tc))
        if pl != pc or tl != tc:
            failures.append(
                f"{label}: legacy pepper={pl} current pepper={pc} | legacy total={tl} current total={tc}"
            )
    ...
    if not args.skip_day29:
        ...
        if not all_z:
            print("WARN: no day-29 zips; skipping day-29 section", file=sys.stderr)
        else:
            for z in main_z:
                check(f"day29 zip day 29 logs/{z.name}", lambda T, zp=z: run_day29_tape(T, zp, comb_dir))
            for z in extra_z:
                check(f"day29 zip extra/{z.name}", lambda T, zp=z: run_day29_tape(T, zp, comb_dir))
            p = comb_dir / "prices_combined_all_runs.csv"
            t = comb_dir / "trades_combined_all_runs.csv"
            if p.is_file() and t.is_file():
                check("day29 combined_all_including_extra", lambda T: run_day29_tape(T, None, comb_dir))
            else:
                print(f"WARN: missing combined tapes under {comb_dir}; skip combined row", file=sys.stderr)

    w = sys.stdout.write
    w("p2 pepper-only: legacy snapshot vs current (match_trades=worse)\n")
    w("label | legacy_pepper | current_pepper | legacy_total | current_total\n")
    w("-" * 100 + "\n")
    for label, pl, pc, tl, tc in rows:
        ok = "OK" if (pl == pc and tl == tc) else "MISMATCH"
        w(f"{ok}  {label[:60]:<60}  {pl:,.2f}  {pc:,.2f}  {tl:,.2f}  {tc:,.2f}\n")
    ...
    if failures:
        ...
        for f in failures:
            w(f"  {f}\n")
        raise SystemExit(1)
    w("\nAll checks passed: identical PnL on every run.\n")
