"""Copy of ``potential2_osmium_only.py`` with a different empty-book passive rule.

When **both** best bid and best ask are missing (no touch on either side), the
original uses ``mid_price ± gap_spread`` with ``gap_spread = 99``. Here we
instead anchor on **previous tick's deep mid** ``prev_mid`` (persisted in
``traderData``), falling back to the fast EWM ``mid_price``, then to the old
``mid ± 99`` behavior if no anchor exists.

When only **one** side is missing, behavior is unchanged (still ``mid ± 99``).
"""
...
class BaseTrader:
    ...
    def _both_touch_sides_missing(self) -> bool:
        return self.best_bid is None and self.best_ask is None

    def _anchor_both_empty(self, mid_price: object) -> float | None:
        """Anchor for passive quotes when both touch sides missing: last deep mid, else EWM."""
        prev = self.saved_data.get(PREV_MID_KEY)
        if prev is not None:
            try:
                return float(prev)
    ...
    def override_best_bid(self, max_quantity, fair_price, gap_spread, mid_price):
        if self.best_bid and self.best_bid < fair_price - 1:
            ...
            if self._both_touch_sides_missing():
                anchor = self._anchor_both_empty(mid_price)
                if anchor is not None:
                    self.buy(int(round(anchor)) - 1, max_quantity=max_quantity)
                else:
                    self.buy(round(float(mid_price) - gap_spread), max_quantity=max_quantity)
            else:
                self.buy(round(float(mid_price) - gap_spread), max_quantity=max_quantity)
    ...
    def override_best_ask(self, max_quantity, fair_price, gap_spread, mid_price):
        if self.best_ask and self.best_ask > fair_price + 1:
            ...
            if self._both_touch_sides_missing():
                anchor = self._anchor_both_empty(mid_price)
                if anchor is not None:
                    self.sell(int(round(anchor)) + 1, max_quantity=max_quantity)
                else:
                    self.sell(round(float(mid_price) + gap_spread), max_quantity=max_quantity)
            else:
                self.sell(round(float(mid_price) + gap_spread), max_quantity=max_quantity)
...
class MeanReversionMarketMaker(BaseTrader):
    def get_orders(self):
        ...
        if self.mid is not None:
            self.saved_data[PREV_MID_KEY] = float(self.mid)
