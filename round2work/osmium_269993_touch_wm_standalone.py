"""
Osmium-only leg from ``R1submissionlogs/269993.zip`` (WM spike freeze + touch stress
freeze + optional width boost).

Same logic as the osmium block in ``269993.py``; no pepper orders.

``traderData`` JSON: ``osm_prev_wall``, ``osm_last_fv``, ``osm_last_wall_mid``,
``osm_freeze_left``, ``osm_frozen_fv``, ``osm_ts_*``, ``osm_prev_touch_*``.
"""
