"""
Osmium-only leg from ``R1submissionlogs/273774.zip`` (wall mid + WM spike freeze).

Same logic as the osmium block in ``273774.py``; no pepper orders.

``traderData`` JSON: ``osm_prev_wall``, ``osm_last_fv``, ``osm_freeze_left``, ``osm_frozen_fv``.
"""
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        if OSMIUM in state.order_depths:
            ...
            prev_wall = store.get("osm_prev_wall")
            last_fv = store.get("osm_last_fv")
            freeze_left = int(store.get("osm_freeze_left", 0) or 0)
            freeze_left_start = freeze_left
            frozen_raw = store.get("osm_frozen_fv")
            ...
            if spike_this_tick and n_freeze > 0:
                ...
                fv = float(last_fv) if last_fv is not None else _fair_osmium_no_store(depth_os)
                if freeze_left <= 0:
                    frozen_fv = None
