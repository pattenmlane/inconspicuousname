"""
Copy for uploads / experiments: lives under ``round2work/submission_exports_pepper_variants/``.
Upstream: ``submissions/test_long80_pepper_osmium_273774style.py``.

Test trader: **pepper** — always try to reach **+80** long with one aggressive
order per tick (best ask when under target, best bid when over). No drift, no
rolling slope.

**Osmium** — same wall-mid Emerald MM + **wall spike freeze** as submission
273774 / ``pepper_70_10_osmium_spike_freeze_combined`` (``OSMIUM_WMFN_JSON``).

Backtest on synthetic **day 119** (exported submission book)::

  PYTHONPATH="$PWD/imc-prosperity-4-backtester:$PWD/imc-prosperity-4-backtester/prosperity4bt" \\
  python3 -m prosperity4bt "$PWD/submissions/test_long80_pepper_osmium_273774style.py" 1-119 \\
...
class Trader:
    ...
    def _pepper_long_80_orders(self, depth: OrderDepth, position: int) -> List[Order]:
        if not depth.buy_orders or not depth.sell_orders:
            return []
    ...
    def run(self, state: TradingState):
        ...
        if depth_pe is not None:
            ...
            po = self._pepper_long_80_orders(depth_pe, pos_pe)
            if po:
                result[PEPPER] = po
