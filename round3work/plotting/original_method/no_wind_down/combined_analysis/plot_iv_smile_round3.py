...
**This copy (no_wind_down):** calendar DTE only — T = dte_from_csv_day(day)/365 with **no**
intraday winding (timestamp ignored for T). For winding, use `original_method/combined_analysis/`
or `original_method/wind_down/combined_analysis/` (symlink).
...
# This file lives under .../no_wind_down/combined_analysis/ → six parents to repo root.
...
def dte_effective(day: int, timestamp: int) -> float:
    """Calendar DTE only (no intraday winding in this branch). `timestamp` kept for API parity."""
    _ = timestamp
    return max(float(dte_from_csv_day(day)), 1e-6)


def t_years_effective(day: int, timestamp: int) -> float:
    _ = timestamp
    return float(dte_from_csv_day(day)) / 365.0
...
def main() -> None:
    ...
    for ax, day in zip(axes, (0, 1, 2)):
        ...
        ax.set_title(f"day={day} (DTE open={dte_from_csv_day(day)}, no intraday winding)")
    ...
    for ax, day in zip(axes, (0, 1, 2)):
        ...
        ax.set_title(f"day={day} DTE open={dte_from_csv_day(day)} (subsampled, no wind)")
        ax.set_xlabel("log(K/S)")
        ax.set_ylabel("IV")
        ax.grid(True, alpha=0.3)
