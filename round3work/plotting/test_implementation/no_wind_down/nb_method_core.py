...
- T (years) = days_to_expiry_open / 365 **constant within each CSV day** (no session wind-down).
...
_OM = Path(__file__).resolve().parent.parent.parent / "original_method" / "no_wind_down" / "combined_analysis"
...
# .../test_implementation/no_wind_down/this_file → repo = five .parent hops.
...
def expiration_time_years(dte_open: int, t_index: int) -> float:
    """Calendar T only: ignore row index / intraday decay (matches no_wind_down combined_analysis)."""
    _ = t_index
    return max(float(dte_open) / 365.0, 1e-12)
