...
Output: round3work/plotting/test_implementation/ (mirrored filenames).

Run from repo:  python3 round3work/plotting/test_implementation/run_nb_method_plots.py
Or cd round3work/plotting && python3 test_implementation/run_nb_method_plots.py
"""
...
_PLOT_DIR = Path(__file__).resolve().parent.parent
_ORIG = _PLOT_DIR / "original_method" / "combined_analysis"
...
def plot_iv_style(ivdf: pd.DataFrame, resdf: pd.DataFrame) -> None:
    ...
    for ax, day in zip(axes, (0, 1, 2)):
        ...
        ax.set_title(f"day={day} (DTE open={dte_from_csv_day(day)}, notebook T)")
        ax.set_xlabel(r"$m=\log(K/S)/\sqrt{T}$")
        ax.set_ylabel("IV")
